...
#
# This makefile requires the environment variable RAFAEL_IOT_SDK_PATH to be set to the top-level rt58x_sdk directory
...
$(warning rt58x_sdk build system only supports GNU Make versions 3.81 or newer. You may see unexpected results with other Makes.)
endif
...
$(error rt58x_sdk build system doesn't support running 'clean' targets along with any others. Run 'make clean' and then run other targets separately.)
endif
endif

OS ?=

# make RAFAEL_IOT_SDK_PATH a "real" absolute path
...
SANITISED_RAFAEL_IOT_SDK_PATH:=$(realpath $(RAFAEL_IOT_SDK_PATH))
else
SANITISED_RAFAEL_IOT_SDK_PATH:=$(realpath $(wildcard $(RAFAEL_IOT_SDK_PATH)))
endif

export RAFAEL_IOT_SDK_PATH := $(SANITISED_RAFAEL_IOT_SDK_PATH)

ifndef RAFAEL_IOT_SDK_PATH
$(error RAFAEL_IOT_SDK_PATH variable is not set to a valid directory.)
endif

ifneq ("$(RAFAEL_IOT_SDK_PATH)","$(SANITISED_RAFAEL_IOT_SDK_PATH)")
$(error If RAFAEL_IOT_SDK_PATH is overriden on command line, it must be an absolute path with no embedded shell special characters)
endif

ifneq ("$(RAFAEL_IOT_SDK_PATH)","$(subst :,,$(RAFAEL_IOT_SDK_PATH))")
$(error RAFAEL_IOT_SDK_PATH cannot contain colons. If overriding RAFAEL_IOT_SDK_PATH on Windows, use MSYS Unix-style /c/dir instead of C:/dir)
endif
...
COMMON_MAKEFILES := $(abspath $(RAFAEL_IOT_SDK_PATH)/make_scripts/project.mk $(RAFAEL_IOT_SDK_PATH)/make_scripts/common.mk $(RAFAEL_IOT_SDK_PATH)/make_scripts/component_wrapper.mk $(firstword $(MAKEFILE_LIST)))
...
COMPONENT_DIRS := $(PROJECT_PATH)/components $(EXTRA_COMPONENT_DIRS) $(RAFAEL_IOT_SDK_PATH)/components $(RAFAEL_IOT_SDK_PATH)/customer_components $(PROJECT_PATH)/$(PROJECT_NAME) $(PROJECT_COMPONENT)
endif
...
ifeq ("$(wildcard ${RAFAEL_IOT_SDK_PATH}/components.mk)","")
...
include $(RAFAEL_IOT_SDK_PATH)/components.mk
...
include $(RAFAEL_IOT_SDK_PATH)/make_scripts/common.mk
include $(RAFAEL_IOT_SDK_PATH)/make_scripts/toolchain.mk
...
bins: all
	@cd $(RAFAEL_IOT_SDK_PATH)/image_conf; python3 flash_build.py $(PROJECT_NAME) $(CONFIG_CHIP_NAME)
info: all
	@cd $(RAFAEL_IOT_SDK_PATH)/image_conf; env SDK_APP_BIN=$(APP_BIN) SDK_APP_MAP=$(APP_MAP) python3 codesize.py

ifeq ("$(wildcard ${RAFAEL_IOT_SDK_PATH}/version.mk)","")
ifeq ("$(wildcard ${PROJECT_PATH}/version.mk)", "")
RAFAEL_SDK_VER := $(shell cd ${RAFAEL_IOT_SDK_PATH} && git describe --always --tags --dirty)
EXTRA_CPPFLAGS ?=
EXTRA_CPPFLAGS += -D RAFAEL_SDK_VER=\"$(RAFAEL_SDK_VER)\"
$(info use git describe to generate Version Define)
else
include $(PROJECT_PATH)/version.mk
$(info use existing project version.mk filee)
endif
else
include $(RAFAEL_IOT_SDK_PATH)/version.mk
$(info use existing version.mk file)
endif
RF_CHIP_NAME := ${CONFIG_CHIP_NAME}
RF_BOARD_NAME := $(CONFIG_TARGET_BOARD)

# Set default LDFLAGS
# -nostdlib
# --specs=nosys.specs
EXTRA_LDFLAGS ?= -Wl,--cref -nostartfiles -nostdlib -nodefaultlibs -nolibc
...
E21_CPU_LDFLAGS := -mcpu=cortex-m3 \
		-mabi=aapcs 
...
EXTRA_CPPFLAGS ?=
CPPFLAGS += -DRF_CHIP_NAME=\"$(RF_CHIP_NAME)\" -MMD -MP $(EXTRA_CPPFLAGS) -DCONFIG_TARGET_BOARD=\"$(CONFIG_TARGET_BOARD)\" -DCHIP_NAME=\"$(CONFIG_CHIP_NAME)\"
CPPFLAGS += -DARCH_ARMCM3
...
# Add -fshort-enums temporarily for RT582
ifeq ("$(CONFIG_CHIP_NAME)", "RT582")
COMMON_FLAGS += -fshort-enums
endif

ifeq ("$(CONFIG_CHIP_NAME)", "RT581")
COMMON_FLAGS += -fshort-enums
endif
...
E21_CPU_CFLAGS := $(strip \
		-mcpu=cortex-m3 -DUSE_FREERTOS -nostartfiles -nostdlib -nodefaultlibs -ffreestanding -lnosys \
	)
...
## TODO move to rt58x_elftool component
$(APP_BIN): $(APP_ELF)
...
+$(MAKE) -C $(BUILD_DIR_BASE)/$(2) -f $(RAFAEL_IOT_SDK_PATH)/make_scripts/component_wrapper.mk COMPONENT_MAKEFILE=$(1)/component.mk COMPONENT_NAME=$(2)
endef
