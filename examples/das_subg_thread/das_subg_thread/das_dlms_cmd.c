...
static uint8_t On_Demand_Type = 0; //On Demand Datatype 3: Load Profile, 4: Midnight, 5: ALT, 6: Event Log, 7:immediate REG
...
static uint8_t AUTO_AA = 0, AUTO_AA_FLAG = 1;//Do AA first when auto reading the meter, only do once AA 1:start 0:enter
static uint8_t DISC_FLAG = 0;
//Power off record 0:power on 1:power off ; 1:power off 15 Second; Actively read loadprofile every hour; Read midnight at 12 midnight; Read ALT notification every two hours while running only once.
static uint8_t EVENT_LOG_POWER_ON = 0, NOT_COMPLETELY_POWER_OFF = 0,FIRST_NOT_COMPLETELY_POWER_ON_FLAG = 1,auto_read_loadprofile = 0 ,auto_read_Midnight = 0, auto_read_ALT = 0, POWERONFLAG = 0 , POWER_ON_NOTIFICATION_FLAG =0, POWERON_Restart =0; 
...
//Task code : Automatic meter reading, Number of tasks Reading tasks Do AA before A3
static uint8_t Tesk_ID = 0, On_Demand_ID = 0, Auto_Reading = 0,Task_count = 0, On_Demand_Reading_Type = 0, Task_count_all = 0, ON_DEMAND_AA_FLAG = 0;
static unsigned char On_Demand_Task_Command[200]={0},On_Demand_Task_Buff[1000] = {0};//A3一堆命令 A3一堆回應  
...
static uint8_t tpc_mkey[16] = {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,0x1F,0x20};

static uint8_t tpc_guk[16]=
	//{0xCD,0x78,0x12,0xE5,0xAA,0xD6,0xE9,0x9E,0xA1,0x52,0xD0,0xA6,0x20,0x1D,0x0A,0x4C};	//Meter 70 GUK  18701666
	//{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f}; // test
	//{0xDC,0xB0,0x06,0xB9,0x82,0xE8,0xEC,0xF9,0x69,0xA6,0x6B,0x5D,0x55,0x81,0x22,0x31};	// GD18667512
	{0x03,0x4E,0x49,0x0E,0xA9,0x33,0xE0,0xA7,0xA9,0xBC,0x40,0xAA,0xCE,0xB5,0x14,0xD8};	//GD18667593
	//{0xEE, 0x67, 0xEB, 0x5F, 0xAF, 0xB7, 0x1A, 0x39, 0xD5, 0xFF, 0x41, 0x34, 0x3C, 0xED, 0x76, 0x30};//GD18667515
	//{0x94,0x12,0xd1,0xd7,0xb0,0x5e,0x47,0x5a,0xbb,0xeb,0x65,0xe1,0x92,0x47,0x47,0x27};	//tpc test 00030040
static uint8_t tpc_dk[16]=
	{0xB5, 0x07, 0xE3, 0x86, 0xEB, 0x00, 0x66, 0xF4, 0xFA, 0x0B, 0xDB, 0x14, 0x63, 0xB6, 0x69, 0xFB}; // DK

static uint8_t tpc_c_iv[12]=
  {0x4D, 0x41, 0x4E, 0x01, 0x23, 0x45, 0x67, 0x89, // Client System Title
   0x00, 0x00, 0x00, 0x01}; // IC

static uint8_t tpc_c_iv_h[12]=
  {0x48, 0x41, 0x4E, 0x01, 0x21, 0xA8, 0x8C, 0x02, // Client System Title
   0x00, 0x00, 0x00, 0x01}; // IC
	
static uint8_t tpc_s_iv[12]=
  {0x20, 0x54, 0x30, 0x00, 0x00, 0xBC, 0x61, 0x4E, // Server System Title
   0x00, 0x00, 0x00, 0x01}; // IC

static uint8_t tpc_s_iv_h[12]=
	{0x20, 0x54, 0x30, 0x00, 0x00, 0xBC, 0x61, 0x4E, // Server System Title
   0x00, 0x00, 0x00, 0x01};
...
static uint8_t tpc_gcm_ak[17]=
  {0x30, // SC
	...
	//0xd0, 0xd1, 0xd2, 0xd3, 0xd4, 0xd5, 0xd6, 0xd7, 0xd8, 0xd9, 0xda, 0xdb, 0xdc, 0xdd, 0xde, 0xdf}; // test
	//0xD2, 0xCB, 0xCC, 0xA5, 0x04, 0xBC, 0x0C, 0x8A, 0xC8, 0xED, 0x60, 0xDD, 0xDF, 0xB1, 0xA7, 0x78};//GD18667515
	0x90,0x68,0x41,0x85,0x03,0x9F,0x8C,0xAB,0x7B,0x4E,0x86,0x64,0xB1,0x41,0x25,0x3C};//GD18667593
	//0x0e,0x09,0x90,0x5c,0x1d,0x09,0xed,0xdf,0x90,0xda,0xf0,0xf9,0x95,0xed,0x30,0x62 };//tpc test 00030040
static uint8_t tpc_gmac_ak[25]=
  {0x10, // SC
	//0xd0, 0xd1, 0xd2, 0xd3, 0xd4, 0xd5, 0xd6, 0xd7, 0xd8, 0xd9, 0xda, 0xdb, 0xdc, 0xdd, 0xde, 0xdf, // Meter 70 AK
  //0xD2, 0xCB, 0xCC, 0xA5, 0x04, 0xBC, 0x0C, 0x8A, 0xC8, 0xED, 0x60, 0xDD, 0xDF, 0xB1, 0xA7, 0x78,//GD18667515
	//0xB3,0xC2,0x99,0x5E,0x4A,0x63,0x75,0xDE,0x5F,0x05,0x04,0xD2,0x06,0x1F,0x68,0x48,//18701666 
	//0xE6,0x9C,0xD0,0xFF,0x07,0xB8,0x05,0x32,0x9E,0xB1,0x23,0x64,0x0B,0x9B,0x2F,0x8B,//GD18667512	
	0x90,0x68,0x41,0x85,0x03,0x9F,0x8C,0xAB,0x7B,0x4E,0x86,0x64,0xB1,0x41,0x25,0x3C,//GD18667593	
	//0x0e,0x09,0x90,0x5c,0x1d,0x09,0xed,0xdf,0x90,0xda,0xf0,0xf9,0x95,0xed,0x30,0x62,//tpc test 00030040
	0x8F, 0x45, 0x32, 0xC8, 0x79, 0x5B, 0xFA, 0x3C}; // StoC (Change data from AARE)
...
//=============================================================================
uint8_t ELS61IOType = 0, ELS61inPointer = 0, ELS61outPointer = 0, ELS61_IO_Difference = 0,ELS61_MESSAGE_LOCK = 0, ELS_CSQ = 0, Get_CSQ_FLAG = 0;
...
//                Functions
//=============================================================================
static void WriteFlashData(uint32_t WriteAddress, unsigned char *data, uint32_t num)
{
...
static int32_t udf_AES_GCM_Decrypt(uint8_t * gcm_ek, uint8_t * gcm_aad, uint8_t * gcm_iv, 
  uint8_t * gcm_ct, uint8_t * gcm_tag, uint8_t * gcm_pt, uint16_t gcm_ctlen, uint16_t gcm_aadlen)
{
...
int32_t udf_AES_GCM_Encrypt(uint8_t * gcm_ek, uint8_t * gcm_aad, uint8_t * gcm_iv, 
  uint8_t * gcm_pt, uint8_t * gcm_ct, uint8_t * gcm_tag, uint16_t gcm_ptlen, uint16_t gcm_aadlen)
{
  	mbedtls_gcm_context ctx;
	mbedtls_gcm_init( &ctx );
	int32_t ret = 0;
...
static void udf_Rafael_data(uint16_t task_id, uint8_t type, char *meter_data, uint8_t Rafael_command, uint16_t meter_len)
{
...
static uint8_t Check_Meter_Res(char *recv_buff, uint16_t len)
{
	...
	else if(*(recv_buff+5)==0x13 && *(recv_buff+11)==0x61) //Meter AARE
	{
		return 61;
	}
	else if((uint8_t)*(recv_buff+9)==0xE7 && (uint8_t)*(recv_buff+11)==0xC4 && (uint8_t)*(recv_buff+13)== meter_fix_cnt)
	{
		return 1;
	}
	else if((uint8_t)*(recv_buff+9)==0xE7 && (uint8_t)*(recv_buff+11)==0xD4)	//Get Response
	{
		return 2;
	}
	else if((uint8_t)*(recv_buff+9)==0xE7 && (uint8_t)*(recv_buff+11)==0xD5)	//Set Response
	{
		return 21;
	}
	else if((uint8_t)*(recv_buff+9)==0xE7 && (uint8_t)*(recv_buff+11)==0xD7)	//Action Response
	{
		return 99;
	}
	else if((uint8_t)*(recv_buff+9)==0xE7 && (uint8_t)*(recv_buff+11)==0x2E)
	{
		printf("=====WTF======\n\r");
		KeyErrorflag = 1;//key錯誤
		ACK_function(1);
		return 33;
	}
...
static void udf_Send_AARQ(uint8_t type)
{
	...
	{0x7E, 0xA0, 0x74, 0x03, 0x23, 0x13, 0x3F, 0xC9, 0xE6, 0xE6, 
	 0x00, 0x60, 0x66, 
	 0xA1, 0x09, 0x06, 0x07, 0x60, 0x85, 0x74, 0x05, 0x08, 0x01, 0x03, 
	 0xA6, 0x0A, 0x04, 0x08, 0x4D, 0x41,  0x4E, 0x01, 0x23, 0x45, 0x67, 0x89, 
	 0x8A, 0x02, 0x07, 0x80, 
   ...
   0xC1, 0x5F, 0x17, 0xBE, 0x08, 0x50, 0xF3, 0x7E}; // 118B
	unsigned char no_en_aarq[]=
	{0x7E,0xA0,0x40,0x03,0x21,0x13,0x91,0xC4,0xE6,0xE6,0x00,0x60,0x32,
	 0xA1,0x09,0x06,0x07,0x60,0x85,0x74,0x05,0x08,0x01,0x01,
	 0xA6,0x0A,0x04,0x08,0x56,0x45,0x52,0x30,0x30,0x30,0x30,0x31,
	 0x8B,0x07,0x60,0x85,0x74,0x05,0x08,0x02,0x00,
	 0xBE,0x10,0x04,0x0E,0x01,0x00,0x00,0x00,0x06,0x5F,0x1F,0x04,0x00,0x00,0x10,0x14,0x03,0x00,
	 0x5D,0x67,0x7E
	};
  int i;
	char strbuf[128];
	...
	else//if(type==2/*get_id_done==1*/)
	{
		...
		udf_AES_GCM_Encrypt(tpc_guk, tpc_gcm_ak, tpc_c_iv, &cmdbuf[72], &cipher[0], &tag[0], 31, 17);
		}
		if(type==4)
		{
		for (i=0; i<4; i++)
		...
		udf_AES_GCM_Encrypt(tpc_guk, tpc_gcm_ak, tpc_c_iv_h, &cmdbuf[72], &cipher[0], &tag[0], 31, 17);
		}
...
unsigned char cmdbuf[]=
	{0x7E, 0xA0, 0x3F, 0x03, 0x23, 0x13, 0x9D, 0x1F, 0xE6, 0xE6, //0~9
	 ...
	 0x09, 0x11, 0x10, 0x00, 0x00, 0x00, 0x02, 0xF9, 0x20, 0x62, 0xB8, 0x99, 0xF5, 0xA4, 0x0A, 0x4E, 0xCF, 0xC2, 0x8A,//31~49
   0x00, 0xEA, 0xDB, 0x61, 0x8B, 0x34, 0x98, 0x1A, 0xBE, 0x16, 0x66, 0xF2, 0xE3, //tag
	 0x1E, 0x7E}; // 46B
  ...
  int i;
	...
	udf_AES_GCM_Encrypt(tpc_guk, tpc_gmac_ak, tpc_c_iv, &cmdbuf[0], &cipher[0], &tag[0], 0, 25);
	...
	udf_AES_GCM_Encrypt(tpc_guk, tpc_gcm_ak, tpc_c_iv, &cmdbuf[18], &cipher[0], &tag[0], 32, 17);
...
static void udf_Han_Client(void)
{
	unsigned char cmdbuf[]={
		0x7E, 0xA0, 0x3F, 0x03, 0x27, 0x13, 0x9D, 0x1F, 0xE6, 0xE6, //0~9
	 	...
	 	0x09, 0x11, 0x10, 0x00, 0x00, 0x00, 0x01, 0xF9, 0x20, 0x62, 0xB8, 0x99, 0xF5, 0xA4, 0x0A, 0x4E, 0xCF, 0xC2, 0x8A,//31~49
		0x00, 0xEA, 0xDB, 0x61, 0x8B, 0x34, 0x98, 0x1A, 0xBE, 0x16, 0x66, 0xF2, 0xE3, //tag
	 	0x1E, 0x7E}; // 46B
	...
	udf_AES_GCM_Encrypt(tpc_guk, tpc_gmac_ak, tpc_c_iv_h, &cmdbuf[0], &cipher[0], &tag[0], 0, 25);
	...
	udf_AES_GCM_Encrypt(tpc_guk, tpc_gcm_ak, tpc_c_iv_h, &cmdbuf[18], &cipher[0], &tag[0], 32, 17);
...
static void udf_GetData_Request(uint8_t * obis_data, uint16_t ic_index, uint8_t source)
{
	if(obis_data[0] == 0x01 && obis_data[1] == 0x07 && obis_data[2] == 0x01 && 
     obis_data[3] == 0x00 && obis_data[4] == 0x63 && obis_data[5] == 0x01 && 
		 ...
		 obis_data[9] == 0x00)  //The first bit was originally 0x00 and changed to 0x01 for identification.
	{
		uint16_t CRC_check;
		unsigned char cmdbuf[]=
		...
		{	
			...
			udf_AES_GCM_Encrypt(tpc_dk, tpc_gcm_ak, tpc_c_iv_h, &cmdbuf[18], &cipher[0], &tag[0], pt_len, 17);
		}
	...
	else
	{
		...
		{0x7E, 0xA0, 0x2C, 0x03, 0x23, 0x13, 0xF1, 0xF9, 0xE6, 0xE6, 
		 ...
		 0x66, 0xF2, 0x12, 0xE3, 0x1E, 0x7E}; // 46B
...
static void udf_Get_New_Number_of_pens(uint16_t ic_count, uint8_t *obis, uint8_t obis_number)//Latest data
{
	uint16_t CRC_check;
	unsigned char cmdbuf[]=
	{0x7E, 0xA0, 0x3F, 0x03, 0x23, 0x13, 0xF1, 0xF9, 0xE6, 0xE6, //[0-9]
	 ...
	 0x01, 0x02, //[30-31]
	 0x02, 0x04, //[32-33]
	 0x06, 0x00, 0x00, 0x00, 0x01, 0x06, 0x00, 0x00, 0x00, 0x05,
	 0x12, 0x00, 0x01, 
	 0x12, 0x00, 0x00,
	0xEA, 0xDB, 0x61, 0x8B, 0x34, 0x98, 0x1A, 0xBE, 0x16, 0x66, 0xF2, 0x12,
	 0xE3, 0x1E, 0x7E};	
...
static void udf_SMIP_Res(uint8_t *meter_data)
{
  ...
  {0x00,0x01,0x00,0x01,0x00,0x01,
	 0x00,0x0D,0xC4,0x01,0x43,0x00,0x0A,0x08,0x31,0x36,0x36,0x39,0x33,0x37,0x35,0x31};
	char strbuf[128];	
...
static void on_Demand_Task_Assembly(unsigned char *auto_data,uint16_t ct_len,unsigned char *pt_confirm)
{
	...
	sprintf(strbuf,"pt[1] = %d , pt[3] = %d\n\r",pt_confirm[1],pt_confirm[3]);
	printf(strbuf);
	if((pt_confirm[1] == 0x02 && pt_confirm[3] == 0x01) || (pt_confirm[1] == 0x02 && pt_confirm[3] == 0x00) || (pt_confirm[1] == 0x01 && pt_confirm[3] == 0x00) || pt_confirm[1] == 0x01)
	{
		...
		if(Task_count > 0)
		{
			sprintf(strbuf,"Task_count = %d FIRST = %02X  END = %02X \n" ,Task_count,(Task_count_all-Task_count)*9+1,(Task_count_all-Task_count)*9+9);
			printf(strbuf);
			
			for(i = (Task_count_all-Task_count)*9 ,j=1 ;i <(Task_count_all-Task_count)*9 + 9; i++,j++)
		...
		{	
			...
			printf("\n");
			udf_Rafael_data(TASK_ID,7,(char *)&On_Demand_Task_Buff[0],3,On_Demand_length);
	...
	else
	{
		...
		{
			for(i = (Task_count_all-Task_count)*9 + 1,j=1 ;i <(Task_count_all-Task_count)*9 + 10; i++,j++)
					obis_data[j] = On_Demand_Task_Command[i];
			udf_GetData_Request(obis_data, ic_count,1);
		}
		else
...
static void Power_On_Event_Log_Return(unsigned char *Notification_pt)//Send reply message
{
	int i=0,j=0,k=0;
	unsigned short crc16;
	unsigned char cipher[128], tag[16];
	...
	unsigned char obis_Power_On_Event_Code[] = {0x12, 0x00, 0x0f} , Power_On_data_Buff[15] = {0};
	unsigned char Power_On_Event_Buff[] = {0x7E, 0xA0, 0x3A, 0x23, 0x03, 0x13, 0xC2, 0x51, 0xE6, 0xE6, 0x00, 0xCA, 0x2C, 0x30, 0x00, 0x00, 0x00, 0x01,
																0xc2, 0x01, 0x0c, 0x07, 0xe2, 0x09, 0x0a, 0xff, 0x0e, 0x03, 0x04, 0xff, 0x80, 0x00, 0x00, 0x00, 0x01, 0x00,
																0x00, 0x60, 0x0b, 0x00, 0xff, 0x02, 0x12, 0x00, 0x07, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
																0x0a, 0x0b, 0x0c, 0x00, 0x00, 0x7e};
	printf("pt[i] = ");
		...
		for(i = 0 ;i <= 136 ; i++)
		{
			if(Notification_pt[i] == obis_Power_On_Event_Code[0])
			{
				if(Notification_pt[i+1] == obis_Power_On_Event_Code[1])
				{
					...
					{
						if(Notification_pt[i-1] == 0x00 &&  Notification_pt[i-2] == 0x00 && Notification_pt[i-3] == 0x80 && Notification_pt[i-4] == 0xff)
						{
							for(j = i-12, k = 0; j <= i+2 & k <= 14; j++,k++)
							{
								if(NEW_POWER_ON_FLAG == 0)
								{
									Power_On_data_Buff[k] = Notification_pt[j];
									sprintf(strbuf,"%02X ",Power_On_data_Buff[k]);
									printf(strbuf);
									FLAG_POWER_ON = 1;
								}
							}
	...
	if( FLAG_POWER_ON == 1)
	{
		...
		udf_AES_GCM_Encrypt(tpc_guk, tpc_gcm_ak, tpc_s_iv, &Power_On_Event_Buff[18], &cipher[0], &tag[0], 27, 17);
		...
		Power_On_Event_Buff[sizeof(Power_On_Event_Buff)-2] = (crc16 >>8) &0xFF;
		...
		printf("\n");
		udf_Rafael_data(1,8,(char *)&Power_On_Event_Buff[0],3,(Power_On_Event_Buff[1]-0xA0)*256+Power_On_Event_Buff[2] + 2);
...
static void udf_Set_Transmit(unsigned char obisTXT[],unsigned char plainTXT[], uint16_t plainTXT_Len, uint32_t frameNum, uint8_t lastNum)
{
  ...
  {0x7E, 0xA0, 0x2C, 0x03, 0x23, 0x13, 0x6C, 0x8D, 0xE6, 0xE6,
   ...
   0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, //50
	 ...
	 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11,
	 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11,
	 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x7E}; //779B
  ...
  cmdTXT[7] = (crc16 >> 8) & 0xFF;
	...
	udf_AES_GCM_Encrypt(tpc_dk, tpc_gcm_ak, tpc_c_iv, &cmdTXT[18 + shift], &cipher[0], &tag[0], plainLen + shiftTrans + shiftOBIS + 4, 17); //��Jcmdbuf,���ͧ������cipher����
...
//		{
//			begin.tm_year = (saveSETdata[10] * 256 + saveSETdata[11] - 1900); 
...
//		}*/	
	if(SETACTION == 0){//No need to continue uploading
		udf_Set_Transmit(TOUobisTXT,&saveSETdata[9],setdataLen-9,SETACTION,0);
		setdataLen = 0;			 
		memset( saveSETdata ,0,sizeof(saveSETdata));}
	else if(SETACTION == 1)
	{
		hitHANDguN=1;
		udf_Set_Transmit(TOUobisTXT,&saveSETdata[9],sendPoint,SETACTION,lastnum);
	}
	else 
	{
		udf_Set_Transmit(TOUobisTXT,&saveSETdata[9+sendPoint*SETACTION],setdataLen-sendPoint*SETACTION,SETACTION,lastnum);
...
{
	unsigned char cmdbuf[]={0x7E, 0xA0, 0x07, 0x03, 0x23, 0x53, 0xB3, 0xF4, 0x7E};
  	int i;
	char strbuf[128];
  	uint16_t CRC_check;
...
{
	unsigned char cmdbuf[]={0x7E, 0xA0, 0x07, 0x03, 0x23, 0x93, 0xBF, 0x32, 0x7E};
	int i;
	char strbuf[128];
  	uint16_t CRC_check;
	if(type == 1)
	 	...
	 	switch (Res_type)
		{
			...
			case 11://RES-Client AA & Change Key RES
				...
				if(meter_data[3]==0x23)
				{
					...
					{
						MAA_flag = 1;
						RegistrationprocessStart = 0;// (u32)now_time + RTC_CNT; What does this do?
						sst_flag = 1;
...
//									tpc_mkey[i] = new_tpc_mkey[i];
			...
			case 2:	//Read_RES
				...
				{	
					...
					else if(MAA_flag == 3 && flag_Power_Off == 0 && NOT_COMPLETELY_POWER_OFF == 0)
					{
						if(On_Demand_Reading_Type == 1)
						{
							on_Demand_Task_Assembly(&pt[0],ct_len,&pt[0]);
