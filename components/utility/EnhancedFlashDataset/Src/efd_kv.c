...
#if defined(EFD_USING_ENV) && !defined(EFD_ENV_USING_LEGACY_MODE)

#ifndef EFD_WRITE_GRAN
#error "Please configure flash write granularity (in efd_cfg.h)"
#endif

#if EFD_WRITE_GRAN != 1 && EFD_WRITE_GRAN != 8 && EFD_WRITE_GRAN != 32 && EFD_WRITE_GRAN != 64
...
/* the string ENV value buffer size for legacy efd_get_env() function */
#ifndef EFD_STR_ENV_VALUE_MAX_SIZE
#define EFD_STR_ENV_VALUE_MAX_SIZE 128
#endif

/* the sector remain threshold before full status */
#ifndef EFD_SEC_REMAIN_THRESHOLD
#define EFD_SEC_REMAIN_THRESHOLD (ENV_HDR_DATA_SIZE + EFD_ENV_NAME_MAX)
#endif
...
#ifndef EFD_ENV_CACHE_TABLE_SIZE
#define EFD_ENV_CACHE_TABLE_SIZE 256
#endif

/* the sector cache table size, it will improve ENV save speed when using cache */
#ifndef EFD_SECTOR_CACHE_TABLE_SIZE
#define EFD_SECTOR_CACHE_TABLE_SIZE 8
#endif

#if EFD_ENV_CACHE_TABLE_SIZE > 0xFFFF
#error "The ENV cache table size must less than 0xFFFF"
#endif

#if (EFD_ENV_CACHE_TABLE_SIZE > 0) && (EFD_SECTOR_CACHE_TABLE_SIZE > 0)
#define EFD_ENV_USING_CACHE
#endif
 ...
 * would return 16.
 */
#define EFD_ALIGN(size, align) (((size) + (align) -1) & ~((align) -1))
/* align by write granularity */
#define EFD_WG_ALIGN(size) (EFD_ALIGN(size, (EFD_WRITE_GRAN + 7) / 8))
/**
 * Return the down number of aligned at specified width. RT_ALIGN_DOWN(13, 4)
 * would return 12.
 */
#define EFD_ALIGN_DOWN(size, align) ((size) & ~((align) -1))
/* align down by write granularity */
#define EFD_WG_ALIGN_DOWN(size) (EFD_ALIGN_DOWN(size, (EFD_WRITE_GRAN + 7) / 8))

#if (EFD_WRITE_GRAN == 1)
#define STATUS_TABLE_SIZE(status_number) ((status_number * EFD_WRITE_GRAN + 7) / 8)
#else
#define STATUS_TABLE_SIZE(status_number) (((status_number - 1) * EFD_WRITE_GRAN + 7) / 8)
#endif
...
#define SECTOR_SIZE EFD_ERASE_MIN_SIZE
#define SECTOR_NUM (ENV_AREA_SIZE / (EFD_ERASE_MIN_SIZE))

#if (SECTOR_NUM < 2)
#error "The sector number must lager then or equal to 2"
#endif

#if (EFD_GC_EMPTY_SEC_THRESHOLD == 0 || EFD_GC_EMPTY_SEC_THRESHOLD >= SECTOR_NUM)
#error "There is at least one empty sector for GC."
#endif

#define SECTOR_HDR_DATA_SIZE (EFD_WG_ALIGN(sizeof(struct sector_hdr_data)))
#define SECTOR_DIRTY_OFFSET ((unsigned long) (&((struct sector_hdr_data *) 0)->status_table.dirty))
#define ENV_HDR_DATA_SIZE (EFD_WG_ALIGN(sizeof(struct env_hdr_data)))
...
#ifdef EFD_ENV_USING_CACHE
/* ENV cache table */
struct env_cache_node env_cache_table[EFD_ENV_CACHE_TABLE_SIZE] = { 0 };
/* sector cache table, it caching the sector //info which status is current using */
struct sector_cache_node sector_cache_table[EFD_SECTOR_CACHE_TABLE_SIZE] = { 0 };
#endif /* EFD_ENV_USING_CACHE */
...
#else
        byte_index               = (status_index - 1) * (EFD_WRITE_GRAN / 8);
        status_table[byte_index] = 0x00;
#endif /* EFD_WRITE_GRAN == 1 */
    }
...
#else  /*  (EFD_WRITE_GRAN == 8) ||  (EFD_WRITE_GRAN == 32) ||  (EFD_WRITE_GRAN == 64) */
        if (status_table[status_num * EFD_WRITE_GRAN / 8] == 0x00)
        {
            break;
        }
#endif /* EFD_WRITE_GRAN == 1 */
...
static EfErrCode write_status(uint32_t addr, uint8_t status_table[], size_t status_num, size_t status_index)
{
    ...
    EFD_ASSERT(status_index < status_num);
    EFD_ASSERT(status_table);
...
#if (EFD_WRITE_GRAN == 1)
    result = efd_port_write(addr + byte_index, (uint32_t *) &status_table[byte_index], 1);
#else  /*  (EFD_WRITE_GRAN == 8) ||  (EFD_WRITE_GRAN == 32) ||  (EFD_WRITE_GRAN == 64) */
    /* write the status by write granularity
     * some flash (like stm32 onchip) NOT supported repeated write before erase */
    result = efd_port_write(addr + byte_index, (uint32_t *) &status_table[byte_index], EFD_WRITE_GRAN / 8);
#endif /* EFD_WRITE_GRAN == 1 */
...
static size_t read_status(uint32_t addr, uint8_t status_table[], size_t total_num)
{
    ...
    efd_port_read(addr, (uint32_t *) status_table, STATUS_TABLE_SIZE(total_num));
...
{
    size_t i, empty_index = EFD_SECTOR_CACHE_TABLE_SIZE;

    for (i = 0; i < EFD_SECTOR_CACHE_TABLE_SIZE; i++)
    {
        if ((empty_addr > sec_addr) && (empty_addr < sec_addr + SECTOR_SIZE))
        {
            ...
            else if ((sector_cache_table[i].addr == FAILED_ADDR) && (empty_index == EFD_SECTOR_CACHE_TABLE_SIZE))
            {
                empty_index = i;
            }
        }
    ...
    if (empty_index < EFD_SECTOR_CACHE_TABLE_SIZE)
    {
...
static void update_env_cache(const char *name, size_t name_len, uint32_t addr)
{
    ...
    for (i = 0; i < EFD_ENV_CACHE_TABLE_SIZE; i++)
    {
        if (addr != FAILED_ADDR)
        {
            ...
            else if ((env_cache_table[i].addr == FAILED_ADDR) && (empty_index == EFD_ENV_CACHE_TABLE_SIZE))
            {
                empty_index = i;
            }
...
static bool get_env_from_cache(const char *name, size_t name_len, uint32_t *addr)
{
    ...
    for (i = 0; i < EFD_ENV_CACHE_TABLE_SIZE; i++)
    {
        ...
        {
            char saved_name[EFD_ENV_NAME_MAX];
            /* read the ENV name in flash */
            efd_port_read(env_cache_table[i].addr + ENV_HDR_DATA_SIZE, (uint32_t *) saved_name, EFD_ENV_NAME_MAX);
            if (!memcmp(name, saved_name, name_len))
            {
                *addr = env_cache_table[i].addr;
                if (env_cache_table[i].active >= 0xFFFF - EFD_ENV_CACHE_TABLE_SIZE)
                {
                    env_cache_table[i].active = 0xFFFF;
                }
                else
                {
                    env_cache_table[i].active += EFD_ENV_CACHE_TABLE_SIZE;
                }
                return true;
            }
        }
...
{
    uint8_t buf[EFD_READ_BUF_SIZE], last_data = 0x00;
    ...
    for (; start < end; start += sizeof(buf))
    {
        ...
        efd_port_read(start, (uint32_t *) buf, read_size);
...
static uint32_t find_next_env_addr(uint32_t start, uint32_t end)
{
    ...
    if (get_sector_from_cache(EFD_ALIGN_DOWN(start, SECTOR_SIZE), &empty_env) && start == empty_env)
    {
        return FAILED_ADDR;
    }
#endif /* EFD_ENV_USING_CACHE */
    ...
    {
        efd_port_read(start, (uint32_t *) buf, sizeof(buf));
        for (i = 0; i < sizeof(buf) - sizeof(uint32_t) && start + i < end; i++)
        {
#ifndef EFD_BIG_ENDIAN /* Little Endian Order */
    ...
    else
    {
        if (pre_env->addr.start <= sector->addr + SECTOR_SIZE)
        {
            ...
            else
            {
                ...
                addr = pre_env->addr.start + EFD_WG_ALIGN(1);
            }
...
static EfErrCode read_env(env_node_obj_t env)
{
    ...
    uint8_t buf[EFD_READ_BUF_SIZE];
    uint32_t calc_crc32 = 0, crc_data_len, env_name_addr;
    EfErrCode result    = EFD_NO_ERR;
    size_t len, size;
    /* read ENV header raw data */
    efd_port_read(env->addr.start, (uint32_t *) &env_hdr, sizeof(struct env_hdr_data));
    ...
    if (env->len == ~0UL || env->len > ENV_AREA_SIZE || env->len < ENV_NAME_LEN_OFFSET)
    {
        ...
        if (env->status != ENV_ERR_HDR)
        {
            ...
            // log_log_info("Error: The ENV @0x%08X length has an error.\n", env->addr.start);
    ...
    for (len = 0, size = 0; len < crc_data_len; len += size)
    {
        ...
        efd_port_read(env->addr.start + ENV_NAME_LEN_OFFSET + len, (uint32_t *) buf, EFD_WG_ALIGN(size));
        calc_crc32 = efd_calc_crc32(calc_crc32, buf, size);
    }
    /* check CRC32 */
    ...
    else
    {
        ...
        efd_port_read(env_name_addr, (uint32_t *) env->name, EFD_WG_ALIGN(env_hdr.name_len));
        /* the value is behind aligned name */
        env->addr.value = env_name_addr + EFD_WG_ALIGN(env_hdr.name_len);
...
static EfErrCode read_sector_meta_data(uint32_t addr, sector_meta_data_t sector, bool traversal)
{
    ...
    EFD_ASSERT(addr % SECTOR_SIZE == 0);
    EFD_ASSERT(sector);

    /* read sector header raw data */
    efd_port_read(addr, (uint32_t *) &sec_hdr, sizeof(struct sector_hdr_data));
        ...
        else if (sector->status.store == SECTOR_STORE_USING)
        {
            ...
            while ((env_meta.addr.start = get_next_env_addr(sector, &env_meta)) != FAILED_ADDR)
            {
                ...
                if (!env_meta.crc_is_ok)
                {
                    ...
                    {
                        // log_info("Error: The ENV (@0x%08X) CRC32 check failed!\n", env_meta.addr.start);
                        sector->remain = 0;
                        result         = EFD_READ_ERR;
                        break;
                    }
                         ...
                         bool (*callback)(env_node_obj_t env, void *arg1, void *arg2))
{
    ...
    {
        if (read_sector_meta_data(sec_addr, &sector, false) != EFD_NO_ERR)
        {
            continue;
        }
        if (callback == NULL)
...
static bool find_env_cb(env_node_obj_t env, void *arg1, void *arg2)
{
    ...
    /* check ENV */
    if (env->crc_is_ok && env->status == ENV_WRITE && !memcmp(env->name, key, key_len))
    {
        *find_ok = true;
        return true;
    }
...
static size_t get_env(const char *key, void *value_buf, size_t buf_len, size_t *value_len)
{
    ...
    if (find_env(key, &env))
    {
        ...
        if (value_buf)
        {
            efd_port_read(env.addr.value, (uint32_t *) value_buf, read_len);
        }
    }
    else if (value_len)
    {
...
bool efd_get_env_obj(const char *key, env_node_obj_t env)
{
    ...
    if (!init_ok)
    {
        // log_info("ENV isn't initialize OK.\n");
        return 0;
    }
...
char *efd_get_env(const char *key)
{
    ...
    if ((get_size = efd_get_env_blob(key, value, EFD_STR_ENV_VALUE_MAX_SIZE, NULL)) > 0)
    {
        /* the return value must be string */
        if (efd_is_str((uint8_t *) value, get_size))
        {
            value[get_size] = '\0';
            return value;
        }
        ...
        {
            // log_info("Warning: The ENV value isn't string. Could not be returned\n");
            return NULL;
        }
    }
...
static EfErrCode write_env_hdr(uint32_t addr, env_hdr_data_t env_hdr)
{
    ...
    result = efd_port_write(addr + ENV_MAGIC_OFFSET, &env_hdr->magic, sizeof(struct env_hdr_data) - ENV_MAGIC_OFFSET);
...
static EfErrCode format_sector(uint32_t addr, uint32_t combined_value)
{
    ...
    result = efd_port_erase(addr, SECTOR_SIZE);
    if (result == EFD_NO_ERR)
    {
        ...
        /* save the header */
        result = efd_port_write(addr, (uint32_t *) &sec_hdr, sizeof(struct sector_hdr_data));
...
static EfErrCode update_sec_status(sector_meta_data_t sector, size_t new_env_len, bool *is_full)
{
    ...
    {
        /* check remain size */
        if (sector->remain < EFD_SEC_REMAIN_THRESHOLD || sector->remain - new_env_len < EFD_SEC_REMAIN_THRESHOLD)
        {
...
static uint32_t alloc_env(sector_meta_data_t sector, size_t env_size)
{
    ...
    {
        if (empty_sector > EFD_GC_EMPTY_SEC_THRESHOLD || gc_request)
        {
        ...
        else
        {
            ...
            // log_info("Trigger a GC check after alloc ENV failed.\n");
            gc_request = true;
        }
    }
...
static EfErrCode del_env(const char *key, env_node_obj_t old_env, bool complete_del)
{
    ...
    if (!old_env)
    {
        ...
        {
            // log_info("Not found '%s' in ENV.\n", key);
            return EFD_ENV_NAME_ERR;
        }
    }
    ...
    else
    {
        ...
        if (!last_is_complete_del && result == EFD_NO_ERR)
        {
#ifdef EFD_ENV_USING_CACHE
            /* delete the ENV in flash and cache */
            if (key != NULL)
            {
                /* when using del_env(key, NULL, true) or del_env(key, env, true) in efd_del_env() and set_env() */
    ...
    dirty_status_addr = EFD_ALIGN_DOWN(old_env->addr.start, SECTOR_SIZE) + SECTOR_DIRTY_OFFSET;
    /* read and change the sector dirty status */
    if (result == EFD_NO_ERR && read_status(dirty_status_addr, status_table, SECTOR_DIRTY_STATUS_NUM) == SECTOR_DIRTY_FALSE)
    {
...
static EfErrCode move_env(env_node_obj_t env)
{
    ...
    if ((env_addr = alloc_env(&sector, env->len)) != FAILED_ADDR)
    {
        if (in_recovery_check)
        {
            ...
            char name[EFD_ENV_NAME_MAX + 1] = { 0 };
            memcpy(name, env->name, env->name_len + 1);
    ...
    /* start move the ENV */
    {
        ...
        for (len = 0, size = 0; len < env_len; len += size)
        {
            ...
            efd_port_read(env->addr.start + ENV_MAGIC_OFFSET + len, (uint32_t *) buf, EFD_WG_ALIGN(size));

            result = efd_port_write(env_addr + ENV_MAGIC_OFFSET + len, (uint32_t *) buf, size);
        }
        write_status(env_addr, status_table, ENV_STATUS_NUM, ENV_WRITE);

#ifdef EFD_ENV_USING_CACHE
        update_sector_cache(EFD_ALIGN_DOWN(env_addr, SECTOR_SIZE),
                            env_addr + ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env->name_len) + EFD_WG_ALIGN(env->value_len));
        update_env_cache(env->name, env->name_len, env_addr);
#endif /* EFD_ENV_USING_CACHE */
    }

    // log_info("Moved the ENV (%.*s) from 0x%08X to 0x%08X.\n", env->name_len, env->name, env->addr.start, env_addr);
...
__retry:
    ...
    {
        // log_info("Warning: Alloc an ENV (size %d) failed when new ENV. Now will GC then retry.\n", env_size);
        gc_collect();
        already_gc = true;
        goto __retry;
    }
...
{
    size_t env_len = ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(key_len) + EFD_WG_ALIGN(buf_len);
...
static bool do_gc(sector_meta_data_t sector, void *arg1, void *arg2)
{
    ...
    if (sector->check_ok && (sector->status.dirty == SECTOR_DIRTY_TRUE || sector->status.dirty == SECTOR_DIRTY_GC))
    {
        ...
        while ((env.addr.start = get_next_env_addr(sector, &env)) != FAILED_ADDR)
        {
            ...
            {
                /* move the ENV to new space */
                if (move_env(&env) != EFD_NO_ERR)
                {
                    // log_info("Error: Moved the ENV (%.*s) for GC failed.\n", env.name_len, env.name);
                }
            }
        }
        format_sector(sector->addr, SECTOR_NOT_COMBINED);
        // log_info("Collect a sector @0x%08X\n", sector->addr);
    }
...
static void gc_collect(void)
{
    ...
    /* do GC collect */
    // log_info("The remain empty sector is %d, GC threshold is %d.\n", empty_sec, EFD_GC_EMPTY_SEC_THRESHOLD);
...
static EfErrCode align_write(uint32_t addr, const uint32_t *buf, size_t size)
{
...
#if (EFD_WRITE_GRAN / 8 > 0)
    uint8_t align_data[EFD_WRITE_GRAN / 8];
    ...
    if (result == EFD_NO_ERR && align_remain)
    {
        memcpy(align_data, (uint8_t *) buf + EFD_WG_ALIGN_DOWN(size), align_remain);
        result = efd_port_write(addr + EFD_WG_ALIGN_DOWN(size), (uint32_t *) align_data, align_data_size);
    }
...
static EfErrCode create_env_blob(sector_meta_data_t sector, const char *key, const void *value, size_t len)
{
    ...
    if (strlen(key) > EFD_ENV_NAME_MAX)
    {
        // log_info("Error: The ENV name length is more than %d\n", EFD_ENV_NAME_MAX);
        return EFD_ENV_NAME_ERR;
    }
    ...
    env_hdr.len       = ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env_hdr.name_len) + EFD_WG_ALIGN(env_hdr.value_len);

    if (env_hdr.len > SECTOR_SIZE - SECTOR_HDR_DATA_SIZE)
    {
        // log_info("Error: The ENV size is too big\n");
        return EFD_ENV_FULL;
    }
    ...
    if (env_addr != FAILED_ADDR || (env_addr = new_env(sector, env_hdr.len)) != FAILED_ADDR)
    {
        ...
        if (result == EFD_NO_ERR)
        {
            ...
            env_hdr.crc32 = efd_calc_crc32(0, &env_hdr.name_len, ENV_HDR_DATA_SIZE - ENV_NAME_LEN_OFFSET);
            env_hdr.crc32 = efd_calc_crc32(env_hdr.crc32, key, env_hdr.name_len);
            align_remain  = EFD_WG_ALIGN(env_hdr.name_len) - env_hdr.name_len;
            while (align_remain--)
            {
                env_hdr.crc32 = efd_calc_crc32(env_hdr.crc32, &ff, 1);
            }
            env_hdr.crc32 = efd_calc_crc32(env_hdr.crc32, value, env_hdr.value_len);
            align_remain  = EFD_WG_ALIGN(env_hdr.value_len) - env_hdr.value_len;
            while (align_remain--)
            {
                env_hdr.crc32 = efd_calc_crc32(env_hdr.crc32, &ff, 1);
            }
        ...
        if (result == EFD_NO_ERR)
        {
            ...
            {
                update_sector_cache(
                    sector->addr, env_addr + ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env_hdr.name_len) + EFD_WG_ALIGN(env_hdr.value_len));
            }
        ...
        if (result == EFD_NO_ERR)
        {
            result = align_write(env_addr + ENV_HDR_DATA_SIZE + EFD_WG_ALIGN(env_hdr.name_len), value, env_hdr.value_len);
        }
        ...
        if (result == EFD_NO_ERR && is_full)
        {
            // log_info("Trigger a GC check after created ENV.\n");
            gc_request = true;
        }
    }
    else
...
EfErrCode efd_del_env(const char *key)
{
    ...
    if (!init_ok)
    {
        // log_info("Error: ENV isn't initialize OK.\n");
        return EFD_ENV_INIT_FAILED;
    }
...
/**
 * The same to efd_del_env on this mode
...
static EfErrCode set_env(const char *key, const void *value_buf, size_t buf_len)
{
    ...
    else
    {
        ...
        /* delete the old ENV */
        if (env_is_found && result == EFD_NO_ERR)
        {
...
/**
 * The same to efd_set_env on this mode.
 * It's compatibility with older versions (less then V4.0).
 *
 * @note this function is DEPRECATED
 *
...
EfErrCode efd_env_set_default(void)
{
    ...
    EFD_ASSERT(default_env_set);
    EFD_ASSERT(default_env_set_size);
...
static bool print_env_cb(env_node_obj_t env, void *arg1, void *arg2)
{
    ...
    if (env->crc_is_ok)
    {
        ...
        *using_size += env->len;
        ...
        if (env->status == ENV_WRITE)
        {
            log_info("%.*s=", env->name_len, env->name);

            if (env->value_len < EFD_STR_ENV_VALUE_MAX_SIZE)
            {
                uint8_t buf[32];
                size_t len, size;
__reload:
                ...
                for (len = 0, size = 0; len < env->value_len; len += size)
                {
                    ...
                    efd_port_read(env->addr.value + len, (uint32_t *) buf, EFD_WG_ALIGN(size));
                    if (print_value)
                    {
                        log_info("%.*s", size, buf);
                    }
                    else if (!efd_is_str(buf, size))
                    {
                        value_is_str = false;
                        break;
                    }
            ...
            {
                log_info("blob @0x%08X %dbytes", env->addr.value, env->value_len);
            }
            log_info("\n");
        }
    }
...
#ifdef EFD_ENV_AUTO_UPDATE
/*
 * Auto update ENV to latest default when current EFD_ENV_VER_NUM is changed.
 */
static void env_auto_update(void)
{
    size_t saved_ver_num, setting_ver_num = EFD_ENV_VER_NUM;
    ...
    if (get_env(VER_NUM_ENV_NAME, &saved_ver_num, sizeof(size_t), NULL) > 0)
    {
        ...
        if (saved_ver_num != setting_ver_num)
        {
            ...
            size_t i, value_len;
            struct sector_meta_data sector;
            // log_info("Update the ENV from version %d to %d.\n", saved_ver_num, setting_ver_num);
            for (i = 0; i < default_env_set_size; i++)
            {
...
static bool check_and_recovery_env_cb(env_node_obj_t env, void *arg1, void *arg2)
{
    ...
    {
        // log_info("Found an ENV (%.*s) which has changed value failed. Now will recovery it.\n", env->name_len, env->name);
        /* recovery the old ENV */
        if (move_env(env) == EFD_NO_ERR)
        {
            // log_info("Recovery the ENV successful.\n");
        }
        else
        {
            // log_info("Warning: Moved an ENV (size %d) failed when recovery. Now will GC then retry.\n", env->len);
            return true;
        }
    }
...
EfErrCode efd_load_env(void)
{
    ...
    {
        // log_info("Warning: All sector header check failed. Set it to default.\n");
        efd_env_set_default();
    }
...
EfErrCode efd_env_init(efd_env const *default_env, size_t default_env_size)
{
    ...
    EFD_ASSERT(default_env);
    EFD_ASSERT(ENV_AREA_SIZE);
    /* must be aligned with erase_min_size */
    EFD_ASSERT(ENV_AREA_SIZE % EFD_ERASE_MIN_SIZE == 0);
    /* sector number must be greater than or equal to 2 */
    EFD_ASSERT(SECTOR_NUM >= 2);
    /* must be aligned with write granularity */
    EFD_ASSERT((EFD_STR_ENV_VALUE_MAX_SIZE * 8) % EFD_WRITE_GRAN == 0);
    ...
    env_start_addr       = EFD_START_ADDR;
    default_env_set      = default_env;
    default_env_set_size = default_env_size;

    // log_log_info("ENV start address is 0x%08X, size is %d bytes.\n", EFD_START_ADDR, ENV_AREA_SIZE);
...
#endif /* defined(EFD_USING_ENV) && !defined(EFD_ENV_USING_LEGACY_MODE) */
