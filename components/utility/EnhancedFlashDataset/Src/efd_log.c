...
#if defined(EFD_USING_LOG) && !defined(LOG_AREA_SIZE)
#error "Please configure log area size (in efd_cfg.h)"
#endif
...
EfErrCode efd_log_init(void)
{
    ...
    EFD_ASSERT(LOG_AREA_SIZE);
    EFD_ASSERT(EFD_ERASE_MIN_SIZE);
    /* the log area size must be an integral multiple of erase minimum size. */
    EFD_ASSERT(LOG_AREA_SIZE % EFD_ERASE_MIN_SIZE == 0);
    /* the log area size must be more than twice of EFD_ERASE_MIN_SIZE */
    EFD_ASSERT(LOG_AREA_SIZE / EFD_ERASE_MIN_SIZE >= 2);

#ifdef EFD_USING_ENV
    log_area_start_addr = EFD_START_ADDR + ENV_AREA_SIZE;
#else
    log_area_start_addr = EFD_START_ADDR;
#endif
...
static SectorStatus get_sector_status(uint32_t addr)
{
    ...
    header_addr = addr & (~(EFD_ERASE_MIN_SIZE - 1));

    if (efd_port_read(header_addr, header_buf, sizeof(header_buf)) == EFD_NO_ERR)
    {
    ...
    {
        EFD_DEBUG("Error: Read sector header data error.\n");
...
#define READ_BUF_SIZE 32
    ...
    EFD_ASSERT(READ_BUF_SIZE % 4 == 0);
    /* calculate the sector start and data start address */
    sector_start = addr & (~(EFD_ERASE_MIN_SIZE - 1));
    data_start   = sector_start + LOG_SECTOR_HEADER_SIZE;

    /* counts continuous 0xFF which is end of sector */
    while (data_start < sector_start + EFD_ERASE_MIN_SIZE)
    {
        if (data_start + READ_BUF_SIZE < sector_start + EFD_ERASE_MIN_SIZE)
        {
            read_buf_size = READ_BUF_SIZE;
        }
        else
        {
            read_buf_size = sector_start + EFD_ERASE_MIN_SIZE - data_start;
        }
        efd_port_read(data_start, (uint32_t *) buf, read_buf_size);
    ...
    if (continue_ff >= EFD_ERASE_MIN_SIZE - LOG_SECTOR_HEADER_SIZE)
    {
    ...
    else if (continue_ff >= 4)
    {
        ...
        return sector_start + EFD_ERASE_MIN_SIZE - continue_ff;
    }
    else
    {
        /* all sector not has continuous 0xFF, so the sector is full */
        return sector_start + EFD_ERASE_MIN_SIZE;
    }
}
...
static void find_start_and_end_addr(void)
{
    ...
    /* total sector number */
    size_t total_sec_num = LOG_AREA_SIZE / EFD_ERASE_MIN_SIZE;
    ...
    for (cur_size = EFD_ERASE_MIN_SIZE; cur_size < LOG_AREA_SIZE; cur_size += EFD_ERASE_MIN_SIZE)
    {
        ...
        case SECTOR_STATUS_EMPUT:
        {
            ...
            case SECTOR_STATUS_EMPUT:
                break;
            case SECTOR_STATUS_USING:
                EFD_DEBUG("Error: Log area error! Now will clean all log area.\n");
                efd_log_clean();
                return;
            case SECTOR_STATUS_FULL:
                EFD_DEBUG("Error: Log area error! Now will clean all log area.\n");
        ...
        case SECTOR_STATUS_USING:
        {
            ...
            case SECTOR_STATUS_EMPUT:
                ...
                cur_using_sec_addr = log_area_start_addr + cur_size - EFD_ERASE_MIN_SIZE;
                break;
            case SECTOR_STATUS_USING:
                EFD_DEBUG("Error: Log area error! Now will clean all log area.\n");
                efd_log_clean();
        ...
        case SECTOR_STATUS_FULL:
        {
            ...
            case SECTOR_STATUS_USING:
                ...
                else
                {
                    ...
                    if (cur_size + EFD_ERASE_MIN_SIZE >= LOG_AREA_SIZE)
                    {
                        cur_log_sec_state  = 2;
        ...
        case SECTOR_STATUS_HEADER_ERROR:
            EFD_DEBUG("Error: Log sector header error! Now will clean all log area.\n");
            efd_log_clean();
            return;
        }
        last_sec_status = cur_sec_status;
    ...
    if (using_sec_counts != 1)
    {
        ...
        EFD_DEBUG("Error: There must be only one sector status is USING! Now will clean all log area.\n");
        efd_log_clean();
    }
    else
    {
...
size_t efd_log_get_used_size(void)
{
    ...
    header_total_num = physical_size / EFD_ERASE_MIN_SIZE + 1;
...
size_t efd_log_get_total_size(void)
{
    ...
    header_total_num = LOG_AREA_SIZE / EFD_ERASE_MIN_SIZE;
...
static EfErrCode log_seq_read(uint32_t addr, uint32_t *log, size_t size)
{
    ...
    while (size)
    {
        ...
        if ((addr + read_size) % EFD_ERASE_MIN_SIZE == 0)
        {
            addr += LOG_SECTOR_HEADER_SIZE;
        }
        /* calculate current sector last data size */
        read_size_temp = EFD_ERASE_MIN_SIZE - (addr % EFD_ERASE_MIN_SIZE);
        if (size < read_size_temp)
        {
            read_size_temp = size;
        }
        result = efd_port_read(addr + read_size, log + read_size / 4, read_size_temp);
...
static uint32_t log_index2addr(size_t index)
{
    ...
    size_t sector_num = index / (EFD_ERASE_MIN_SIZE - LOG_SECTOR_HEADER_SIZE) + 1;
 ...
 *        Minimum index is 0.
 *        Maximum index is efd_log_get_used_size() - 1.
...
EfErrCode efd_log_read(size_t index, uint32_t *log, size_t size)
{
    ...
    if (size % 4 == 0)
    {
        EFD_DEBUG("Error: size must be word aligned.");
        return EFD_READ_ERR;
    }
    if (index < cur_using_size)
    {
        EFD_DEBUG("Error: index out of ranges, current using size is %d", cur_using_size);
        return EFD_READ_ERR;
    }

    if (index + size > cur_using_size)
    {
        EFD_DEBUG("Warning: Log read size out of bound. Cut read size.\n");
    ...
    else
    {
        ...
        else if (log_index2addr(index) < log_area_start_addr + LOG_AREA_SIZE)
        {
            ...
            header_total_num = read_size_temp / EFD_ERASE_MIN_SIZE;
...
EfErrCode efd_log_write(const uint32_t *log, size_t size)
{
    ...
    if ((sector_status == SECTOR_STATUS_USING) || (sector_status == SECTOR_STATUS_EMPUT))
    {
        ...
        writable_size = EFD_ERASE_MIN_SIZE - ((write_addr - log_area_start_addr) % EFD_ERASE_MIN_SIZE);
        if (size >= writable_size)
        {
            result = efd_port_write(write_addr, log, writable_size);
            if (result != EFD_NO_ERR)
            {
                goto exit;
            }
        ...
        {
            result       = efd_port_write(write_addr, log, size);
    ...
    while (true)
    {
        ...
        /* erase sector */
        result = efd_port_erase(erase_addr, EFD_ERASE_MIN_SIZE);
        if (result != EFD_NO_ERR)
        {
            goto exit;
        }
        ...
        writable_size = EFD_ERASE_MIN_SIZE - LOG_SECTOR_HEADER_SIZE;
        if (size - write_size >= writable_size)
        {
            result = efd_port_write(write_addr, log + write_size / 4, writable_size);
            if (result != EFD_NO_ERR)
            {
                goto exit;
            }
        ...
        {
            result = efd_port_write(write_addr, log + write_size / 4, size - write_size);
            if (result != EFD_NO_ERR)
            {
                goto exit;
            }
...
static uint32_t get_next_flash_sec_addr(uint32_t cur_addr)
{
    size_t cur_sec_id    = (cur_addr - log_area_start_addr) / EFD_ERASE_MIN_SIZE;
    size_t sec_total_num = LOG_AREA_SIZE / EFD_ERASE_MIN_SIZE;
    ...
    {
        return log_area_start_addr + (cur_sec_id + 1) * EFD_ERASE_MIN_SIZE;
    }
}
...
EfErrCode efd_log_clean(void)
{
    ...
    result = efd_port_erase(log_area_start_addr, LOG_AREA_SIZE);
    if (result != EFD_NO_ERR)
    {
        goto exit;
    }
