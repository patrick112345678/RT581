    ...
    int (*cb_write)(uint8_t *, uint32_t, void *))
{
    ...
    do
    {
        ...
        {
            char    str_buf[12] = {0};
            snprintf(str_buf, 12, "\x1b[%dC", (int)(wr_pos - cursor_pos));
    ...
    int (*cb_write)(uint8_t *, uint32_t, void *))
{
    ...
    case SH_ESC_CODE_HOME:
        ...
        {
            char    str_buf[12] = {0};
            snprintf(str_buf, 12, "\x1b[%dD", (int)(*pCursor_pos));
            cb_write((uint8_t *)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
            *pCursor_pos = 0;
        }
        break;
    case SH_ESC_CODE_END:
        ...
        {
            char    str_buf[12] = {0};
            snprintf(str_buf, 12, "\x1b[%dC", (int)((*pWr_pos) - (*pCursor_pos)));
            cb_write((uint8_t *)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
            *pCursor_pos = *pWr_pos;
        }
        break;
    ...
    do
    {
...

        {
            ...
            if ( esc_code == SH_ESC_CODE_TOTAL )
            {
                ...
                {
                    char        str_buf[12] = {0};
                    snprintf(str_buf, 12, "\x1b[%dD", (int)(g_sh_dev.wr_pos - g_sh_dev.cursor_pos - 1));
                    cb_write((uint8_t *)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
                }
