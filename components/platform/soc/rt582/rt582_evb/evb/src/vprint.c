...
extern void debug_print(uint8_t *data, uint32_t len);
...
#define OUT(q, x) { putchar(x); q++; }
#define EMIT(x) { if (o < n) { OUT(q, x); } o++; }
           ...
           int base, int width, int prec)
{
    ...
    if (qq != qqbuf + sizeof(qqbuf))
    {
        debug_print((uint8_t *)qq, qqbuf + sizeof(qqbuf) - qq);
    }
...
extern char *cvt(double arg, int ndigits, int *decpt, int *sign, char *buf, int eflag);
...
extern void ee_bufcpy(char *pd, char *ps, int count);
extern void parse_float(double value, char *buffer, char fmt, int precision);
extern void decimal_point(char *buffer);
extern void cropzeros(char *buffer);
...
int vsnprintf_ex(char *buffer, size_t n, const char *format, va_list ap)
{
    const char *p = format;
    char ch;
    char *q = buffer;
