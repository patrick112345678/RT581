...
#include "cpc_drv.h"
#include "cpc_hdlc.h"
#include "cpc_system_secondary.h"
...
static TimerHandle_t sResetTimer;
static uint32_t enter_irq_end_ms;
static cpc_timer_handle_t enter_irq_timer;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static cpc_endpoint_handle_t system_ep = {.ep = NULL};
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *      starts an NCP mode.
                                                                               ******************************************************************************/
static cpc_system_reboot_mode_t prop_bootloader_reboot_mode = REBOOT_APPLICATION;

/*******************************************************************************
 *****************************   PROTOTYPES   **********************************
 ******************************************************************************/

static void on_write_completed(cpc_user_endpoint_id_t endpoint_id,
                               void *buffer,
                               void *arg,
                               status_t status);

static status_t open_endpoint(void);
static void on_system_ep_error(cpc_user_endpoint_id_t endpoint_id);
...
static void enter_irq_timer_callback(cpc_timer_handle_t *handle,
                                     void *data);

__weak void system_on_information_received(uint8_t endpoint_id, void *arg);
__weak const char *cpc_secondary_app_version(void);
...
static status_t open_endpoint(void)
{
    ...
    status = cpc_open_service_endpoint(&system_ep, CPC_ENDPOINT_SYSTEM, CPC_OPEN_ENDPOINT_FLAG_UFRAME_ENABLE, 10);
    if (status == CPC_STATUS_ALREADY_EXISTS)
    {
        return CPC_STATUS_ALREADY_INITIALIZED;
    }
    ...
    status = cpc_set_endpoint_option(&system_ep, CPC_ENDPOINT_ON_POLL, (void *)on_poll);
    if (status != STATUS_OK)
    {
        cpc_close_endpoint(&system_ep);
        return status;
    }

    if (system_on_information_received)
    {
        status = cpc_set_endpoint_option(&system_ep, CPC_ENDPOINT_ON_UFRAME_RECEIVE, (void *)system_on_information_received);
        if (status != STATUS_OK)
        {
            cpc_close_endpoint(&system_ep);
            return status;
    ...
    status = cpc_set_endpoint_option(&system_ep, CPC_ENDPOINT_ON_UFRAME_WRITE_COMPLETED, (void *)on_write_completed);
    if (status != STATUS_OK)
    {
        cpc_close_endpoint(&system_ep);
        return status;
    ...
    status = cpc_set_endpoint_option(&system_ep, CPC_ENDPOINT_ON_IFRAME_WRITE_COMPLETED, (void *)on_write_completed);
    if (status != STATUS_OK)
    {
        cpc_close_endpoint(&system_ep);
        return status;
    ...
    status = cpc_set_endpoint_option(&system_ep, CPC_ENDPOINT_ON_ERROR, (void *)on_system_ep_error);
    if (status != STATUS_OK)
    {
        cpc_close_endpoint(&system_ep);
        return status;
...
void cpc_system_process(void)
{
    ...
    if (restart_flag)
    {
        state = cpc_get_endpoint_state(&system_ep);

        if (state == CPC_STATE_FREED)
        {
            if (open_endpoint() == STATUS_OK)
            {
                restart_flag = false;
            }
            else
    ...
    if (process_uframes_flag)
    {
        ...
        status_t status = cpc_read(&system_ep, &data, &data_length, 0, CPC_FLAG_NO_BLOCK | CPC_FLAG_UNNUMBERED_INFORMATION);
        if (status == STATUS_OK)
        {
            if (data_length > 0)
            {
...
static void on_system_ep_error(cpc_user_endpoint_id_t endpoint_id)
{
    configASSERT(endpoint_id == CPC_ENDPOINT_SYSTEM);
    restart_flag = true;
}
...
//  __weak void sscpc_system_reset(cpc_system_reboot_mode_t reboot_mode)
// {
//   (void)reboot_mode;
// #if (defined(CATALOG_EMLIB_CORE_PRESENT))
// #if (defined(CATALOG_GECKO_BOOTLOADER_INTERFACE_PRESENT))
...
// #endif  // CATALOG_GECKO_BOOTLOADER_INTERFACE_PRESENT
...
// #endif  // CATALOG_EMLIB_CORE_PRESENT
//   exit(1);
// }
                               ...
                               status_t status)
{
    ...
    if (buffer != NULL)
    {
        cpc_free_command_buffer(buffer);
    }
...
{
    status_t status;
    cpc_system_cmd_t *tx_command;
    cpc_system_property_cmd_t *tx_prop_command;
    cpc_endpoint_state_t *ep_state;

    status = cpc_get_system_command_buffer(&tx_command);
    if (status != STATUS_OK)
    {
        return status;
    }
    ...
    tx_prop_command = (cpc_system_property_cmd_t *)(tx_command->payload);

    tx_prop_command->property_id = EP_ID_TO_PROPERTY_STATE(endpoint_id);

    ep_state = (cpc_endpoint_state_t *)tx_prop_command->payload;

    tx_command->header.length = sizeof(cpc_system_property_cmd_t) + sizeof(cpc_endpoint_state_t);

    *ep_state = CPC_STATE_CLOSING;

    status = cpc_write(&system_ep,
                       (void *)tx_command,
                       sizeof(cpc_system_cmd_header_t) + tx_command->header.length,
                       0,
                       NULL);
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
__weak cpc_system_status_t cpc_get_reset_reason(void)
{
    cpc_system_status_t last_status = STATUS_RESET_POWER_ON;
#if defined(RMU_PRESENT)
...
static void send_reset_reason(void)
{
    ...
    cpc_system_cmd_t *tx_command;
    cpc_system_property_cmd_t *tx_prop_command;
    cpc_system_status_t *tx_prop_last_status;

    status = cpc_get_system_command_buffer(&tx_command);
    configASSERT(status == STATUS_OK);
    ...
    tx_prop_last_status = (cpc_system_status_t *)tx_prop_command->payload;

    *tx_prop_last_status = cpc_get_reset_reason();

    tx_command->header.length = sizeof(cpc_system_property_cmd_t) + sizeof(cpc_system_status_t);

    status = cpc_write(&system_ep,
                       (void *)tx_command,
                       sizeof(cpc_system_cmd_header_t) + tx_command->header.length,
                       CPC_FLAG_UNNUMBERED_INFORMATION,
                       NULL);
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_last_status(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;

    prop_cmd_buff = (cpc_system_property_cmd_t *)tx_command->payload;
    prop_cmd_buff->property_id = PROP_LAST_STATUS;
    *((cpc_system_status_t *)(prop_cmd_buff->payload)) = STATUS_OK;

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(cpc_system_status_t);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_protocol_version(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;

    prop_cmd_buff = (cpc_system_property_cmd_t *)tx_command->payload;
    prop_cmd_buff->property_id = PROP_PROTOCOL_VERSION;
    prop_cmd_buff->payload[0] = CPC_PROTOCOL_VERSION;

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(uint8_t);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *   Ship the hardcoded major and minor version number back to the primary.
                                                                               ******************************************************************************/
static void on_property_get_secondary_cpc_version(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
    uint32_t *version;
    ...
    tx_command->header.length = sizeof(cpc_property_id_t) + (3 * sizeof(uint32_t));
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_secondary_app_version(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
    ...
    app_version = cpc_secondary_app_version();
    app_version_len = strlen(app_version) + 1;
    memcpy(prop_cmd_buff->payload, app_version, app_version_len > CPC_RX_PAYLOAD_MAX_LENGTH ? CPC_RX_PAYLOAD_MAX_LENGTH : app_version_len);

    tx_command->header.length = sizeof(cpc_property_id_t) + app_version_len;
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_capabilities(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
    ...
    *capabilities = CPC_PACKED_ENDPOINT_PRESENT * CPC_CAPABILITIES_PACKED_ENDPOINT_MASK + CPC_ENDPOINT_GPIO_ENABLED * CPC_CAPABILITIES_GPIO_ENDPOINT_MASK + cpc_driver_capabilities.uart_flowcontrol * CPC_CAPABILITIES_UART_FLOW_CONTROL_MASK;

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(uint32_t);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_rx_capabilities(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
    uint16_t *rx_capability;

    xTimerStop(sResetTimer, 0);

    prop_cmd_buff = (cpc_system_property_cmd_t *)tx_command->payload;
    prop_cmd_buff->property_id = PROP_RX_CAPABILITY;
    rx_capability = (uint16_t *)(prop_cmd_buff->payload);
    *rx_capability = CPC_RX_PAYLOAD_MAX_LENGTH;

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(uint16_t);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_fc_validation_value(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_bus_speed_value(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *prop_cmd_buff;
    ...
    *bus_speed_value = cpc_drv_get_bus_speed();
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_bootloader_info(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *tx_property;
    uint32_t *infos;

    tx_property = (cpc_system_property_cmd_t *)tx_command->payload;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_bootloader_reboot_mode(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *tx_property;
    cpc_system_reboot_mode_t *mode;

    tx_property = (cpc_system_property_cmd_t *)tx_command->payload;
    tx_property->property_id = PROP_BOOTLOADER_REBOOT_MODE;
    mode = (cpc_system_reboot_mode_t *)(tx_property->payload);
    *mode = prop_bootloader_reboot_mode;

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(cpc_system_reboot_mode_t);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_security_state(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *tx_property;
...
#ifdef CATALOG_CPC_SECURITY_PRESENT
    tx_property->property_id = PROP_SECURITY_STATE;
    *security_state = cpc_security_get_state();
#else
                                           ...
                                           uint8_t ep_id)
{
    ...
    reply_prop_cmd_buff = (cpc_system_property_cmd_t *)tx_command->payload;
    ...
    *reply_ep_state = cpc_get_endpoint_state(&dummy_ep);

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(cpc_endpoint_state_t);
}
                                                ...
                                                uint8_t ep_id)
{
    ...
    *reply_ep_encryption = cpc_get_endpoint_encryption(&dummy_ep);

    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(bool);
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_endpoint_states(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *reply_prop_cmd_buff;
    ...
    for (size_t i = 0; i != CPC_ENDPOINT_MAX_COUNT / 2; i++)
    {
        cpc_endpoint_handle_t dummy_ep1 = {.ep = NULL, .id = 2 * i};
        cpc_endpoint_handle_t dummy_ep2 = {.ep = NULL, .id = 2 * i + 1};

        cpc_endpoint_state_t ep1_state = cpc_get_endpoint_state(&dummy_ep1);
        cpc_endpoint_state_t ep2_state = cpc_get_endpoint_state(&dummy_ep2);

        // Although an 'cpc_endpoint_state_t' is an 8 bit value, the number of
    ...
    tx_command->header.length = sizeof(cpc_property_id_t) + (CPC_ENDPOINT_MAX_COUNT * sizeof(uint8_t) / 2);
}
...
static void on_property_get_core_debug_counters(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *reply_prop_cmd_buff;

    reply_prop_cmd_buff = (cpc_system_property_cmd_t *)tx_command->payload;
    memcpy(reply_prop_cmd_buff->payload, &cpc_core_debug.core_counters, sizeof(cpc_core_debug_counters_t));

    reply_prop_cmd_buff->property_id = PROP_CORE_DEBUG_COUNTERS;
    tx_command->header.length = sizeof(cpc_property_id_t) + sizeof(cpc_core_debug_counters_t);
}
#endif
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get_set_not_found(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *tx_property_cmd;

    tx_property_cmd = (cpc_system_property_cmd_t *)tx_command->payload;
    tx_property_cmd->property_id = PROP_LAST_STATUS;
    *((cpc_system_status_t *)(tx_property_cmd->payload)) = STATUS_PROP_NOT_FOUND;
                                  ...
                                  cpc_system_cmd_t *rx_command)
{
    ...
    tx_property_command = (cpc_system_property_cmd_t *)tx_command->payload;

    cpc_remote_disconnected(endpoint_id);

    tx_command->header.length = sizeof(cpc_property_id_t);
                                              ...
                                              cpc_system_cmd_t *rx_command)
{
    ...
    rx_property_command = (cpc_system_property_cmd_t *)rx_command->payload;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_set_enter_irq(cpc_system_cmd_t *tx_command,
                                      cpc_system_cmd_t *rx_command)
{
    cpc_system_property_cmd_t *tx_property_command;
    cpc_system_property_cmd_t *rx_property_command;
    cpc_system_enter_irq_cmd_t *enter_irq_command;

    rx_property_command = (cpc_system_property_cmd_t *)rx_command->payload;
    enter_irq_command = (cpc_system_enter_irq_cmd_t *)rx_property_command->payload;
    enter_irq_end_ms = enter_irq_command->end_in_ms;
    cpc_timer_start_timer_ms(&enter_irq_timer,
                                                   ...
                                                   cpc_system_cmd_t *rx_command)
{
    cpc_system_property_cmd_t *tx_property_command;
    cpc_system_property_cmd_t *rx_property_command;
    cpc_system_reboot_mode_t *rx_mode;
    cpc_system_reboot_mode_t *tx_mode;

    tx_property_command = (cpc_system_property_cmd_t *)tx_command->payload;
    rx_property_command = (cpc_system_property_cmd_t *)rx_command->payload;
    rx_mode = (cpc_system_reboot_mode_t *)rx_property_command->payload;
    tx_mode = (cpc_system_reboot_mode_t *)tx_property_command->payload;
    ...
    case REBOOT_APPLICATION:
        ...
        *tx_mode = REBOOT_APPLICATION;
        tx_command->header.length += sizeof(cpc_system_reboot_mode_t);
        break;

    case REBOOT_BOOTLOADER:
        /* Check if the bootloader interface is present */

#if (defined(CPC_HAS_BOOTLOADER_SUPPORT))
    ...
    default:
        ...
        *((cpc_system_status_t *)(tx_property_command->payload)) = STATUS_INVALID_ARGUMENT;
        tx_command->header.length += sizeof(cpc_system_status_t);
        break;
    }
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_set_read_only(cpc_system_cmd_t *tx_command)
{
    cpc_system_property_cmd_t *tx_property_cmd;

    tx_property_cmd = (cpc_system_property_cmd_t *)tx_command->payload;
    tx_property_cmd->property_id = PROP_LAST_STATUS;
    *((cpc_system_status_t *)(tx_property_cmd->payload)) = STATUS_INVALID_COMMAND_FOR_PROP;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *   of the operation.
                                                                               ******************************************************************************/
static void on_noop(cpc_system_cmd_t *noop,
                    uint32_t *reply_data_lenght)
{
    noop->header.command_id = CMD_SYSTEM_NOOP;
    noop->header.length = 0;
    *reply_data_lenght = sizeof(cpc_system_cmd_header_t) + noop->header.length;
}

/***************************************************************************/ /**
                                                                               * Handle reset request
                                                                               ******************************************************************************/
__weak cpc_system_status_t cpc_secondary_on_reset_request(cpc_system_reboot_mode_t reboot_mode)
{
    switch (reboot_mode)
    {
    case REBOOT_APPLICATION:
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *   _reboot_mode'
                                                                               ******************************************************************************/
static void on_reset(cpc_system_cmd_t *reset,
                     uint32_t *reply_data_lenght,
                     void **on_write_complete_arg)
{
    cpc_system_status_t *reset_status;

    reset_status = (cpc_system_status_t *)(reset->payload);
    reset->header.command_id = CMD_SYSTEM_RESET;

    reset->header.length = sizeof(cpc_system_status_t);
    *reply_data_lenght = sizeof(cpc_system_cmd_header_t) + reset->header.length;
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
static void on_property_get(cpc_system_cmd_t *rx_command,
                            cpc_system_cmd_t *reply,
                            uint32_t *reply_data_lenght)
{
    cpc_system_property_cmd_t *rx_property_cmd = (cpc_system_property_cmd_t *)rx_command->payload;
    ...
    default:
        ...
        if (rx_property_cmd->property_id >= EP_ID_TO_PROPERTY_STATE(0) && rx_property_cmd->property_id <= EP_ID_TO_PROPERTY_STATE(255))
        {
        ...
        if (rx_property_cmd->property_id >= EP_ID_TO_PROPERTY_ENCRYPTION(0) && rx_property_cmd->property_id <= EP_ID_TO_PROPERTY_ENCRYPTION(255))
        {
            uint8_t ep_id = PROPERTY_ID_TO_EP_ID(rx_property_cmd->property_id);
    ...
    *reply_data_lenght = sizeof(cpc_system_cmd_header_t) + reply->header.length;
}
...
/***************************************************************************/ /**
                                                                               ...
                                                                               *   property identifier.
                                                                               ******************************************************************************/
static void on_property_set(cpc_system_cmd_t *rx_command,
                            cpc_system_cmd_t *reply,
                            uint32_t *reply_data_lenght)
{
    cpc_system_property_cmd_t *rx_property_cmd;

    rx_property_cmd = (cpc_system_property_cmd_t *)(rx_command->payload);

    // Reply to a PROPERTY-GET with a PROPERTY-IS
    reply->header.command_id = CMD_SYSTEM_PROP_VALUE_IS;

    // Populate the reply command buffer according to the property_id
    if (rx_property_cmd->property_id >= PROP_ENDPOINT_STATE_0 && rx_property_cmd->property_id <= PROP_ENDPOINT_STATE_255)
    {
                    ...
                    void **on_write_complete_arg)
{
    ...
    cpc_system_cmd_t *rx_command = (cpc_system_cmd_t *)poll_data;
    cpc_system_cmd_t *tx_command;
    *reply_data = NULL;
    *reply_data_lenght = 0;
    (void)endpoint_id;

    if (frame_type != CPC_HDLC_FRAME_TYPE_UNNUMBERED && frame_type != CPC_HDLC_FRAME_TYPE_DATA)
    {
        configASSERT(false);
        return; // Drop packet
    }

    if (endpoint_id != CPC_ENDPOINT_SYSTEM)
    {
        configASSERT(false);
        return; // Drop packet
    }
    // Make sure the length of the payload from the command matches the returned length.
    if (rx_command->header.length != (poll_data_length - sizeof(cpc_system_cmd_header_t)))
    {
        configASSERT(false);
        return; // Drop packet
    }
    ...
    *on_write_complete_arg = NULL;
    ...
    status_t status = cpc_get_system_command_buffer(&tx_command);
    ...
    *reply_data = tx_command;
    ...
    if (frame_type == CPC_HDLC_FRAME_TYPE_UNNUMBERED)
    {
        cpc_system_property_cmd_t *rx_property_cmd = (cpc_system_property_cmd_t *)rx_command->payload;
        switch (rx_command->header.command_id)
        {
        case CMD_SYSTEM_RESET:
            on_reset((cpc_system_cmd_t *)*reply_data, reply_data_lenght, on_write_complete_arg);
            break;

        case CMD_SYSTEM_PROP_VALUE_GET:
            if (rx_property_cmd->property_id == PROP_RX_CAPABILITY || rx_property_cmd->property_id == PROP_CAPABILITIES || rx_property_cmd->property_id == PROP_BUS_SPEED_VALUE || rx_property_cmd->property_id == PROP_PROTOCOL_VERSION || rx_property_cmd->property_id == PROP_BOOTLOADER_INFO || rx_property_cmd->property_id == PROP_SECONDARY_CPC_VERSION || rx_property_cmd->property_id == PROP_SECONDARY_APP_VERSION)
            {
                on_property_get(rx_command, (cpc_system_cmd_t *)*reply_data, reply_data_lenght);
            }
            break;
        ...
        case CMD_SYSTEM_PROP_VALUE_SET:
            ...
            {
                on_property_set(rx_command, (cpc_system_cmd_t *)*reply_data, reply_data_lenght);
            }
            break;
    ...
    else if (frame_type == CPC_HDLC_FRAME_TYPE_DATA)
    {
        switch (rx_command->header.command_id)
        {
        case CMD_SYSTEM_NOOP:
            on_noop((cpc_system_cmd_t *)*reply_data, reply_data_lenght);
            break;
                                     ...
                                     void *data)
{
    ...
    uint32_t now_ms = cpc_timer_tick_to_ms(cpc_timer_get_tick_count());
    uint32_t end_ms = *(uint32_t *)data;
    while ((cpc_timer_tick_to_ms(cpc_timer_get_tick_count()) - now_ms) <= end_ms)
        ;
}
