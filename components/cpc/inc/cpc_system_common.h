...
#ifndef CPC_SYSTEM_COMMON_H
#define CPC_SYSTEM_COMMON_H
...
SENUM_GENERIC(cpc_property_id_t, uint32_t)
{
...
/***************************************************************************/ /**
                                                                               ...
                                                                               ******************************************************************************/
#define EP_ID_TO_PROPERTY_ID(property, ep_id) ((cpc_property_id_t)((property) | ((ep_id)&0x000000FF)))
...
SENUM_GENERIC(cpc_system_status_t, uint32_t)
{
...
SENUM_GENERIC(cpc_system_reboot_mode_t, uint32_t)
{
...
typedef struct
{
    cpc_system_cmd_id_t command_id; ///< Identifier of the command.
    uint8_t seq;                    ///< Command sequence number.
    uint16_t length;                ///< Length of the payload in bytes.
} cpc_system_cmd_header_t;

typedef struct
{
    cpc_system_cmd_header_t header;      ///< Command header
    uint8_t payload[PAYLOAD_LENGTH_MAX]; ///< Command payload.
} cpc_system_cmd_t;

typedef struct
{
    cpc_property_id_t property_id; ///< Identifier of the property.
    uint8_t payload[12];           ///< Property value.
} cpc_system_property_cmd_t;
