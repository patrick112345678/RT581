 ...
 * DSR.
 */
/* PURPOSE: copy-paste from secur_ccm.c to get zb_aes128()
*/
...
#if defined ZB_SOFT_SECURITY && !defined ZB_AES128_ONLY
    ...
    zb_uint8_t *encrypted)
{
    ...
    TRACE_MSG(TRACE_SECUR3, "encrypted_auth ccm_m %hd key %p nonce %p string_m %p string_m_len %d t %p encrypted %p",
...
#ifndef ZB_WINDOWS
void
zbee_sec_hash(zb_uint8_t *input, zb_ushort_t input_len, zb_uint8_t *output)
{
...
void zb_cmm_key_hash(zb_uint8_t *key, zb_uint8_t input, zb_uint8_t *hash_key)
{
    ...
    zb_uint8_t  hash_out[ZB_CCM_KEY_SIZE + 1];
    zb_ushort_t i;
#define ipad  0x36
#define opad  0x5c
     ...
     */
    zbee_sec_hash(hash_out, ZB_CCM_KEY_SIZE + 1, hash_in + ZB_CCM_KEY_SIZE);
    /* Hash the contents of hash_in to get the final result. */
    zbee_sec_hash(hash_in, 2 * ZB_CCM_KEY_SIZE, hash_out);
