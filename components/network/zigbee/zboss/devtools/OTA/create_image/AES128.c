...
static void xor_round_key(uint8 *state, uint8 *keys, uint16 round)
{
...
// url: en.wikipedia.org/wiki/Rijndael_S-box
static void sub_bytes(uint8 *a, uint16 n)
{
    uint16 i;
    for (i = 0; i < n; i++)
    {
...
static void key_schedule_core(uint8 *a, uint16 i)
{
    uint8 temp = a[0]; // Rotate the output eight bits to the left
    a[0] = a[1];
    a[1] = a[2];
    a[2] = a[3];
    a[3] = temp;
    sub_bytes(a, 4); // Apply Rijndael's S-box on all four individual bytes in the output word
    a[0] ^= lookup_rcon[i]; // On just the first (leftmost) uint8 of the output word, perform the rcon operation with i
    // as the input, and exclusive or the rcon output with the first uint8 of the output word
}

// Expand the 16-uint8 key to 11 round keys (176 bytes)
// url: en.wikipedia.org/wiki/Rijndael_key_schedule#The_key_schedule
static void expand_key(uint8 *key, uint8 *keys)
{
    uint16 bytes = 16; // The count of how many bytes we've created so far
    uint16 i = 1; // The rcon iteration value i is set to 1
    uint16 j; // For repeating the second stage 3 times
    uint8 t[4]; // Temporary working area known as 't' in the Wiki article
    ...
    while (bytes < 176) // Until we have 176 bytes of expanded key, we do the following:
    {
        ...
        xor(t, keys + bytes - 16, 4); // We exclusive-or t with the four-uint8 block 16 bytes before the new expanded key.
        ...
        for (j = 0; j < 3; j++)
        {
            ...
            xor(t, keys + bytes - 16, 4); // We exclusive-or t with the four-uint8 block n bytes before
            ZB_MEMCPY(keys + bytes, t, 4); // This becomes the next 4 bytes in the expanded key
            bytes += 4; // Keep track of how many expanded key bytes we've added
        }
    }
...
static void shift_rows(uint8 *state)
{
    uint16 i;
    uint8 temp[16];
...
static void mix_col (uint8 *state)
{
    uint8 a0 = state[0];
    uint8 a1 = state[1];
    uint8 a2 = state[2];
...
static void mix_cols (uint8 *state)
{
    mix_col(state);
    mix_col(state + 4);
    mix_col(state + 8);
