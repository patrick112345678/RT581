...
void zb_gp_proxy_data_indication(zb_uint8_t param)
{
    ...
    if (ZGP_IS_PROXY_IN_COMMISSIONING_MODE())
    {
        ...
        else if (gpdf_info->status == GP_DATA_IND_STATUS_NO_SECURITY &&
        ...
        else if (gpdf_info->zgpd_cmd_id < ZB_GPDF_CMD_COMMISSIONING &&
    ...
    if (param)
    {
        ...
        else if (gpdf_info->status == GP_DATA_IND_STATUS_SECURITY_SUCCESS || gpdf_info->status == GP_DATA_IND_STATUS_NO_SECURITY)
        {
...
static zb_ret_t zgp_sink_gp_pairing_configuration_add_new_entry(zb_uint8_t param)
{
    ...
    else if (ZB_ZGP_GP_PAIRING_CONF_GET_SEQ_NUM_CAPS(conf->options))
    {
        ent.security_counter = conf->sec_frame_counter;
    }
    ...
    else if (ZB_ZGP_GP_PAIRING_CONF_GET_COMMUNICATION_MODE(conf->options) == ZGP_COMMUNICATION_MODE_GROUPCAST_DERIVED)
    {
