...
*/
#define ZB_TRACE_FILE_ID 40205
#include "zboss_api.h"
#include "zb_led_button.h"
#include "zb_simple_sensor.h"
...
enum test_step_e
{
    ...
    TEST_STEP_READ_BINARY_INPUT_OUT_OF_SERVICE, /** Read OutOfService attribute from Binary Input (Basic) cluster */
    TEST_STEP_WRITE_BINARY_INPUT_OUT_OF_SERVICE, /** Write OutOfService attribute to Binary Input (Basic) cluster */
    TEST_STEP_READ_BINARY_INPUT_PRESENT_VALUE, /** Read PresentValue attribute from Binary Input (Basic) cluster */
    TEST_STEP_WRITE_BINARY_INPUT_PRESENT_VALUE, /** Write PresentValue attribute to Binary Input (Basic) cluster */
    TEST_STEP_READ_BINARY_INPUT_STATUS_FLAG, /** Read StatusFlag attribute from Binary Input (Basic) cluster */
...
ZB_HA_DECLARE_SIMPLE_SENSOR_CLUSTER_LIST_ZED(simple_sensor_clusters,  identify_attr_list);
ZB_HA_DECLARE_SIMPLE_SENSOR_EP_ZED(simple_sensor_ep, ZB_SWITCH_ENDPOINT, simple_sensor_clusters);
ZB_HA_DECLARE_SIMPLE_SENSOR_CTX(device_ctx, simple_sensor_ep);
...
void test_next_step(zb_uint8_t param)
{
    ...
    break;
    case TEST_STEP_READ_BINARY_INPUT_OUT_OF_SERVICE:
        TRACE_MSG(TRACE_APP1, "Read OutOfService attribute from Binary Input (Basic) cluster", (FMT__0));
        g_test_step++;
        send_read_attr(buffer, ZB_ZCL_CLUSTER_ID_BINARY_INPUT, ZB_ZCL_ATTR_BINARY_INPUT_OUT_OF_SERVICE_ID);
        break;
    case TEST_STEP_WRITE_BINARY_INPUT_OUT_OF_SERVICE:
    {
        zb_bool_t binary_input_out_of_service_def = ZB_ZCL_BINARY_INPUT_OUT_OF_SERVICE_DEFAULT_VALUE;
        TRACE_MSG(TRACE_APP1, "Write OutOfService attribute to Binary Input (Basic) cluster", (FMT__0));
        g_test_step++;
        send_write_attr(buffer, ZB_ZCL_CLUSTER_ID_BINARY_INPUT, ZB_ZCL_ATTR_BINARY_INPUT_OUT_OF_SERVICE_ID, ZB_ZCL_ATTR_TYPE_BOOL, (zb_uint8_t *)&binary_input_out_of_service_def);
    }
    break;
    case TEST_STEP_READ_BINARY_INPUT_PRESENT_VALUE:
        TRACE_MSG(TRACE_APP1, "Read PresentValue attribute from Binary Input (Basic) cluster", (FMT__0));
        g_test_step++;
        send_read_attr(buffer, ZB_ZCL_CLUSTER_ID_BINARY_INPUT, ZB_ZCL_ATTR_BINARY_INPUT_PRESENT_VALUE_ID);
        break;
    case TEST_STEP_WRITE_BINARY_INPUT_PRESENT_VALUE:
    {
        zb_bool_t binary_input_present_value_def = ZB_FALSE;
        TRACE_MSG(TRACE_APP1, "Write PresentValue attribute to Binary Input (Basic) cluster", (FMT__0));
        g_test_step++;
        send_write_attr(buffer, ZB_ZCL_CLUSTER_ID_BINARY_INPUT, ZB_ZCL_ATTR_BINARY_INPUT_PRESENT_VALUE_ID, ZB_ZCL_ATTR_TYPE_BOOL, (zb_uint8_t *)&binary_input_present_value_def);
    }
    break;
    case TEST_STEP_READ_BINARY_INPUT_STATUS_FLAG:
        TRACE_MSG(TRACE_APP1, "Read StatusFlag attribute from Binary Input (Basic) cluster", (FMT__0));
        g_test_step++;
        send_read_attr(buffer, ZB_ZCL_CLUSTER_ID_BINARY_INPUT, ZB_ZCL_ATTR_BINARY_INPUT_STATUS_FLAG_ID);
        break;
