...
enum test_step_e
{
  ...
  TEST_STEP_PRICE_SEND_CMD_GET_CURRENT_PRICE, /** Send Get Current Price command to Price cluster */
  TEST_STEP_PRICE_SEND_CMD_GET_SCHEDULED_PRICES, /** Send Get Scheduled Prices command to Price cluster */
  TEST_STEP_READ_PRICE_COMMODITY_TYPE, /** Read CommodityType attribute from Price cluster */
  TEST_STEP_DRLC_SEND_REPORT_EVENT_STATUS_REQ, /** Send Report Event Status command to Demand Response and Load Control cluster */
  TEST_STEP_DRLC_SEND_GET_SCHEDULED_EVENTS_REQ, /** Send Get Scheduled Events command to Demand Response and Load Control cluster */
  TEST_STEP_MESSAGING_SEND_GET_LAST_MESSAGE_REQ, /** Send Get Last Message command to Messaging (Smart Energy) cluster */
  TEST_STEP_MESSAGING_SEND_MESSAGE_CONFIRMATION_REQ, /** Send Message Confirmation command to Messaging (Smart Energy) cluster */
  TEST_STEP_READ_TUNNELING_CLOSE_TUNNEL_TIMEOUT, /** Read CloseTunnelTimeout attribute from Tunneling (Smart Energy) cluster */
  TEST_STEP_TUNNELING_SEND_REQUEST_TUNNEL, /** Send RequestTunnel command to Tunneling (Smart Energy) cluster */
  TEST_STEP_TUNNELING_SEND_TRANSFER_DATA, /** Send TransferData command to Tunneling (Smart Energy) cluster */
  TEST_STEP_TUNNELING_SEND_CLOSE_TUNNEL, /** Send CloseTunnel command to Tunneling (Smart Energy) cluster */
...
zb_uint8_t g_attr_price_cli_price_increase_randomize_minutes = 0x05;
zb_uint8_t g_attr_price_cli_price_decrease_randomize_minutes = 0x0F;
zb_uint8_t g_attr_price_cli_commodity_type = 0;
/* Demand Response and Load Control cluster attributes data */
zb_uint8_t g_attr_drlc_utility_enrollment_group = 0x00;
zb_uint8_t g_attr_drlc_start_randomization_minutes = 0x1E;
zb_uint8_t g_attr_drlc_duration_randomization_minutes = 0x1E;
zb_uint16_t g_attr_drlc_device_class_value = 0;
ZB_ZCL_DECLARE_PRICE_CLI_ATTRIB_LIST(price_cli_attr_list, &g_attr_price_cli_price_increase_randomize_minutes, &g_attr_price_cli_price_decrease_randomize_minutes, &g_attr_price_cli_commodity_type);
ZB_ZCL_DECLARE_DRLC_ATTRIB_LIST(drlc_attr_list, &g_attr_drlc_utility_enrollment_group, &g_attr_drlc_start_randomization_minutes, &g_attr_drlc_duration_randomization_minutes, &g_attr_drlc_device_class_value);
/********************* Declare device **************************/
#define ZB_SWITCH_ENDPOINT          10

ZB_HA_DECLARE_SE_TEST_SAMPLE_CLUSTER_LIST_ZED(se_test_sample_clusters,  price_cli_attr_list, drlc_attr_list);
ZB_HA_DECLARE_SE_TEST_SAMPLE_EP_ZED(se_test_sample_ep, ZB_SWITCH_ENDPOINT, se_test_sample_clusters);
...
void test_device_interface_cb(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "> test_device_interface_cb param %hd id %hd",
  ...
  switch (device_cb_param->device_cb_id)
  {
    ...
    {
      zb_zcl_tunneling_request_tunnel_response_t *params =
        (zb_zcl_tunneling_request_tunnel_response_t*)device_cb_param->cb_param.gnr.in;
      TRACE_MSG(TRACE_APP1, "ZB_ZCL_TUNNELING_REQUEST_TUNNEL_RESPONSE_CB_ID", (FMT__0));
      TRACE_MSG(TRACE_APP1,
                "tunnel_id 0x%x, tunnel_status 0x%hx, max_incoming_transfer_size %d",
                (FMT__D_H_D, params->tunnel_id, params->tunnel_status, params->max_incoming_transfer_size));
    }
    break;
...
void test_next_step(zb_uint8_t param)
{
  ...
  case TEST_STEP_IDENTIFY_SEND_TRIGGER_VARIANT_REQ:
    ...
    break;
  case TEST_STEP_PRICE_SEND_CMD_GET_CURRENT_PRICE:
    {
      TRACE_MSG(TRACE_APP1, "Send Get Current Price command to Price cluster", (FMT__0));
      g_test_step++;
      ZB_ZCL_PRICE_SEND_CMD_GET_CURRENT_PRICE(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, ZB_TRUE);
    }
    break;
  case TEST_STEP_PRICE_SEND_CMD_GET_SCHEDULED_PRICES:
    {
      zb_zcl_price_get_scheduled_prices_payload_t payload = { 0 };
      TRACE_MSG(TRACE_APP1, "Send Get Scheduled Prices command to Price cluster", (FMT__0));
      g_test_step++;
      ZB_ZCL_PRICE_SEND_CMD_GET_SCHEDULED_PRICES(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, &payload);
    }
    break;
  case TEST_STEP_READ_PRICE_COMMODITY_TYPE:
    TRACE_MSG(TRACE_APP1, "Read CommodityType attribute from Price cluster", (FMT__0));
    g_test_step++;
    send_read_attr(buffer, ZB_ZCL_CLUSTER_ID_PRICE, ZB_ZCL_ATTR_PRICE_SRV_COMMODITY_TYPE);
    break;
  case TEST_STEP_DRLC_SEND_REPORT_EVENT_STATUS_REQ:
    {
/* [drlc_send_cmd_report_event_status] */
      zb_zcl_drlc_report_event_status_payload_t cmd_data = { 0 };
      TRACE_MSG(TRACE_APP1, "Send Report Event Status command to Demand Response and Load Control cluster", (FMT__0));
      g_test_step++;
      cmd_data.event_status = ZB_ZCL_DRLC_EVENT_LCE_RECEIVED;
      cmd_data.event_status_time = ZB_TIMER_GET();
      cmd_data.cooling_temperature_set_point_applied = 0x8000;
      cmd_data.heating_temperature_set_point_applied = 0x8000;
      cmd_data.average_load_adjustment_percentage_applied = 0x80;
      cmd_data.duty_cycle_applied = 0xFF;
      cmd_data.event_control = ZB_ZCL_DRLC_DEVICE_CLASS_HVAC;
      ZB_ZCL_DRLC_SEND_CMD_REPORT_EVENT_STATUS(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, &cmd_data);
/* [drlc_send_cmd_report_event_status] */
    }
    break;
  case TEST_STEP_DRLC_SEND_GET_SCHEDULED_EVENTS_REQ:
    {
      zb_zcl_drlc_get_scheduled_events_payload_t cmd_data = { 0 };
      TRACE_MSG(TRACE_APP1, "Send Get Scheduled Events command to Demand Response and Load Control cluster", (FMT__0));
      g_test_step++;
      cmd_data.start_time = ZB_TIMER_GET();
      cmd_data.number_of_events = 0;
      cmd_data.issuer_event_id = 0xFFFFFFFF;
      ZB_ZCL_DRLC_SEND_CMD_GET_SCHEDULED_EVENTS(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, &cmd_data);
    }
    break;
  case TEST_STEP_MESSAGING_SEND_GET_LAST_MESSAGE_REQ:
    {
      TRACE_MSG(TRACE_APP1, "Send Get Last Message command to Messaging (Smart Energy) cluster", (FMT__0));
      g_test_step++;
      ZB_ZCL_MESSAGING_SEND_GET_LAST_MSG(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED);
    }
    break;
  case TEST_STEP_MESSAGING_SEND_MESSAGE_CONFIRMATION_REQ:
    {
      zb_zcl_messaging_message_confirm_payload_t cmd_data = { 0 };
      TRACE_MSG(TRACE_APP1, "Send Message Confirmation command to Messaging (Smart Energy) cluster", (FMT__0));
      g_test_step++;
      cmd_data.confirmation_time = ZB_TIMER_GET();
      ZB_ZCL_MESSAGING_SEND_MSG_CONFIRMATION(buffer, (zb_addr_u*)&DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, &cmd_data);
    }
    break;
  case TEST_STEP_READ_TUNNELING_CLOSE_TUNNEL_TIMEOUT:
    TRACE_MSG(TRACE_APP1, "Read CloseTunnelTimeout attribute from Tunneling (Smart Energy) cluster", (FMT__0));
    g_test_step++;
    send_read_attr(buffer, ZB_ZCL_CLUSTER_ID_TUNNELING, ZB_ZCL_ATTR_TUNNELING_CLOSE_TUNNEL_TIMEOUT_ID);
    break;
  case TEST_STEP_TUNNELING_SEND_REQUEST_TUNNEL:
    {
      TRACE_MSG(TRACE_APP1, "Send RequestTunnel command to Tunneling (Smart Energy) cluster", (FMT__0));
      g_test_step++;
      ZB_ZCL_TUNNELING_SEND_REQUEST_TUNNEL(buffer, DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, ZB_AF_HA_PROFILE_ID, ZB_ZCL_ENABLE_DEFAULT_RESPONSE, NULL, ZB_ZCL_TUNNELING_PROTOCOL_MANUFACTURER_DEFINED, 0 /* no manufacture code */, ZB_FALSE, 10 /* max transfer size */);
    }
    break;
  case TEST_STEP_TUNNELING_SEND_TRANSFER_DATA:
    {
      const zb_uint8_t MAX_TUNNEL_DATA_SIZE = 20;
          ...
          && MAX_TUNNEL_DATA_SIZE > 0)
      {
        ...
        ZB_ZCL_TUNNELING_CLIENT_SEND_TRANSFER_DATA(buffer, ENDPOINT_ED, ZB_AF_HA_PROFILE_ID, ZB_ZCL_ENABLE_DEFAULT_RESPONSE, NULL, tunnel_id, MAX_TUNNEL_DATA_SIZE, data);
      }
    }
    break;
  case TEST_STEP_TUNNELING_SEND_CLOSE_TUNNEL:
    {
      zb_uint16_t tunnel_id = 0;
      TRACE_MSG(TRACE_APP1, "Send CloseTunnel command to Tunneling (Smart Energy) cluster", (FMT__0));
      g_test_step++;
      ZB_ZCL_TUNNELING_SEND_CLOSE_TUNNEL(buffer, DST_ADDR, DST_ADDR_MODE, ENDPOINT_C, ENDPOINT_ED, ZB_AF_HA_PROFILE_ID, ZB_ZCL_ENABLE_DEFAULT_RESPONSE, NULL, tunnel_id);
    }
    break;
