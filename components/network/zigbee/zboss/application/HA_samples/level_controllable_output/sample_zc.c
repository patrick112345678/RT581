...
{
  zb_uint8_t start_up_current_level;
  zb_uint8_t align[3];
...
zb_uint8_t g_attr_start_up_current_level;
zb_uint8_t g_attr_options = 0x01;//ZB_ZCL_LEVEL_CONTROL_OPTIONS_DEFAULT_VALUE;

ZB_ZCL_DECLARE_LEVEL_CONTROL_ATTRIB_LIST_EXT(level_control_attr_list, &g_attr_level_control_current_level, &g_attr_level_control_remaining_time, &g_attr_start_up_current_level, &g_attr_options);

ZB_ZCL_DECLARE_ON_OFF_ATTRIB_LIST_EXT_WITH_START_UP_ON_OFF(on_off_attr_list, &g_attr_on_off_on_off, &g_attr_on_off_global_scene_control, &g_attr_on_off_on_time, &g_attr_on_off_off_wait_time, &g_attr_start_up_on_off);
...
/********************* Declare device **************************/
ZB_HA_DECLARE_LEVEL_CONTROLLABLE_OUTPUT_CLUSTER_LIST(level_controllable_output_clusters, basic_attr_list, identify_attr_list, scenes_attr_list, groups_attr_list, on_off_attr_list, level_control_attr_list);
ZB_HA_DECLARE_LEVEL_CONTROLLABLE_OUTPUT_EP(level_controllable_output_ep, ENDPOINT_C, level_controllable_output_clusters);
ZB_HA_DECLARE_LEVEL_CONTROLLABLE_OUTPUT_CTX(device_ctx, level_controllable_output_ep);
/* [COMMON_DECLARATION] */
zb_uint16_t g_dst_addr = 0;

void apply_start_up_current_level()
{
  if (g_attr_start_up_current_level < 0xff)
  {
    g_attr_level_control_current_level = g_attr_start_up_current_level;
  }
}
...
void nvram_read_app_data(zb_uint8_t page, zb_uint32_t pos, zb_uint16_t payload_length)
{
  ...
  if (ret == RET_OK)
  {
    g_attr_level_control_current_level = ds.start_up_current_level;
    //apply_start_up_current_level();
  }
}
...
zb_ret_t nvram_write_app_data(zb_uint8_t page, zb_uint32_t pos)
{
  ...
  ds.start_up_current_level = g_attr_start_up_current_level;
  /* If we fail, trace is given and assertion is triggered */
          ...
          device_cb_param->cb_param.set_attr_value_param.attr_id == ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID)
      {
        ...
        else
        {
          ...
          zb_osif_led_off(0);
               ...
               device_cb_param->cb_param.set_attr_value_param.attr_id == ZB_ZCL_ATTR_LEVEL_CONTROL_START_UP_CURRENT_LEVEL_ID)
      {
#ifdef ZB_USE_NVRAM
        if (device_cb_param->cb_param.set_attr_value_param.values.data8 != ZB_ZCL_LEVEL_CONTROL_START_UP_CURRENT_LEVEL_USE_PREVIOUS_VALUE)
        {
          g_attr_start_up_current_level = device_cb_param->cb_param.set_attr_value_param.values.data8;
        }
        else
        {
          zb_zcl_attr_t *attr_desc;
          attr_desc = zb_zcl_get_attr_desc_a(ENDPOINT_C, ZB_ZCL_CLUSTER_ID_LEVEL_CONTROL,
                                       ZB_ZCL_CLUSTER_SERVER_ROLE,ZB_ZCL_ATTR_LEVEL_CONTROL_CURRENT_LEVEL_ID);
          g_attr_start_up_current_level =  ZB_ZCL_GET_ATTRIBUTE_VAL_8(attr_desc);
        }
        /* If we fail, trace is given and assertion is triggered */
