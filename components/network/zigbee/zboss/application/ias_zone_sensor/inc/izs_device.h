...
#ifdef IZS_WWAH
#define IZS_JOIN_RETRY_DELAY(_cnt) (5 * ZB_TIME_ONE_SECOND)
#else
...
/* take into account the first join attempt */
...
#define MAX_REDELIVERY_ATTEMPTS() g_device_ctx.app_event_retry_params.max_re_delivery_attempts
#define FIRST_BACKOFF_TIME() g_device_ctx.app_event_retry_params.first_backoff_time_in_seconds
#define MAX_BACKOFF_TIME() g_device_ctx.app_event_retry_params.max_backoff_time_in_seconds
#define COMMON_RATIO() g_device_ctx.app_event_retry_params.backoff_sequence_common_ratio

#define DEBUG_REPORT_TABLE_SIZE  10

#define IZS_COMMON_CLUSTER_LIST           \
...
#ifdef IZS_WWAH
#define IZS_IN_CLUSTER_COUNT  6
#define IZS_OUT_CLUSTER_COUNT 1
#define IZS_CLUSTER_LIST ZB_ZCL_CLUSTER_ID_WWAH, IZS_COMMON_CLUSTER_LIST

#define IZS_DECLARE_IAS_ZONE_CLUSTER_LIST(                                     \
  cluster_list_name,                                                           \
  basic_attr_list,                                                             \
  wwah_attr_list,                                                              \
  identify_attr_list,                                                          \
...
#else /* !IZS_WWAH */
#define IZS_IN_CLUSTER_COUNT  5
#define IZS_OUT_CLUSTER_COUNT 1
#define IZS_CLUSTER_LIST IZS_COMMON_CLUSTER_LIST
...
/* Declare simple descriptor for IAS Zone device */
#define IZS_DECLARE_IAS_ZONE_SIMPLE_DESC(                       \
...
/* Declare endpoint for IAS Zone device */
#define IZS_DECLARE_IAS_ZONE_EP(ep_name, ep_id, cluster_list)           \
      IZS_DECLARE_IAS_ZONE_SIMPLE_DESC(                                 \
          ep_name,                                                      \
          ep_id,                                                        \
          IZS_IN_CLUSTER_COUNT,                                         \
          IZS_OUT_CLUSTER_COUNT);                                       \
...
/* IAS Zone device states enumeration*/
...
/* IZS_DEVICE_GET_ZONE_STATUS must return the current sone status, rather than the zone status
   that is being transferred to the remote - chances are that the remote delays the APS ack
...
typedef struct izs_device_ctx_s
{
  ...
  zb_bufid_t notification_buf; /* buffer is used for sending notifications */
  zb_bool_t notification_in_progress;

  zb_uint16_t current_zone_state;
  zb_uint16_t retry_backoff_cnt; /* Retry backoff counter */
  zb_zcl_wwah_enable_wwah_app_event_retry_algorithm_t app_event_retry_params;
...
typedef ZB_PACKED_PRE struct izs_device_nvram_dataset_s
{
  zb_uint8_t enrollment_method;
  zb_uint8_t zone_status;
  /* Size of the structure in bytes must be divisible by 4 */
  zb_uint8_t aligned[2];
} ZB_PACKED_STRUCT izs_device_nvram_dataset_t;

/*** Check dataset alignment for IAR compiler and ARM Cortex target platfrom ***/
ZB_ASSERT_IF_NOT_ALIGNED_TO_4(izs_device_nvram_dataset_t);

/*** Function for persisting application data (called directly into application) ***/
...
/*** Actions that should be performed before commissioning is started ***/
