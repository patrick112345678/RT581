...
light_zc_ctx_t zc_ctx; /* Global device context */
#endif

zb_ieee_addr_t g_zc_addr = LIGHT_ZC_ADDRESS; /* IEEE address of the device */
                            ...
                            simple_device_t *light_control)
{
  ...
  bind_param->req_dst_addr = light_control->short_addr;
...
static void device_match(zb_uint8_t buf_ref, zb_uint16_t dev_idx)
{
  ...
  /* match_desc_param already has allocated memory for one cluster identifier */
    ...
    for (i = 0; i < LIGHT_ZC_MAX_DEVICES; ++i)
    {
      if (zc_ctx.devices[i].dev_type == SIMPLE_DEV_TYPE_UNUSED ||
          (zc_ctx.devices[i].dev_type != SIMPLE_DEV_TYPE_UNUSED &&
           ZB_IEEE_ADDR_CMP(dev_annce_params->ieee_addr, zc_ctx.devices[i].ieee_addr)))
      {
        dev = &zc_ctx.devices[i];

        if (zc_ctx.devices[i].dev_type == SIMPLE_DEV_TYPE_UNUSED)
        {
          ZB_IEEE_ADDR_COPY(dev->ieee_addr, dev_annce_params->ieee_addr);
          dev->short_addr = dev_annce_params->device_short_addr;
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_DEVICE_REBOOT:
        ...
        /* Turn off link key exchange if legacy device support (<ZB3.0) is needed */
