...
#define ZB_TRACE_FILE_ID 40030

#include "zboss_api.h"
#include "zb_energy_service_interface.h"
...
#define ESI_PRICE_SCHEDULED_PRICES_TIMEOUT (3 * ZB_TIME_ONE_SECOND)
...
typedef struct esi_dev_identify_attrs_s
{
  zb_uint16_t identify_time;
} esi_dev_identify_attrs_t;
...
typedef struct esi_dev_ctx_s
{
  ...
  esi_dev_identify_attrs_t identify_attrs;
...
ZB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_CLUSTER_LIST(esi_dev_clusters,
                                                        basic_attr_list,
                                                        identify_attr_list,
                                                        price_attr_list,
                                                        time_attr_list,
...
ZB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_EP(esi_dev_ep, ESI_DEV_ENDPOINT, esi_dev_clusters);

ZB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_CTX(esi_dev_zcl_ctx, esi_dev_ep);
...
static void esi_price_ack_cb(zb_uint8_t param)
{
  ...
  if (ESI_DEV_PRICE_CTX()->state == ESI_DEV_GET_SCHED_PRICES_STATE_WAIT_PRICE_ACK)
  {
    ...
    ZB_SCHEDULE_APP_ALARM(esi_get_sched_prices_cb_send_pp, 0, ESI_PRICE_SCHEDULED_PRICES_TIMEOUT);
  }
                                    ...
                                    zb_uint16_t cluster)
{
  ...
  {
    if (cluster == ZB_ZCL_CLUSTER_ID_DRLC)
    {
      ZB_IEEE_ADDR_COPY(g_dev_ctx.drlc_client_address.addr_long, addr);
      g_dev_ctx.drlc_client_ep = ep;
      ZB_SCHEDULE_APP_ALARM_CANCEL(esi_dev_cmd_send_lce_event1, ZB_ALARM_ANY_PARAM);
      ZB_GET_OUT_BUF_DELAYED(esi_dev_cmd_send_lce_event1);
    }
    else if (cluster == ZB_ZCL_CLUSTER_ID_CALENDAR)
    {
      ZB_IEEE_ADDR_COPY(g_dev_ctx.calendar_client_address.addr_long, addr);
      g_dev_ctx.calendar_client_ep = ep;
...
void esi_finding_binding_initiator(zb_uint8_t param)
{
  ZVUNUSED(param);
  zb_bdb_finding_binding_initiator(ESI_DEV_ENDPOINT, finding_binding_cb);
}
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch (sig)
    {
      ...
      case ZB_BDB_SIGNAL_STEERING:
        ...
        zb_bdb_finding_binding_target(ESI_DEV_ENDPOINT);
        break;
      ...
      case ZB_BDB_SIGNAL_FINDING_AND_BINDING_TARGET_FINISHED:
        ...
        zb_bdb_finding_binding_initiator(ESI_DEV_ENDPOINT, finding_binding_cb);
        ZB_SCHEDULE_APP_ALARM(esi_finding_binding_initiator, 0, (ZB_BDBC_MIN_COMMISSIONING_TIME_S + 10) * ZB_TIME_ONE_SECOND);
        break;
