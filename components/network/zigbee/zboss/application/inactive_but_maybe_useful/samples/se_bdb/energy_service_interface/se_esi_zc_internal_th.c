...
static void esi_th_entry_point(zb_uint8_t param);
...
MAIN()
{
  ...
  ZB_INIT("esi_device_internal_th");
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch (sig)
    {
      ...
      case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ZB_SCHEDULE_APP_CALLBACK(esi_th_entry_point, param);
        param = 0;
        break;
...
enum esi_th_leave_machine_state_e
{
  ESI_TH_LEAVE_STATE_NONE,
  ESI_TH_LEAVE_STATE_INITED,
  ESI_TH_LEAVE_STATE_PLANNED_REJOIN,
  ESI_TH_LEAVE_STATE_LEAVE_REJOIN_SENT,
  ESI_TH_LEAVE_STATE_PLANNED_LEAVE,
  ESI_TH_LEAVE_STATE_LEAVE_AWAY_SENT,
};

static zb_uint8_t gs_esi_th_state = ESI_TH_LEAVE_STATE_NONE;


static void esi_th_kick_child_away(zb_uint8_t param,
                                        zb_uint8_t rejoin,
                                        zb_uint8_t dut_addr_ref)
{
  zb_zdo_mgmt_leave_param_t *req_param;
  zb_uint16_t nwk_addr;

  zb_address_short_by_ref(&nwk_addr, dut_addr_ref);
  ...
  req_param->rejoin = rejoin;
  zdo_mgmt_leave_req(param, NULL);
}

static void esi_th_rejoin_child(zb_uint8_t param, zb_uint16_t dut_addr_ref)
{
  esi_th_kick_child_away(param, 1, dut_addr_ref);
  gs_esi_th_state = ESI_TH_LEAVE_STATE_LEAVE_REJOIN_SENT;
}

static void esi_th_rejoin_child_caller(zb_uint8_t dut_addr_ref)
{
  ZB_GET_OUT_BUF_DELAYED2(esi_th_rejoin_child, dut_addr_ref);
}

static void esi_th_leave_child(zb_uint8_t param, zb_uint16_t dut_addr_ref)
{
  esi_th_kick_child_away(param, 0, dut_addr_ref);
  gs_esi_th_state = ESI_TH_LEAVE_STATE_LEAVE_AWAY_SENT;
}

static void esi_th_leave_child_caller(zb_uint8_t dut_addr_ref)
{
  ZB_GET_OUT_BUF_DELAYED2(esi_th_leave_child, dut_addr_ref);
}
...
static void esi_th_entry_point(zb_uint8_t param)
{
  zb_zdo_app_signal_hdr_t *sg_p = NULL;
  zb_zdo_app_signal_type_t sig = zb_get_app_signal(param, &sg_p);
  zb_ieee_addr_t dut_ieee;
  zb_address_ieee_ref_t dut_addr_ref;
  zb_zdo_signal_device_annce_params_t *dev_annce_params;

  /* Lil' state machine */
  if (gs_esi_th_state == ESI_TH_LEAVE_STATE_NONE)
  {
    gs_esi_th_state = ESI_TH_LEAVE_STATE_INITED;
  }

  if (gs_esi_th_state == ESI_TH_LEAVE_STATE_INITED &&
      sig == ZB_SE_TC_SIGNAL_CHILD_JOIN_CBKE)
  {
    ZB_IEEE_ADDR_COPY(dut_ieee, ZB_ZDO_SIGNAL_GET_PARAMS(sg_p, zb_ieee_addr_t));

    if (zb_address_by_ieee(dut_ieee, ZB_FALSE, ZB_FALSE, &dut_addr_ref) == RET_OK)
    {
      ZB_SCHEDULE_APP_ALARM(esi_th_rejoin_child_caller, dut_addr_ref, 60 * ZB_TIME_ONE_SECOND);

      gs_esi_th_state = ESI_TH_LEAVE_STATE_PLANNED_REJOIN;
    }
  }
  else if (gs_esi_th_state == ESI_TH_LEAVE_STATE_LEAVE_REJOIN_SENT &&
           sig == ZB_ZDO_SIGNAL_DEVICE_ANNCE)
  {
    dev_annce_params = ZB_ZDO_SIGNAL_GET_PARAMS(sg_p, zb_zdo_signal_device_annce_params_t);

    if (zb_address_by_short(dev_annce_params->device_short_addr,
                           ZB_FALSE, ZB_FALSE, &dut_addr_ref) == RET_OK)
    {
      ZB_SCHEDULE_APP_ALARM(esi_th_rejoin_child_caller, dut_addr_ref, 10 * ZB_TIME_ONE_SECOND);
      gs_esi_th_state = ESI_TH_LEAVE_STATE_PLANNED_REJOIN;

      //ZB_SCHEDULE_APP_ALARM(esi_th_leave_child_caller, dut_addr_ref, 60 * ZB_TIME_ONE_SECOND);
      //gs_esi_th_state = ESI_TH_LEAVE_STATE_PLANNED_LEAVE;
    }
  }
  else if (gs_esi_th_state == ESI_TH_LEAVE_STATE_LEAVE_AWAY_SENT &&
           sig == ZB_ZDO_SIGNAL_LEAVE_INDICATION)
  {
    gs_esi_th_state = ESI_TH_LEAVE_STATE_PLANNED_LEAVE;
  }
