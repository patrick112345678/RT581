...
#define ESI_BDB_DEV_ENDPOINT 2
#define ESI_DEV_CHANNEL_MASK (1L << 22) /* 22 channel. */

/* Test with subghz channels */
//#define ESI_SUBGHZ

/* For MM testing */
#if defined ZB_SUBGHZ_BAND_ENABLED && defined ESI_SUBGHZ
...
#define ESI_TIME_STATUS_INITIAL ((1 << ZB_ZCL_TIME_MASTER) | (1 << ZB_ZCL_TIME_MASTER_ZONE_DST) | (1 << ZB_ZCL_TIME_SUPERSEDING))
...
#define ESI_DEV_MEASURE_TIMEOUT ZB_TIME_ONE_SECOND
...
typedef struct esi_bdb_dev_ctx_s
{
    zb_zcl_basic_attrs_t basic_attrs;
...
/* device context */
static esi_bdb_dev_ctx_t g_bdb_dev_ctx;
...
ZB_ZCL_DECLARE_IDENTIFY_ATTR_LIST(bdb_identify_attr_list, g_bdb_dev_ctx.identify_attrs);

/* /\* Metering cluster attributes *\/ */
/* ZB_ZCL_DECLARE_METERING_ATTR_LIST(bdb_metering_attr_list, g_bdb_dev_ctx.metering_attrs); */

/* On/Off cluster attributes */
ZB_ZCL_DECLARE_ON_OFF_ATTR_LIST(bdb_on_off_attr_list, g_bdb_dev_ctx.on_off_attrs);
...
ZB_BDB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_CLUSTER_LIST(esi_bdb_dev_clusters,
        bdb_basic_attr_list,
        bdb_identify_attr_list,
        /* bdb_metering_attr_list, */
        bdb_on_off_attr_list
                                                        );

ZB_BDB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_EP(esi_bdb_dev_ep, ESI_BDB_DEV_ENDPOINT, esi_bdb_dev_clusters);

#ifdef ESI_START_IN_BDB_MODE
/* Start in BDB mode */
ZBOSS_DECLARE_DEVICE_CTX_1_EP(esi_dev_zcl_ctx, esi_bdb_dev_ep);
#elif (defined ZB_SE_BDB_MIXED)
/* Start in SE+BDB mode: accept both SE and BDB modes */
ZB_DECLARE_ENERGY_SERVICE_INTERFACE_DEV_CTX(esi_dev_zcl_ctx, esi_dev_ep, esi_bdb_dev_ep);
#else
/* Start in SE mode */
ZBOSS_DECLARE_DEVICE_CTX_1_EP(esi_dev_zcl_ctx, esi_dev_ep);
#endif
...
void esi_dev_clusters_attrs_init(zb_uint8_t param)
{
    ...
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_CALENDAR, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_PRICE, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_TIME, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_KEEP_ALIVE, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_DRLC, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_TUNNELING, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_MESSAGING, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_MDU_PAIRING, 1);
    zb_zcl_set_cluster_encryption(ESI_DEV_ENDPOINT, ZB_ZCL_CLUSTER_ID_METERING, 1);
...
static void esi_get_sched_prices_cb_on_sent(zb_uint8_t param)
{
    ...
    ZB_SCHEDULE_APP_ALARM(esi_get_sched_prices_cb_send_pp, 0, ESI_PRICE_ACK_TIMEOUT);
...
static void esi_get_sched_prices_cb_send_pp(zb_uint8_t param)
{
    ...
    if ((ESI_DEV_PRICE_CTX().state == ESI_DEV_GET_SCHED_PRICES_STATE_WAIT_PRICE_ACK) &&
            (!ESI_DEV_PRICE_CTX().price_ack_disabled))
    {
    ...
    ZB_ZCL_PRICE_SEND_CMD_PUBLISH_PRICE(param,
                                        ...
                                        ESI_DEV_ENDPOINT,
                                        &ESI_DEV_PRICE_CTX().pp_pl,
                                        (ESI_DEV_PRICE_CTX().price_ack_disabled) ? esi_get_sched_prices_cb_on_sent : NULL);
...
static void esi_get_sched_prices_cb(zb_uint8_t param)
{
    ...
    /* NOTE: use long address */
    ESI_DEV_PRICE_CTX().cli_addr.addr_short = ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).src_addr;
...
static void esi_price_ack_cb(zb_uint8_t param)
{
    ...
    if (ESI_DEV_PRICE_CTX().state == ESI_DEV_GET_SCHED_PRICES_STATE_WAIT_PRICE_ACK)
    {
        ...
        zb_uint8_t pp_buf = 0;
        ESI_DEV_PRICE_CTX().state = ESI_DEV_GET_SCHED_PRICES_STATE_FINI;
        (void)zb_schedule_alarm_cancel(esi_get_sched_prices_cb_send_pp, ZB_ALARM_ALL_CB, &pp_buf);
        esi_get_sched_prices_cb_send_pp(pp_buf);
    }
    ...
    zb_zcl_drlc_lce_payload_t *out)
{
    ...
    out->device_class = ZB_ZCL_DRLC_DEVICE_CLASS_SMART_APPLIANCE;
...
void esi_tunneling_tx_resp(zb_uint8_t param, zb_uint16_t user_param)
{
    ...
    if (ZB_ZCL_TUNNELING_SERVER_SEND_TRANSFER_DATA(param,
            ESI_DEV_ENDPOINT,
            get_profile_id_by_endpoint(ESI_DEV_ENDPOINT),
...
void esi_send_publish_week_profile(zb_uint8_t param)
{
    ...
    {
        zb_zcl_calendar_publish_week_profile_payload_t payload; /* or preinit with = ZB_ZCL_ENERGY_MANAGEMENT_REPORT_EVENT_STATUS_PAYLOAD_INIT; */
...
void esi_send_cancel_calendar(zb_uint8_t param)
{
    ...
    {
        zb_zcl_calendar_cancel_calendar_payload_t payload; /* or preinit with = ZB_ZCL_CALENDAR_CANCEL_CALENDAR_INIT; */
        /*Here we must get events if present and pass them to client*/
...
zb_bool_t esi_dev_set_real_time_clock(zb_uint32_t time)
{
...
/** Init default reporting configuration of ZCL attributes (Metering - Current Summation Delivered
    and Status).
*/
void esi_dev_default_reporting_init(zb_uint8_t ep)
{
    zb_zcl_reporting_info_t rep_info;

    TRACE_MSG(TRACE_APP1, ">> esi_dev_default_reporting_init", (FMT__0));
    ...
    rep_info.dst.profile_id = get_profile_id_by_endpoint(ep);

    rep_info.u.send_info.min_interval = ZB_SE_METERING_DATA_UPDATE_RATE;
    rep_info.u.send_info.max_interval = ZB_ZCL_MAX_REPORTING_INTERVAL;
    rep_info.u.send_info.def_min_interval = ZB_SE_METERING_DATA_UPDATE_RATE;
    rep_info.u.send_info.def_max_interval = ZB_ZCL_MAX_REPORTING_INTERVAL;
...
static void esi_dev_measure(zb_uint8_t param)
{
...
#ifndef ESI_START_IN_BDB_MODE
    ZB_ZCL_SET_ATTRIBUTE(ESI_DEV_ENDPOINT,
    ...
    /* ZB_ZCL_SET_ATTRIBUTE(ESI_BDB_DEV_ENDPOINT, */
    /*                      ZB_ZCL_CLUSTER_ID_METERING, */
    /*                      ZB_ZCL_CLUSTER_SERVER_ROLE, */
    /*                      ZB_ZCL_ATTR_METERING_CURRENT_SUMMATION_DELIVERED_ID, */
    /*                      (zb_uint8_t *)&new_curr_summ_delivered, */
    ...
    TRACE_MSG(TRACE_APP1, "metering status: 0x%x",
              (FMT__H, new_metering_status));
    ...
    ZB_SCHEDULE_APP_ALARM(esi_dev_measure, 0, ESI_DEV_MEASURE_TIMEOUT);
}
...
void esi_dev_app_init(zb_uint8_t param)
{
    ...
    /* Define callback for set real time clock */
    ZB_ZCL_TIME_SET_REAL_TIME_CLOCK_CB(esi_dev_set_real_time_clock);
    ...
    /** [ESI_DEV_SET_ROLE] */
#ifdef ESI_START_IN_BDB_MODE
    zb_set_network_coordinator_role(ESI_DEV_CHANNEL_MASK);
#else
...
static void esi_dev_left_button_handler(zb_uint8_t param)
{
    ...
    /* ZB_SE_BDB_MIXED: f&b target will be started from ZB_SE_TC_SIGNAL_CHILD_JOIN_NON_CBKE */
}
/** [PERMIT_JOINING] */
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
...
#ifndef ESI_START_IN_BDB_MODE
            esi_dev_default_reporting_init(ESI_DEV_ENDPOINT);
#endif
#ifdef ZB_SE_BDB_MIXED
            /* esi_dev_default_reporting_init(ESI_BDB_DEV_ENDPOINT); */
#endif
...
#endif
#if (defined ZB_SE_BDB_MIXED && defined ESI_START_IN_BDB_MODE)
            bdb_start_top_level_commissioning(ZB_BDB_NETWORK_STEERING);
#endif
            /* Start measuring */
            ZB_SCHEDULE_APP_ALARM_CANCEL(esi_dev_measure, ZB_ALARM_ANY_PARAM);
            ZB_SCHEDULE_APP_CALLBACK(esi_dev_measure, 0);
            break;

#if (defined ZB_SE_BDB_MIXED && defined ESI_START_IN_BDB_MODE)
        case ZB_BDB_SIGNAL_STEERING:
            TRACE_MSG(TRACE_APP1, "Successfull steering, start f&b target", (FMT__0));
            zb_bdb_finding_binding_target(ESI_BDB_DEV_ENDPOINT);
            break;
#endif
        ...
        case ZB_SE_TC_SIGNAL_CHILD_JOIN_NON_CBKE:
        {
            ...
            TRACE_MSG(TRACE_APP1, "Child " TRACE_FORMAT_64 " joined & established non-CBKE TCLK",
                      (FMT__A, TRACE_ARG_64(remote_device_addr)));
#if (defined ZB_SE_BDB_MIXED /* && defined ESI_START_IN_BDB_MODE */)
            /* Start F&B target for this device */
            zb_bdb_finding_binding_target(ESI_BDB_DEV_ENDPOINT);
#endif
        }
        break;
