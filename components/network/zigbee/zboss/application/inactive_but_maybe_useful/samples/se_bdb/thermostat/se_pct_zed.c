...
void pct_dev_nvram_read_app_data(zb_uint8_t page, zb_uint32_t pos, zb_uint16_t payload_length);
zb_ret_t pct_dev_nvram_write_app_data(zb_uint8_t page, zb_uint32_t pos);
...
/** [handle_manage_event] */
/* Handle received Energy Management Cluster's Mange Event command
...
/** [PCT_DEV_INIT] */
  ...
  zb_register_zboss_callback(ZB_ZCL_DEVICE_CB, SET_ZBOSS_CB(pct_zcl_cmd_device_cb));
...
/*********************  SE Programmable Controlled Thermostat ZED  **************************/
...
static void pct_dev_left_button_handler(zb_uint8_t param)
{
  if (!param)
  {
    ...
    ZB_GET_OUT_BUF_DELAYED(pct_dev_left_button_handler);
  }
  else
  {
