 ...
 * DSR.
 */
/* PURPOSE: Gas metering ZR device sample
*/

#define ZB_TRACE_FILE_ID 40181
...
static void metering_dev_show_attr(zb_uint16_t attr_id)
{
    ...
    case ZB_ZCL_ATTR_METERING_STATUS_ID:
        ...
        TRACE_MSG(TRACE_APP1, "   - check meter: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_CHECK_METER));
        TRACE_MSG(TRACE_APP1, "   - low battery: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_LOW_BATTERY));
        TRACE_MSG(TRACE_APP1, "   - tamper detect: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_TAMPER_DETECT));
        TRACE_MSG(TRACE_APP1, "   - low pressure: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_LOW_PRESSURE));
        TRACE_MSG(TRACE_APP1, "   - leak detect: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_LEAK_DETECT));
        TRACE_MSG(TRACE_APP1, "   - service disconnect: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_SERVICE_DISCONNECT));
        TRACE_MSG(TRACE_APP1, "   - reverse flow: %d",
                  (FMT__D, g_dev_ctx.metering_attrs.status & ZB_ZCL_METERING_GAS_REVERSE_FLOW));
        break;
...
void metering_dev_clusters_attrs_init(zb_uint8_t param)
{
    ...
    g_dev_ctx.metering_attrs.unit_of_measure = ZB_ZCL_METERING_UNIT_OF_MEASURE_M3_M3H_BINARY;
    g_dev_ctx.metering_attrs.summation_formatting = METERING_DEV_SUMMATION_FORMATTING;
    g_dev_ctx.metering_attrs.device_type = ZB_ZCL_METERING_DEVICE_TYPE_GAS_METERING;
...
void metering_dev_app_init(zb_uint8_t param)
{
    ...
    zb_se_set_network_router_role(METERING_DEV_CHANNEL_MASK);
...
static void metering_dev_main(zb_uint8_t param)
{
    ...
    case 5:
    case 8:
        g_dev_ctx.metering_attrs.status = ZB_ZCL_METERING_GAS_LOW_PRESSURE;
        break;
    ...
    TRACE_MSG(TRACE_APP1, "instantaneous gas consumption: 0.%llu %s",
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_ZDO_SIGNAL_SKIP_STARTUP:
            ...
            zb_se_load_ecc_cert(KEC_CS1, ca_public_key, gas_metering_certificate, gas_metering_private_key);
#endif
#ifdef SE_CRYPTOSUITE_2
            zb_se_load_ecc_cert(KEC_CS2, ca_283_public_key, gas_metering_cert_283, gas_metering_private_283);
#endif
        ...
        case ZB_SE_SIGNAL_CBKE_OK:
            TRACE_MSG(TRACE_APP1, "CBKE OK", (FMT__0));
            break;
