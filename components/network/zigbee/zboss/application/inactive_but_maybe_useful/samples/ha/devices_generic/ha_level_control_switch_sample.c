 ...
 * DSR.
 */
/* PURPOSE: Level control switch for HA profile
*/

#define ZB_TRACE_FILE_ID 40158
#include "zb_common.h"
...
#define HA_LEVEL_CONTROL_SWITCH_ENDPOINT          10
...
ZB_HA_DECLARE_LEVEL_CONTROL_SWITCH_CLUSTER_LIST(level_control_switch_clusters,
                                                basic_attr_list,
                                                identify_attr_list);

ZB_HA_DECLARE_LEVEL_CONTROL_SWITCH_EP(level_control_switch_ep,
                                      HA_LEVEL_CONTROL_SWITCH_ENDPOINT,
                                      level_control_switch_clusters);

ZB_HA_DECLARE_LEVEL_CONTROL_SWITCH_CTX(level_control_switch_ctx, level_control_switch_ep);
/** [COMMON_DECLARATION] */
...
MAIN()
{
  ...
  ZB_INIT("ha_level_control_switch_sample");
  ...
  /****************** Register Device ********************************/
  /** [REGISTER] */
  ZB_AF_REGISTER_DEVICE_CTX(&level_control_switch_ctx);
  ZB_AF_SET_ENDPOINT_HANDLER(HA_LEVEL_CONTROL_SWITCH_ENDPOINT, zcl_specific_cluster_cmd_handler);
  /** [REGISTER] */
...
zb_uint8_t zcl_specific_cluster_cmd_handler(zb_uint8_t param)
{
  ...
  /* Oct 16, 2012 CR:MAJOR You're leaking for a buffer for each processed command without output
   * (e. g. Default response for Level Control cluster).
   */
