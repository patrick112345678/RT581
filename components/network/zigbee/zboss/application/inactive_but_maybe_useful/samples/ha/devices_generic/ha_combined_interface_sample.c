 ...
 * DSR.
 */
/* PURPOSE: Combined interface device sample for HA profile
*/

#define ZB_TRACE_FILE_ID 40166

/* 2018/08/10 CR:MAJOR (rev. 39308): Remove all private header includes. One include
       #include "zboss_api.h"
   should be enough. */
...
/* 2018/08/10 CR:MAJOR (rev. 39308): It is wrong way of device definition, remove it. Device should be enabled
 * in zb_ha_device_config.h (ZB_ALL_DEVICE_SUPPORT) or in vendor includes. */
#define ZB_HA_DEFINE_DEVICE_COMBINED_INTERFACE
#include "zb_ha.h"

/* 2018/08/10 CR:MINOR (rev. 39308): Here and in other samples: suggest to remove this "error-check" define. */
#if ! defined ZB_ED_ROLE
#error define ZB_ED_ROLE to compile ze tests
#endif

/* 2018/08/10 CR:MINOR (rev. 39308): Where it is possible, avoid different endpoint numbers (12, 10, 9, 14, 21
 * etc). Just use 1 - it is easier.
 * For test purposes it may be also possible to bypass endpoint discovery (to simplify the code -
 * not sending a set of Match Descriptor/Active Endpoint/Simple Descriptor) requests. */
#define HA_COMBINED_INTERFACE_ENDPOINT          12
...
zb_uint8_t g_switch_type = ZB_ZCL_ON_OFF_SWITCH_CONFIGURATION_SWITCH_TYPE_TOGGLE;
zb_uint8_t g_switch_actions = ZB_ZCL_ON_OFF_SWITCH_CONFIGURATION_SWITCH_ACTIONS_TYPE1;
...
MAIN()
{
  ...
  ZB_INIT("ha_combined_interface_sample");
...
zb_uint8_t zcl_specific_cluster_cmd_handler(zb_uint8_t param)
{
  ...
  {
    /* TODO: handle commands with direction "to client", if needed*/
