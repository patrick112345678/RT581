...
zb_ieee_addr_t g_ed_addr = LIGHT_CONTROL_IEEE_ADDR;
...
static void trace_apsde_data_ind(zb_apsde_data_indication_t *data_ind)
{
  if (data_ind)
  {
    TRACE_MSG(TRACE_APP1, "APSDE data indication fileds", (FMT__0));
    TRACE_MSG(TRACE_APP1, "fc:           0x%hx", (FMT__H, data_ind->fc));
    TRACE_MSG(TRACE_APP1, "src_addr:     0x%x",  (FMT__D, data_ind->src_addr));
    TRACE_MSG(TRACE_APP1, "dst_addr:     0x%x",  (FMT__D, data_ind->dst_addr));
    TRACE_MSG(TRACE_APP1, "group_addr:   0x%x",  (FMT__D, data_ind->group_addr));
    TRACE_MSG(TRACE_APP1, "dst_endpoint: %hd",   (FMT__H, data_ind->dst_endpoint));
    TRACE_MSG(TRACE_APP1, "src_endpoint: %hd",   (FMT__H, data_ind->src_endpoint));
    TRACE_MSG(TRACE_APP1, "clusterid:    0x%x",  (FMT__D, data_ind->clusterid));
    TRACE_MSG(TRACE_APP1, "profileid:    0x%x",  (FMT__D, data_ind->profileid));
    TRACE_MSG(TRACE_APP1, "aps_counter:  %hd",   (FMT__H, data_ind->aps_counter));
    TRACE_MSG(TRACE_APP1, "mac_src_addr: 0x%x",  (FMT__D, data_ind->mac_src_addr));
    TRACE_MSG(TRACE_APP1, "mac_dst_addr: 0x%x",  (FMT__D, data_ind->mac_dst_addr));
    TRACE_MSG(TRACE_APP1, "tsn:          %hd",   (FMT__H, data_ind->tsn));
  }
  else
  {
    TRACE_MSG(TRACE_ERROR, "APSDE data indication is invalid!", (FMT__0));
  }
}
...
static zb_uint8_t data_indication(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, ">> data_indication, param %hd", (FMT__H, param));

  if (0x1111 == data_ind->profileid
      && 0x2222 == data_ind->clusterid)
  {
    TRACE_MSG(TRACE_APP1, "zb_buf_len: %hd, aps_payload_size: %hd",
              (FMT__H_H, ZB_BUF_LEN(buf), aps_payload_size));

    for (zb_uint8_t i = 0; i < aps_payload_size; ++i)
    {
      TRACE_MSG(TRACE_APP1, "aps_payload[%hd] == 0x%hx",
                (FMT__H_H, i, aps_payload_ptr[i]));
    }

    trace_apsde_data_ind(data_ind);

    ZB_FREE_BUF(buf);
    processed = ZB_TRUE;
  }

  TRACE_MSG(TRACE_APP1, "<< data_indication", (FMT__0));
...
void find_light_bulb_alarm(zb_uint8_t param)
{
  ...
  if (param == 0)
  {
    ZB_GET_OUT_BUF_DELAYED(find_light_bulb_alarm);
  }
  else
  {
...
void bulb_ieee_addr_req(zb_uint8_t param)
{
  ...
  zb_zdo_ieee_addr_req(ZB_REF_FROM_BUF(buf), bulb_ieee_addr_req_cb);
}
...
void light_control_leave_nwk(zb_uint8_t param)
{
  ...
  if (!param)
  {
    ZB_GET_OUT_BUF_DELAYED(light_control_leave_nwk);
  }
  else
  {
    zb_buf_t *buf = ZB_BUF_FROM_REF(param);
...
{
  ZB_GET_OUT_BUF_DELAYED(light_control_send_on_off_alarm);
}

void light_control_send_on_off_cb(zb_uint8_t param)
{
  zb_zcl_command_send_status_t *cmd_send_status = ZB_GET_BUF_PARAM(ZB_BUF_FROM_REF(param), zb_zcl_command_send_status_t);
...
{
  ZB_GET_OUT_BUF_DELAYED2(light_control_send_on_off, on_off);
}
...
{
  ZB_GET_OUT_BUF_DELAYED2(light_control_send_step, dir);
}
