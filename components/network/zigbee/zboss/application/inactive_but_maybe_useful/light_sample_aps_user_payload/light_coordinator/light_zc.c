...
static void send_aps_payload_delayed(zb_uint8_t param, zb_uint16_t param2);
static void send_aps_payload(zb_uint8_t param);
static void send_aps_payload_cb(zb_uint8_t param);
...
MAIN()
{
    ...
    zb_set_nvram_erase_at_start(light_zc_hal_is_button_pressed(BULB_BUTTON_2_IDX));
#else
    /*
...
static void device_annce_handler(zb_uint8_t buf_ref)
{
    ...
    ZB_GET_OUT_BUF_DELAYED2(send_aps_payload_delayed, 10);
...
static void send_aps_payload_delayed(zb_uint8_t param, zb_uint16_t param2)
{
    ZB_SCHEDULE_APP_ALARM(send_aps_payload, param, param2 * ZB_TIME_ONE_SECOND);
}
...
static void send_aps_payload(zb_uint8_t param)
{
    ...
    zb_addr_u addr_u = { 0 };
    zb_aps_addr_mode_t addr_mode = ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
    static zb_bool_t aps_ack_is_enabled = ZB_TRUE;

#define MAX_PAYLOAD_SIZE 82
    static zb_uint8_t payload_size = 79;
    zb_uint8_t payload_ptr[MAX_PAYLOAD_SIZE] = { 0 };

    TRACE_MSG(TRACE_APP1, ">> send_aps_payload, param: %hd", (FMT__H, param));

    if (ZB_APS_ADDR_MODE_64_ENDP_PRESENT == addr_mode)
    {
        ZB_IEEE_ADDR_COPY(addr_u.addr_long, zc_ctx.devices[0].ieee_addr);
    }
    else
    {
        addr_u.addr_short = zb_address_short_by_ieee(zc_ctx.devices[0].ieee_addr);
    }
    ...
    for (zb_uint8_t i = 0; i < payload_size; ++i)
    {
        payload_ptr[i] = i;
    }

    aps_ack_is_enabled = (zb_bool_t)!aps_ack_is_enabled;
...
static void send_aps_payload_cb(zb_uint8_t param)
{
    ...
    ZB_GET_OUT_BUF_DELAYED2(send_aps_payload_delayed, 5);
    ...
    if (param)
    {
        ...
        case ZB_APS_USER_PAYLOAD_CB_STATUS_NO_MAC_ACK:
            TRACE_MSG(TRACE_APP1, "Transmission status: NO_MAC_ACK", (FMT__0));
            break;
