 ...
 * DSR.
 */
/* PURPOSE: Light control sample
*/

#define ZB_TRACE_FILE_ID 40013

#include "light_control_HA_1_2_bulb.h"
#include "zll/zb_zll_dimmer_switch.h"
#ifdef ZB_USE_BUTTONS
#include "light_control_HA_1_2_bulb_hal.h"
#endif

#define ZB_ZLL_DEFINE_DEVICE_DIMMER_SWITCH
...
zb_uint16_t g_profile_id = ZB_AF_ZLL_PROFILE_ID;
/******************* Declare attributes ************************/
...
/********************* Declare device **************************/
ZB_ZLL_DECLARE_DIMMER_SWITCH_CLUSTER_LIST(dimmer_switch_clusters,
        basic_attr_list,
        identify_attr_list);

ZB_ZLL_DECLARE_DIMMER_SWITCH_EP(dimmer_switch_ep,
                                LIGHT_CONTROL_ENDPOINT,
                                dimmer_switch_clusters);

ZB_ZLL_DECLARE_DIMMER_SWITCH_CTX(dimmer_switch_ctx, dimmer_switch_ep);
...
void bulb_ieee_addr_req_cb(zb_uint8_t param)
{
    ...
    if (resp->status == ZB_ZDP_STATUS_SUCCESS)
    {
        ...
        ZB_SCHEDULE_APP_CALLBACK(light_control_bind_bulb_on_off, param);
    }
    else
    {
...
void light_control_bind_bulb_on_off(zb_uint8_t param)
{
    zb_bufid_t buf = param;
...
void light_control_send_on_off(zb_uint8_t param, zb_uint16_t on_off)
{
    ...
    ZB_ZCL_ON_OFF_SEND_REQ(
        ...
        0,
        LIGHT_CONTROL_ENDPOINT,
        ZB_AF_HA_PROFILE_ID,/*Some bulbs understands only commands with HA profile despite it requires ZLL profile in endpoint*/
        ZB_FALSE,
        cmd_id,
#ifndef ZB_USE_BUTTONS
