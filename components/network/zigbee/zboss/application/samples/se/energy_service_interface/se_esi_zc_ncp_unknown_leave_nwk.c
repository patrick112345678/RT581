...
#define ESI_DEV_CHANNEL_MASK (1L << 19) /* 19 channel. */

/* For MM testing */
#if defined ZB_SUBGHZ_BAND_ENABLED
#define ESI_DEV_CHANNEL_PAGE ZB_CHANNEL_PAGE28_SUB_GHZ
#define ESI_DEV_CHANNEL_MASK1 (1L << 19) /* 1 channel page 28. */
#else
...
#define IHD_DEV_ADDR_INTERNAL          {0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb}
#define EL_METERING_DEV_ADDR_INTERNAL  {0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11}
#define GAS_METERING_DEV_ADDR_INTERNAL {0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x22}
#define PCT_DEV_ADDR_INTERNAL          {0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33}
#define NWK_LEAVE_UNKNOWN_ADDR         {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}
...
zb_ieee_addr_t ihd_dev_addr_internal = IHD_DEV_ADDR_INTERNAL;
zb_ieee_addr_t el_metering_dev_addr_internal = EL_METERING_DEV_ADDR_INTERNAL;
zb_ieee_addr_t gas_metering_dev_addr_internal = GAS_METERING_DEV_ADDR_INTERNAL;
zb_ieee_addr_t pct_dev_addr_internal = PCT_DEV_ADDR_INTERNAL;

static zb_uint16_t zr_nwk_addr;

static zb_ieee_addr_t leave_ieee_addr;
...
static void esi_get_sched_prices_cb_send_pp(zb_uint8_t param)
{
    if (!param)
    {
        zb_buf_get_out_delayed(esi_get_sched_prices_cb_send_pp);
        return;
    }
...
void esi_send_publish_calendar(zb_uint8_t param)
{
    ...
    if (!param)
    {
        zb_buf_get_out_delayed(esi_send_publish_calendar);
    }
    else
    {
...
void esi_send_publish_week_profile(zb_uint8_t param)
{
    ...
    if (!param)
    {
        zb_buf_get_out_delayed(esi_send_publish_week_profile);
    }
    else
    {
...
void esi_send_publish_day_profile(zb_uint8_t param)
{
    ...
    if (!param)
    {
        zb_buf_get_out_delayed(esi_send_publish_day_profile);
    }
    else
    {
...
void esi_send_cancel_calendar(zb_uint8_t param)
{
    ...
    if (!param)
    {
        zb_buf_get_out_delayed(esi_send_cancel_calendar);
    }
    else
    {
...
static void esi_zcl_cmd_device_cb(zb_uint8_t param)
{
    ...
    case ZB_ZCL_TUNNELING_TRANSFER_DATA_CLI_CB_ID:
    {
        ...
        zb_buf_get_out_delayed_ext(esi_tunneling_tx_resp, tr_data->hdr.tunnel_id, ZB_UNUSED_PARAM);
        ZB_ZCL_DEVICE_CMD_PARAM_STATUS(param) = RET_OK;
    }
    break;
...
MAIN()
{
    zb_ieee_addr_t nwk_leave_unknown_ieee_addr = NWK_LEAVE_UNKNOWN_ADDR;
    ...
    ZB_CERT_HACKS().enable_leave_to_router_hack = ZB_TRUE;

    /* Send nwk leave from an unknown address */
    ZB_IEEE_ADDR_COPY(ZB_CERT_HACKS().nwk_leave_from_unknown_ieee_addr, &nwk_leave_unknown_ieee_addr);
    ZB_CERT_HACKS().nwk_leave_from_unknown_short_addr = 0x1234;
    ZB_CERT_HACKS().nwk_leave_from_unknown_addr = ZB_TRUE;
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    static zb_bool_t is_first_cbke = ZB_TRUE;
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        {
            ...
            zr_nwk_addr = dev_annce_params->device_short_addr;
            ZB_IEEE_ADDR_COPY(leave_ieee_addr, dev_annce_params->ieee_addr);
            zb_buf_get_out_delayed_ext(se_send_key_est_match_desc_addr, zr_nwk_addr, 0);
            param = 0;
            if (is_first_cbke)
            {
                TRACE_MSG(TRACE_APP1, "zboss_signal_handler: set alarm for 1st leave", (FMT__0));
                ZB_SCHEDULE_APP_ALARM(zb_leave_req, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(10000));
                is_first_cbke = ZB_FALSE;
            }
            break;
        }
        case ZB_ZDO_SIGNAL_DEVICE_AUTHORIZED:
        {
            TRACE_MSG(TRACE_APP1, "zboss_signal_handler: authorized received, sending 2nd leave", (FMT__0));
            ZB_SCHEDULE_APP_ALARM(zb_leave_req, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(5000));
            break;
        }
        ...
        case ZB_ZDO_SIGNAL_SKIP_STARTUP:
            ...
            zb_secur_ic_str_add(ihd_dev_addr_internal, ncp_installcode, NULL);
            zb_secur_ic_str_add(el_metering_dev_addr_internal, ncp_installcode, NULL);
            zb_secur_ic_str_add(gas_metering_dev_addr_internal, ncp_installcode, NULL);
            zb_secur_ic_str_add(pct_dev_addr_internal, ncp_installcode, NULL);
...
void zb_nwk_leave_req(zb_bufid_t buf)
{
    ...
    static zb_uint8_t leave_cnt = 0;

    TRACE_MSG(TRACE_APP1, "requesting nlme_leave_req", (FMT__0));

    /* consider the overflow scenario of this variable,
       for now it is ok since it is used in tests up to 60 seconds */
    leave_cnt++;

    if (leave_cnt >= 2)
    {
        TRACE_MSG(TRACE_APP1, "schedule 3th leave, from parent without rejoin", (FMT__0));
        ZB_SCHEDULE_APP_ALARM(zb_leave_req, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(5000));
    }

    lr = ZB_BUF_GET_PARAM(buf, zb_nlme_leave_request_t);
    ZB_IEEE_ADDR_COPY(lr->device_address, leave_ieee_addr);
    ...
    if (leave_cnt > 1)
    {
        ZB_CERT_HACKS().nwk_leave_from_unknown_addr = ZB_FALSE;
        lr->rejoin = ZB_FALSE;
    }
