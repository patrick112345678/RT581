...
/* Enables usage of Price cluster */
...
/* Default values for tunneling cluster */
...
/* Structure for Price cluster context */
 ...
 *  Application may ignore callback IDs in which it is not interested. */
static void esi_zcl_cmd_device_cb(zb_uint8_t param)
{
  ...
  switch (ZB_ZCL_DEVICE_CMD_PARAM_CB_ID(param))
  {
    ...
    case ZB_ZCL_TUNNELING_REQUEST_TUNNEL_CB_ID:
    {
      ...
      zb_zcl_tunnel_request_params_out_t *tunnel_params = ZB_ZCL_DEVICE_CMD_PARAM_OUT_GET(param, zb_zcl_tunnel_request_params_out_t);
      ...
      {
        tunnel_params->tunnel_status = ZB_ZCL_TUNNELING_STATUS_PROTOCOL_NOT_SUPPORTED;
      }
      else if (req->flow_control_support != ZB_FALSE)
      {
        tunnel_params->tunnel_status = ZB_ZCL_TUNNELING_STATUS_FLOW_CONTROL_NOT_SUPPORTED;
      }
      else
      {
        tunnel_params->tunnel_status = ZB_ZCL_TUNNELING_STATUS_SUCCESS;
        tunnel_params->max_incoming_to_srv_transfer_size = req->max_incoming_transfer_size;
      }
      ZB_ZCL_DEVICE_CMD_PARAM_STATUS(param) = RET_OK;
    }
    break;
    case ZB_ZCL_TUNNELING_TRANSFER_DATA_CLI_CB_ID:
    {
      ...
      zb_buf_get_out_delayed_ext(esi_tunneling_tx_resp, tr_data->hdr.tunnel_id, 0);
      ZB_ZCL_DEVICE_CMD_PARAM_STATUS(param) = RET_OK;
    }
    break;
...
#if 0
  /* TODO: the mechanism was changed. Rewrite the sample. */
  /* Define callback for set real time clock */
  ZB_ZCL_TIME_SET_REAL_TIME_CLOCK_CB(esi_dev_set_real_time_clock);
#endif

/* ENABLE_RUNTIME_APP_CONFIG and ENABLE_PRECOMMISSIONED_REJOIN are enabled only
 * in build without ZB_PRODUCTION_CONFIG */
#ifdef ENABLE_RUNTIME_APP_CONFIG
...
MAIN()
{
  ...
  /* Application init:
   * configure stack parameters (IEEE address, device role, channel mask etc),
   * register device context and ZCL commands device callback),
   * init application attributes/context. */
  esi_dev_app_init(0);
...
static void esi_permit_join_after_dev_annce(zb_uint8_t param)
{
  zb_se_permit_joining(ESI_PERMIT_JOINING_DURATION_TIME);
...
static void esi_schedule_service_discovery(zb_uint8_t param)
{
  ZVUNUSED(param);
