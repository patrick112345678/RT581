...
#define METERING_DEV_CHANNEL_MASK2 (1L << 19)
...
/* Inits device context */
void metering_dev_ctx_init();
/* Inits device ZCL attributes */
void metering_dev_clusters_attrs_init(zb_uint8_t param);
/* Inits default reporting configuration of ZCL attributes (Metering - Current Summation Delivered
 *  and Status) */
...
/* Generates and accumulates measurements - emulates real device */
...
/* Declare SE cluster list for Metering Load Control device */
ZB_SE_DECLARE_MLC_PHYSICAL_DEVICE_CLUSTER_LIST(physical_dev_clusters,
                                               basic_attr_list,
                                               kec_attr_list);

/* Declare first endpoint for Metering Load Control device */
ZB_SE_DECLARE_MLC_PHYSICAL_DEVICE_EP(physical_dev_ep, MLC_PHYSICAL_DEV_ENDPOINT,
                                     physical_dev_clusters);

/* Declare second SE cluster list for Metering Load Control device */
ZB_SE_DECLARE_MLC_METERING_DEVICE_CLUSTER_LIST(metering_dev_clusters,
                                               metering_attr_list);

/* Declare second endpoint for Metering Load Control device */
ZB_SE_DECLARE_MLC_METERING_DEVICE_EP(metering_dev_ep, MLC_METERING_DEV_ENDPOINT,
                                     metering_dev_clusters);

/* Declare SE third cluster list for the first endpoint */
ZB_SE_DECLARE_MLC_LOAD_CONTROL_DEVICE_CLUSTER_LIST(load_control_dev_clusters,
                                                   drlc_attr_list);

/* Declare third endpoint for Metering Load Control device */
ZB_SE_DECLARE_MLC_LOAD_CONTROL_DEVICE_EP(load_control_dev_ep, MLC_LOAD_CONTROL_DEV_ENDPOINT,
                                         load_control_dev_clusters);

/* Declare application's device context for the three-endpoint device */
ZBOSS_DECLARE_DEVICE_CTX_3_EP(metering_dev_zcl_ctx, physical_dev_ep, metering_dev_ep, load_control_dev_ep);

/** [COMMON_DECLARATION] */


/* Prints Metering attributes to the trace by an attribute ID */
...
/* Saves Metering device's data into NVRAM. */
...
/* Reads Metering device's stored data from NVRAM. */
...
/* The application callback to get size of application-specific NVRAM dataset */
...
static void gas_zcl_cmd_device_cb(zb_uint8_t param)
{
  ...
  switch (ZB_ZCL_DEVICE_CMD_PARAM_CB_ID(param))
  {
    case ZB_ZCL_DRLC_LOAD_CONTROL_EVENT_CB_ID:
    {
      ...
      /* TODO: write device handler for received command */
...
void metering_dev_app_init(zb_uint8_t param)
{
  ...
  ZB_ZCL_REGISTER_DEVICE_CB(gas_zcl_cmd_device_cb);
...
/* Set up long address of the device if production configuration is disabled */
#ifdef ENABLE_RUNTIME_APP_CONFIG
  zb_set_long_address(g_dev_addr);
#endif

/* Set up device's role in a network, define used channel  */
...
static void metering_dev_left_button_handler(zb_uint8_t param)
{
  if (!param)
  {
    ...
    zb_buf_get_out_delayed(metering_dev_left_button_handler);
  }
  else
  {
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch (sig)
    {
      ...
      case ZB_ZDO_SIGNAL_SKIP_STARTUP:
        ...
        /* Init default reporting info after NVRAM has been loaded */
