...
static void metering_zcl_cmd_device_cb(zb_uint8_t param)
{
    ...
    case ZB_ZCL_TUNNELING_REQUEST_TUNNEL_RESPONSE_CB_ID:
    {
        ...
        if (resp->tunnel_status == ZB_ZCL_TUNNELING_TUNNEL_STATUS_SUCCESS)
        {
            /* Send tunneling data */
...
#ifdef TEST_DUTY_C
static void crash_poll_keepalive(zb_uint8_t param)
{
    if (!param)
    {
        ZB_GET_OUT_BUF_DELAYED(crash_poll_keepalive);
    }
    /* If suspended, stop sending. */
    else
    {
        if (!s_tx_suspended)
        {
        ...
        }
        {
            ...
            n++;
            ZB_SCHEDULE_APP_ALARM(crash_poll_keepalive, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(n == 50 ? 31000 : 500));
        }
    }
}
#endif

#ifdef TEST_SUBG_FILTERING
/* That calls are here to test that sub-ghz cluster is absent when operating at 2.4 band. */

void call_simple_desc_req(zb_uint8_t param)
{
    zb_zdo_simple_desc_req_t *req;

    ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(param), sizeof(zb_zdo_simple_desc_req_t), req);
...
void call_match_desc_req(zb_uint8_t param)
{
    zb_zdo_match_desc_param_t *req;

    ZB_BUF_INITIAL_ALLOC(ZB_BUF_FROM_REF(param), sizeof(zb_zdo_match_desc_param_t) + (1) * sizeof(zb_uint16_t), req);
    ...
    zb_zdo_match_desc_req(param, NULL);
}
#endif
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_SE_SIGNAL_SERVICE_DISCOVERY_OK:
            ...
            zb_se_indicate_commissioning_stopped();
#ifdef TEST_DUTY_C
            //        zb_start_high_freq_msgs(ZB_ZCL_CLUSTER_ID_KEEP_ALIVE);
            ZB_SCHEDULE_APP_ALARM(crash_poll_keepalive, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(1000));
#endif
#ifdef TEST_SUBG_FILTERING
            ZB_GET_OUT_BUF_DELAYED(call_simple_desc_req);
            ZB_GET_OUT_BUF_DELAYED(call_match_desc_req);
#endif
            break;
