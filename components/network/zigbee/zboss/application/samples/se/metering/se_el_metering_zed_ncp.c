...
/*Adress for KE whitelist and test partner lk*/

#define IHD_DEV_ADDR {0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb}
#define MET_DEV_ADDR {0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11}

#define NCP_TEST_KEC_ENDPOINT 20
...
static void unbind_kec_cluster(zb_uint8_t param);
...
static void metering_zcl_cmd_device_cb(zb_uint8_t param)
{
    ...
    case ZB_ZCL_TUNNELING_REQUEST_TUNNEL_RESPONSE_CB_ID:
    {
        ...
        if (resp->tunnel_status == ZB_ZCL_TUNNELING_STATUS_SUCCESS)
        {
            ...
            zb_buf_get_out_delayed(metering_tunneling_tx);
        }
        else
        {
    ...
    case ZB_ZCL_TUNNELING_TRANSFER_DATA_SRV_CB_ID:
    {
        ...
        zb_buf_get_out_delayed(metering_tunneling_close_tunnel);
...
void metering_dev_app_init(zb_uint8_t param)
{
    ...
    ZB_ZCL_REGISTER_DEVICE_CB(metering_zcl_cmd_device_cb);
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_ZDO_SIGNAL_SKIP_STARTUP:
            ...
            {
                /* Add ieee address of non TC devices authorized to request CBKE*/
                zb_ieee_addr_t ihd_addr = IHD_DEV_ADDR;
                zb_ieee_addr_t met_addr = MET_DEV_ADDR;
                zb_secur_ke_whitelist_add(ihd_addr);
                zb_secur_ke_whitelist_add(met_addr);
            }
#endif
        ...
        case ZB_SE_SIGNAL_APS_KEY_READY:
            TRACE_MSG(TRACE_APP1, "Got partner LK", (FMT__0));
            {
                static zb_int_t once = 0;
                if (once == 0)
                {
                    once++;
                    ZB_SCHEDULE_APP_ALARM(test_partner_lk, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(5000));
                }
                else if (once == 1)
                {
                    once++;
                    ZB_SCHEDULE_APP_ALARM(unbind_kec_cluster, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(10000));
                }
            }
            break;
...
static void test_partner_lk(zb_uint8_t param)
{
    ...
    else
    {
        ...
        req->dst_addr = zb_address_short_by_ieee(ihd_addr);
        ZB_IEEE_ADDR_COPY(req->ieee_addr, ihd_addr);
        req->start_index = 0;
        req->request_type = 0;
...
static void start_partner_lk(zb_uint8_t param)
{
    ...
    {
        TRACE_MSG(TRACE_ERROR, "Failed to get nwk addr of IHD", (FMT__0));
        zb_buf_free(param);
    }
}

static void unbind_kec_cluster_cb(zb_uint8_t param)
{
    zb_buf_free(param);
}
...
static void unbind_kec_cluster(zb_uint8_t param)
{
    ...
    zb_ieee_addr_t src_addr = MET_DEV_ADDR;
    zb_ieee_addr_t dst_addr = IHD_DEV_ADDR;

    if (param == 0)
    {
        zb_buf_get_out_delayed(unbind_kec_cluster);
        return;
    }

    TRACE_MSG(TRACE_APP1, ">> unbind_kec_cluster", (FMT__0));
    req = ZB_BUF_GET_PARAM(param, zb_zdo_bind_req_param_t);

    ZB_IEEE_ADDR_COPY(req->src_address, &dst_addr);
    req->src_endp =  NCP_TEST_KEC_ENDPOINT;
    req->cluster_id = ZB_ZCL_CLUSTER_ID_KEY_ESTABLISHMENT;
    req->dst_addr_mode = ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
    ZB_IEEE_ADDR_COPY(req->dst_address.addr_long, &src_addr);
    req->dst_endp = METERING_DEV_ENDPOINT;
    req->req_dst_addr = zb_address_short_by_ieee(dst_addr);

    zb_zdo_unbind_req(param, unbind_kec_cluster_cb);
