...
/* IHD IEEE address to be used for CBKE with non TC */
#define IHD_DEV_ADDR {0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb}

/* address of device allowed to request CBKE while that is not ZC/TC */
static zb_ieee_addr_t ncp_adress_to_whitelist = {0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb};
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_ZDO_SIGNAL_SKIP_STARTUP:
            ...
            /* Add ieee address of non TC device authorized to request CBKE */
            zb_secur_ke_whitelist_add(ncp_adress_to_whitelist);
#else
#error ZBOSS must be compiled with ZB_SE_KE_WHITELIST for this sample
        ...
        case ZB_SE_SIGNAL_APS_KEY_READY:
            ...
            /* Request CBKE to peer non TC */
            {
                zb_ieee_addr_t ihd_addr = IHD_DEV_ADDR;
                zb_uint16_t ihd_short_addr =   zb_address_short_by_ieee(ihd_addr);
                zb_buf_get_out_delayed_ext(se_send_key_est_match_desc_addr, ihd_short_addr, 0);
            }

            /* Don't do PARTNER_LK from our side in whitelist sample
            {
