...
#define ZB_TRACE_FILE_ID 40015

#include "simple_gw.h"
#include "simple_gw_device.h"

/* Needed for definition of zb_enable_control4_emulator() internal func */
#include "zb_common.h"
#include "zb_nwk.h"
...
simple_control4_gw_attr_t simple_c4_controller_attr;

ZB_ZCL_DECLARE_CONTROL4_NETWORKING_CONTROLLER_ATTRIB_LIST(c4_attr_list,
        &simple_c4_controller_attr.access_point_node_ID,
        &simple_c4_controller_attr.access_point_long_ID,
        &simple_c4_controller_attr.access_point_cost
                                                         );
...
#ifdef GW_CONTROL4_COMPATIBLE
ZB_ZCL_CONTROL4_NETWORK_DECLARE_CLUSTER_LIST(c4_network_ctrl_list, c4_attr_list);

ZB_ZCL_CONTROL4_NETWORK_DECLARE_EP(c4_ctrl_ep, c4_network_ctrl_list);

ZB_HA_DECLARE_SIMPLE_GW_CTX(simple_gw_ctx, simple_gw_ep, c4_ctrl_ep);
#else
...
#ifdef GW_CONTROL4_COMPATIBLE
void simple_c4_controller_cluster_attr_init(void)
{
    ZB_IEEE_ADDR_COPY(simple_c4_controller_attr.access_point_long_ID, g_zc_addr);
    simple_c4_controller_attr.access_point_node_ID = ZB_PIBCACHE_NETWORK_ADDRESS();
    simple_c4_controller_attr.access_point_cost = ZB_ZCL_CONTROL4_NETWORKING_ACCESS_POINT_COST_DEF_VALUE;

    TRACE_MSG(TRACE_APP1, "<< simple_c4_controller_cluster_attr_init", (FMT__0));
}
#endif /* GW_CONTROL4_COMPATIBLE */
...
#ifdef GW_CONTROL4_COMPATIBLE
    simple_c4_controller_cluster_attr_init();
#endif
...
#ifdef GW_CONTROL4_COMPATIBLE
/* Write MTORR_PERIOD and WINDOW_ANNOUNCE values on ZED after joining */
void simple_control4_gw_mtorr_and_announce_update(zb_bufid_t param, zb_uint16_t idx)
{
    zb_uint8_t *cmd_ptr;

    /* Set MTORR_PERIOD period to 15 seconds (0x000f)*/
    zb_uint16_t mtorr_period = 0x000f;

    /* Set ANNOUNCE_WINDOW to 20 seconds (0x0014) */
    zb_uint16_t announce_window = 0x0014;

    if (!param)
    {
        zb_buf_get_out_delayed_ext(simple_control4_gw_mtorr_and_announce_update, idx, 0);
    }
    else
    {
        if (idx != SIMPLE_GW_INVALID_DEV_INDEX)
        {
            if (g_device_ctx.devices[idx].dev_state != NO_DEVICE)
            {
                ZB_ZCL_GENERAL_INIT_WRITE_ATTR_REQ(param, cmd_ptr, ZB_ZCL_ENABLE_DEFAULT_RESPONSE);

                ZB_ZCL_GENERAL_ADD_VALUE_WRITE_ATTR_REQ(cmd_ptr, ZB_ZCL_ATTR_CONTROL4_NETWORKING_MTORR_PERIOD_ID,
                                                        ZB_ZCL_ATTR_TYPE_U16, (zb_uint8_t *) &mtorr_period);
                ZB_ZCL_GENERAL_ADD_VALUE_WRITE_ATTR_REQ(cmd_ptr, ZB_ZCL_ATTR_CONTROL4_NETWORKING_ANNOUNCE_WINDOW_ID,
                                                        ZB_ZCL_ATTR_TYPE_U16, (zb_uint8_t *) &announce_window);

                ZB_ZCL_GENERAL_SEND_WRITE_ATTR_REQ(param, cmd_ptr, g_device_ctx.devices[idx].short_addr,
                                                   ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
                                                   ZB_CONTROL4_NETWORK_ENDPOINT, ZB_CONTROL4_NETWORK_ENDPOINT,
                                                   ZB_AF_CONTROL4_PROFILE_ID, ZB_ZCL_CLUSTER_ID_CONTROL4_NETWORKING, NULL);
    ...
    TRACE_MSG(TRACE_APP1, "<< simple_control4_gw_mtorr_and_announce_update", (FMT__0));
}

/* Request Control4 immediate announce command to a device */
void simple_control4_gw_req_immediate_announce(zb_bufid_t param, zb_uint16_t idx)
{
    if (!param)
    {
        zb_buf_get_out_delayed_ext(simple_control4_gw_req_immediate_announce, idx, 0);
    }
    else
    {
        if (idx != SIMPLE_GW_INVALID_DEV_INDEX)
        {
            if (g_device_ctx.devices[idx].dev_state != NO_DEVICE)
            {
                ZB_ZCL_C4_IMMEDIATE_ANNOUNCE_REQ(param,
                                                 g_device_ctx.devices[idx].short_addr,
                                                 ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
    ...
    TRACE_MSG(TRACE_APP1, "<< simple_control4_gw_req_immediate_announce", (FMT__0));
}

void simple_control4_gw_schedule_immediate_announce(zb_uint8_t idx)
{
    zb_buf_get_out_delayed_ext(simple_control4_gw_req_immediate_announce, idx, 0);

    TRACE_MSG(TRACE_APP1, "<< simple_control4_gw_schedule_immediate_announce", (FMT__0));
}
#endif
...
#ifdef GW_CONTROL4_COMPATIBLE
        /* get an updated idx to be used in delayed immediate announce*/
        idx = simple_gw_get_dev_index_by_short_addr(short_addr);
#endif
    }

#ifdef GW_CONTROL4_COMPATIBLE
    /* Update MTORR_PERIOD and ANNOUNCE_WINDOW */
    ZB_SCHEDULE_APP_CALLBACK2(simple_control4_gw_mtorr_and_announce_update, 0, idx);
    /*
      Send control4 immediate announce 5 seconds after receiving device announce
      This 5 second delay makes it easier to distinguish, in a capture, the initial identify/announce
      from device answer to the immediate announce.
    */
    ZB_SCHEDULE_APP_ALARM(simple_control4_gw_schedule_immediate_announce, idx, 5 * ZB_TIME_ONE_SECOND);
#endif
}
...
#ifdef GW_CONTROL4_COMPATIBLE
            /*Update c4 attr with nwk short addr, not in initialization because nwk short_addr is only known if device is in a nwk*/
            simple_c4_controller_attr.access_point_node_ID = ZB_PIBCACHE_NETWORK_ADDRESS();
#endif /* GW_CONTROL4_COMPATIBLE */
