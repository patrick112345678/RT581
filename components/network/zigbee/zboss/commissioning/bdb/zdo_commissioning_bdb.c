...
__weak zb_uint8_t ZB_RAF_SCAN_DURATION = ZB_DEFAULT_SCAN_DURATION;
#endif
...
#ifdef ZB_RAF_FAST_JOIN_CONFIGURATION
  ZB_BDB().bdb_scan_duration = ZB_RAF_SCAN_DURATION;

  /* scan duration range from 0 ~ 14 */
  if(ZB_BDB().bdb_scan_duration > 14)
  {
      ZB_BDB().bdb_scan_duration = 5;
  }
#else
...
static void bdb_handle_comm_signal(zb_commissioning_signal_t signal, zb_bufid_t param)
{
  ...
  switch(signal)
  {
    ...
    case ZB_COMM_SIGNAL_DEV_ANNCE_SENT:
      TRACE_MSG(TRACE_INFO1, "dev_annce_sent", (FMT__0));
...
void zb_bdb_initiate_tc_rejoin(zb_uint8_t param)
{
  ...
  if (ret == RET_OK)
  {
    ...
    bdb_commissioning_signal(BDB_COMM_SIGNAL_INIT_TC_REJOIN, ZB_BUF_INVALID);
  }
