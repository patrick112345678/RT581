...
static zb_bool_t level_control_check_req_options(zb_uint8_t param, zb_uint8_t endpoint)
{
  ...
  if (attr_desc != NULL)
  {
    ...
    ZB_ZCL_LEVEL_CONTROL_GET_CMD_OPTIONS(param, req_options, have_options_in_req);
...
#ifdef ZB_ZCL_SUPPORT_CLUSTER_ON_OFF
    /* ZCL7, 3.10.2.2.8.1 ExecuteIfOff Options Bit
...
static void move_to_level_continue(zb_uint8_t endpoint)
{
  ...
  if (move_variables->is_onoff &&
      ...
      move_variables->end_level <= ZB_ZCL_LEVEL_CONTROL_LEVEL_MIN_VALUE)/* Randy add */
