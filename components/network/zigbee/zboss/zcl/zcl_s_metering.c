...
zb_uint8_t gs_metering_server_received_commands[] =
{
  ZB_ZCL_CLUSTER_ID_METERING_SERVER_ROLE_RECEIVED_CMD_LIST
};
...
zb_discover_cmd_list_t gs_metering_server_cmd_list =
{
  sizeof(gs_metering_server_received_commands), gs_metering_server_received_commands,
  sizeof(gs_metering_server_generated_commands), gs_metering_server_generated_commands
};

zb_bool_t zb_zcl_process_s_metering_specific_commands(zb_uint8_t param);

static zb_ret_t check_value_metering(zb_uint16_t attr_id, zb_uint8_t endpoint, zb_uint8_t *value);

void zb_zcl_metering_init_server()
{
  zb_zcl_add_cluster_handlers(ZB_ZCL_CLUSTER_ID_METERING,
                              ZB_ZCL_CLUSTER_SERVER_ROLE,
                              (zb_zcl_cluster_check_value_t)check_value_metering,
                              (zb_zcl_cluster_write_attr_hook_t)NULL,
                              (zb_zcl_cluster_handler_t)zb_zcl_process_s_metering_specific_commands);
}
...
#define ZB_ZCL_METERING_SEND_CMD_HELPER(_fn, _param, _cmd_info, _payload, _payload_size, _cb)  \
  _fn((_param), \
     ...
     (_payload), (_payload_size), (_cmd_info)->seq_number, (_cb))
...
static zb_uint8_t *zb_zcl_metering_put_get_profile_response(zb_uint8_t *data, const void *pl_arg)
{
  ...
  for (i = 0; i < pl->number_of_periods_delivered; i++)
  {
...
static zb_uint8_t *zb_zcl_metering_put_get_sampled_data_response(zb_uint8_t *data, const void *pl_arg)
{
  ...
  for (i = 0; i < pl->number_of_samples; i++)
  {
...
static zb_uint8_t *zb_zcl_metering_put_publish_snapshot(zb_uint8_t *data, const void *pl_arg)
{
  ...
  switch (pl->snapshot_payload_type)
  {
    case ZB_ZCL_METERING_TOU_DELIVERED_REGISTERS:
    {
      ...
      for (i = 0; i < sub_payload->number_of_tiers_in_use; i++)
      {
    ...
    case ZB_ZCL_METERING_BLOCK_TIER_DELIVERED:
    {
      ...
      for (i = 0; i < sub_payload->number_of_tiers_and_block_thresholds_in_use; i++)
      {
...
void zb_zcl_metering_send_cmd_request_fast_poll_mode_response(zb_uint8_t param, const zb_addr_u *dst_addr,
                                                                   ...
                                                                   zb_uint8_t pl_size, zb_uint8_t tsn,
                                                                   zb_callback_t cb)
{
  ...
  zb_zcl_send_cmd_tsn(param, dst_addr, dst_addr_mode, dst_ep,
                  ZB_ZCL_FRAME_DIRECTION_TO_CLI,
                                                             ...
                                                             zb_uint8_t pl_size, zb_uint8_t tsn, zb_callback_t cb)
{
  if (0 == pl_size)
  {
    pl_size = ZB_OFFSETOF(zb_zcl_metering_get_sampled_data_response_payload_t, samples) + ZB_24BIT_SIZE * pl->number_of_samples;
                                                                ...
                                                                const zb_zcl_parsed_hdr_t *cmd_info)
{
  ...
  zb_zcl_metering_request_fast_poll_mode_response_payload_t pl_out = { 0 };
  ...
  {
    TRACE_MSG(TRACE_ZCL1, "Invalid request_fast_poll_mode payload", (FMT__0));
                                                      ...
                                                      const zb_zcl_parsed_hdr_t *cmd_info)
{
  ...
  {
    TRACE_MSG(TRACE_ZCL1, "Invalid get_snapshot payload", (FMT__0));
    zb_zcl_send_default_handler(param, cmd_info, ZB_ZCL_STATUS_INVALID_FIELD);
    return ZB_TRUE;
  }
                                                          ...
                                                          const zb_zcl_parsed_hdr_t *cmd_info)
{
  ...
  zb_zcl_metering_get_sampled_data_response_payload_t pl_out = { 0 };
  ...
  {
    TRACE_MSG(TRACE_ZCL1, "Invalid get_sampled_data payload", (FMT__0));
    zb_zcl_send_default_handler(param, cmd_info, ZB_ZCL_STATUS_INVALID_FIELD);
    return ZB_TRUE;
  }
                                                     ...
                                                     const zb_zcl_parsed_hdr_t *cmd_info)
{
  ...
  zb_zcl_metering_get_profile_response_payload_t pl_out = { 0 };
  ...
  {
    TRACE_MSG(TRACE_ZCL1, "Invalid get_profile payload", (FMT__0));
    zb_zcl_send_default_handler(param, cmd_info, ZB_ZCL_STATUS_INVALID_FIELD);
    return ZB_TRUE;
  }
...
zb_bool_t zb_zcl_process_s_metering_specific_commands(zb_uint8_t param)
{
  ...
  {
    ZCL_CTX().zb_zcl_cluster_cmd_list = &gs_metering_server_cmd_list;
    return ZB_TRUE;
  }
