...
zb_uint8_t zb_zcl_handle(zb_uint8_t param)     /* VS Fixed */
{
  ...
  if (status == ZB_ZCL_STATUS_SUCCESS)
  {
    ...
    if (!processed)
    {
      ...
      if (cmd_info_ptr->is_common_command)
      {
        status = cmd_info_ptr->is_manuf_specific ?
          ZB_ZCL_STATUS_UNSUP_MANUF_GEN_CMD : ZB_ZCL_STATUS_UNSUP_GEN_CMD;
      }
      else
      {
        status = cmd_info_ptr->is_manuf_specific ?
          ZB_ZCL_STATUS_UNSUP_MANUF_CLUST_CMD : ZB_ZCL_STATUS_UNSUP_CLUST_CMD;
      }
    }
