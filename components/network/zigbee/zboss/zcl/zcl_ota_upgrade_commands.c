...
#define OTA_MIN_BLOCK_REQ_DELAY_RAF ZB_TIME_BEACON_INTERVAL_TO_MSEC(  \
    ZB_N_APS_ACK_WAIT_DURATION_FROM_NON_SLEEPY * (ZB_N_APS_MAX_FRAME_RETRIES - 1) / (ZB_APS_DUPS_TABLE_SIZE*4) )
#define OTA_BLOCK_REQ_DELAY(_delay) (((_delay > OTA_MIN_BLOCK_REQ_DELAY_RAF)) ? (_delay) : OTA_MIN_BLOCK_REQ_DELAY_RAF)

static void zb_zcl_ota_upgrade_send_block_requset(zb_uint8_t param, zb_time_t current_delay)
{
    ...
    TRACE_MSG(TRACE_ZCL1, "> zb_zcl_ota_upgrade_send_block_requset param %hx", (FMT__H, param));
    ...
    TRACE_MSG(TRACE_ZCL1, "< zb_zcl_ota_upgrade_send_block_requset", (FMT__0));
}
...
zb_ret_t zb_zcl_ota_upgrade_start_client(zb_uint8_t server_ep, zb_uint16_t server_addr)
{
            ...
            server_ep != ZB_ZCL_OTA_UPGRADE_SERVER_ENDPOINT_DEF_VALUE)
    {
        ...
        zb_zcl_ota_upgrade_set_query_interval(endpoint, 1); //set the normal query interval to 1 min
        ...
        if ( ZB_NWK_IS_ADDRESS_BROADCAST(cmd_info.addr_data.common_data.dst_addr) )
        {
            ...
            case ZB_ZCL_OTA_UPGRADE_IMAGE_NOTIFY_PAYLOAD_JITTER_CODE_IMAGE:
                is_agree_file &=
                    ...
                    (payload.image_type == 321);//Randy add
            /* FALLTHROUGH */
    ...
    else
    {
        ...
        case ZB_ZCL_STATUS_SUCCESS:
        {
            ...
            if (app_status == ZB_ZCL_OTA_UPGRADE_STATUS_OK)
            {
                ...
                zb_zcl_ota_upgrade_send_block_requset(param, delay);
                ...
                && ZB_ZCL_OTA_UPGRADE_IMAGE_STATUS_DOWNLOADING == ZB_ZCL_GET_ATTRIBUTE_VAL_8(attr_desc))
        {
            ...
            if (ZCL_CTX().ota_cli.resend_retries < ZCL_OTA_MAX_RESEND_RETRIES)
            {
                ...
                ZB_ZCL_OTA_UPGRADE_SEND_IMAGE_BLOCK_REQ(send_buf,
                                                        ...
                                                        client_data->max_data_size,
                                                        our_long_address,
                                                        OTA_BLOCK_REQ_DELAY(delay), OTA_BLOCK_REQ_DELAY(delay));
                client_data->img_block_req_sent = 1;
                ZB_SCHEDULE_ALARM_CANCEL(resend_buffer, 0);
                /* Extend resend inteval to exclude situation when we request new block and retransmit APS packet
                 * with old requet. */
        ...
        zb_zcl_ota_upgrade_image_block_res_t *payload)
{
    ...
    zb_zcl_ota_upgrade_send_block_requset(param, zb_zcl_ota_upgrade_get16(endpoint, ZB_ZCL_ATTR_OTA_UPGRADE_MIN_BLOCK_REQUE_ID));
...
static zb_ret_t upgrade_end_resp_handler(zb_uint8_t param)
{
    ...
    else
    {
        ...
        TRACE_MSG(TRACE_ZCL2, "saved parameter %d %d", (FMT__L_D_D,
                  zb_zcl_ota_upgrade_get32(endpoint, ZB_ZCL_ATTR_OTA_UPGRADE_DOWNLOADED_FILE_VERSION_ID),
                  zb_zcl_ota_upgrade_get16(endpoint, ZB_ZCL_ATTR_OTA_UPGRADE_MANUFACTURE_ID)));
...
static zb_uint8_t zb_zcl_process_ota_upgrade_default_response_commands_cli(zb_uint8_t param)
{
...
static zb_bool_t zb_zcl_process_ota_cli_upgrade_specific_commands(zb_uint8_t param)
{
    ...
    case ZB_ZCL_CMD_DEFAULT_RESP:
        zb_zcl_process_ota_upgrade_default_response_commands_cli(param);
        processed = ZB_FALSE;
        break;
    default:
        processed = ZB_FALSE;
