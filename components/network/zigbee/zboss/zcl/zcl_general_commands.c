...
static void zb_zcl_read_attr_handler_continue(zb_uint8_t param)
{
    ...
    */
    ...
    {
        ZB_HTOLE16((zb_uint8_t *)&attr_id, (zb_uint8_t *)&read_attr_req->attr_id[i]);
    ...
    if (cmd_info->profile_id == 0xffff)
    {
        profile_id = ep_desc->profile_id;
    }
    else
    {
        profile_id = cmd_info->profile_id;
    }
...
void zb_zcl_write_attr_handler(zb_uint8_t param)
{
    ...
    /* ZCL spec, 2.4.3 Write attributes Command Frame */
        ...
        if (write_attr_req != NULL)
        {
            ...
            else
            {
                ...
                if (ZB_ZCL_CMD_WRITE_ATTRIB_NO_RESP != cmd_info->cmd_id)
                {
                    ...
                    {
                        if (status == ZB_ZCL_STATUS_LIMIT_REACHED)
                        {
                            ZVUNUSED(zb_zcl_set_attr_val(ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).dst_endpoint,
                                                         cmd_info->cluster_id,
                                                         (cmd_info->cmd_direction == ZB_ZCL_FRAME_DIRECTION_TO_SRV) ?
...
static zb_uint8_t check_config_rep_req(zb_zcl_configure_reporting_req_t *config_rep_req, zb_zcl_attr_t *attr_desc)
{
    ...
    if (config_rep_req->direction == ZB_ZCL_CONFIGURE_REPORTING_SEND_REPORT)
    {
        ...
        {
            /* ZCL spec 2.4.7.3 Effect on Receipt: if the attribute type is set to array,
...
static void zb_zcl_disc_attr_handler(zb_uint8_t param)
{
    ...
    /* ZCL spec, 2.4.13 Discovery Attributes Response Command
