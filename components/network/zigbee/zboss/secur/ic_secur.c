...
static zb_uint8_t c2b(char c)
{
  ...
  ZB_ASSERT(c > '\0');
  if((c <= '9') && (c >= '0')) { return uc - 48u; }
  else if((c <= 'F') && (c >= 'A')) { return uc - 55u; }
  else if((c <= 'f') && (c >= 'a')) { return uc - 87u; }
  else { return 16u; };
}
...
zb_ret_t zb_secur_ic_from_string(char *ic_str, zb_uint8_t *ic_type, zb_uint8_t *ic)
{
    ...
    switch(n)
    {
      case 4: ic_type_tmp = ZB_IC_TYPE_48; break;
      case 5: ic_type_tmp = ZB_IC_TYPE_64; break;
      case 7: ic_type_tmp = ZB_IC_TYPE_96; break;
      case 9: ic_type_tmp = ZB_IC_TYPE_128; break;
      default:
