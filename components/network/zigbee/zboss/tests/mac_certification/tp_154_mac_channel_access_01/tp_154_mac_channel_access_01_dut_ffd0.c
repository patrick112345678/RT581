 ...
 * DSR.
 */
/* PURPOSE: P/154/MAC/CHANNEL-ACCESS-01 DUT FFD0
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_CHANNEL_ACCESS_01_DUT_FFD0
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_channel_access_01_dut_ffd0");
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
  zb_mcps_data_confirm_params_t data_conf_params = *ZB_BUF_GET_PARAM(param, zb_mcps_data_confirm_params_t);
	...
	if(data_conf_params.status == 0xe1) // CHANNEL_ACCESS_FAILURE
	{
		zb_osif_led_on(0);
		TRACE_MSG(TRACE_APP1, "0xe1 = CHANNEL_ACCESS_FAILURE", (FMT__0));
		TRACE_MSG(TRACE_APP1, "Channel_access01 success", (FMT__0));
	}
