...
#define ZB_TEST_NAME TP_154_MAC_SCANNING_01_DUT_FFD1
#define ZB_TRACE_FILE_ID 57203
#include "zb_common.h"
#include "zb_scheduler.h"
#include "zb_bufpool.h"
#include "../common/zb_cert_test_globals.h"
#include "tp_154_mac_scanning_01_common.h"
#include "zb_osif.h"
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_scanning_01_dut_ffd1");
...
static void test_mlme_scan_request(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "MLME-SCAN.request() - ED scan", (FMT__0));
...
ZB_MLME_SCAN_CONFIRM(zb_uint8_t param)
{
  zb_mac_scan_confirm_t *scan_confirm = 0;
	...
	for (channel_num = 0; channel_num < scan_confirm->result_list_size; channel_num++)
	{
		TRACE_MSG(TRACE_APP1, "ED value[channel %d] = -%d dB", (FMT__H_H, (channel_num+11),
                                                scan_confirm->list.energy_detect[channel_num]));
	}	
	*/
	if (scan_confirm->status == MAC_SUCCESS &&
		  scan_confirm->scan_type == 0x00 &&
	    scan_confirm->channel_page == 0x00 &&
	    scan_confirm->result_list_size == 0x10 &&
	    scan_confirm->unscanned_channels == 0x00000000)
	{
		zb_osif_led_on(0);
		TRACE_MSG(TRACE_APP1, "Scanning01 success", (FMT__0));
	}
  zb_buf_free(param);
}
