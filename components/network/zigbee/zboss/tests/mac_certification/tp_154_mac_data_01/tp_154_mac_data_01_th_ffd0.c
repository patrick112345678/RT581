 ...
 * DSR.
 */
/* PURPOSE: TP/154/MAC/DATA-01 TH FFD0
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_DATA_01_TH_FFD0
#define ZB_TRACE_FILE_ID 57351
#include "zb_common.h"
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_data_01_th_ffd0");
...
static void test_mcps_data_request(zb_uint8_t param)
{
  ...
  dut_test_step++;
  msdu_length = (dut_test_step != TH2DUT_SHORT2SHORT_INDIRECT_ACK_MAX_PACKET) ? TEST_MSDU_LENGTH : TEST_MAX_MSDU_LENGTH;
  ...
  switch(dut_test_step)
  {
    case  TH2DUT_SHORT2SHORT_INDIRECT_ACK:
      ...
      data_req->dst_addr.addr_short = TEST_DUT_RFD1_SHORT_ADDRESS;
      data_req->dst_pan_id = TEST_PAN_ID;
      data_req->msdu_handle = TEST_TH_FFD0_MSDU_HANDLE;
    ...
    case  TH2DUT_SHORT2EXT_INDIRECT_ACK:
      ...
      ZB_IEEE_ADDR_COPY(data_req->dst_addr.addr_long, g_ieee_addr_dut_rfd1);
      data_req->dst_pan_id = TEST_PAN_ID;
      data_req->msdu_handle = TEST_TH_FFD0_MSDU_HANDLE;
      data_req->tx_options = 0x05;
      break;
...
ZB_MLME_ASSOCIATE_INDICATION(zb_uint8_t param)
{
  ...
  ZB_MLME_BUILD_ASSOCIATE_RESPONSE(param, device_address, TEST_DUT_RFD1_SHORT_ADDRESS, 0);
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
  ...
  if (dut_test_step < TEST_STEP_FINISHED)
  {
    if (dut_test_step >= TH2DUT_SHORT2SHORT_INDIRECT_ACK)
    {
...
ZB_MCPS_DATA_INDICATION(zb_uint8_t param)
{
  ...
  {
    dut_test_step++;
    if (dut_test_step >= DUT2TH_SHORT2SHORT_UNICAST_ACK_MAX_PACKET)
    {
      ZB_SCHEDULE_CALLBACK(test_mcps_data_request, param);
      param = 0;
    }
