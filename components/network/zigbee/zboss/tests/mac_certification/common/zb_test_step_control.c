...
#ifndef ZB_NSNG
static void test_control_button1_handler(zb_uint8_t unused);
#endif /* ZB_NSNG */
...
zb_ret_t test_control_force_next()
{
  ...
  if (test_control_ctx.step_idx < test_control_ctx.steps_num)
  {
    ...
    step_info = test_step_get_info(test_control_ctx.step_idx);
		if (!step_info->step_time)
    {
      ZB_SCHEDULE_CALLBACK(step_info->callback, step_info->param);
    }
    else
    {
      ZB_SCHEDULE_ALARM(step_info->callback, step_info->param, step_info->step_time);
    }
