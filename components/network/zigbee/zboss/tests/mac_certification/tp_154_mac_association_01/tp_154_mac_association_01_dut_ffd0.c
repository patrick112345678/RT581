 ...
 * DSR.
 */
/* PURPOSE: P/154/MAC/ASSOCIATION-01 DUT FFD0
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_ASSOCIATION_01_DUT_FFD0
#define ZB_TRACE_FILE_ID 57195

#include "zb_common.h"
#include "zb_scheduler.h"
#include "zb_bufpool.h"

#include "tp_154_mac_association_01_common.h"
...
MAIN()
{
    ...
    ZB_INIT("tp_154_mac_association_01_dut_ffd0");
...
ZB_MLME_ASSOCIATE_INDICATION(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, "DeviceAddress = " TRACE_FORMAT_64,
              (FMT__A, TRACE_ARG_64(request->device_address)));
    TRACE_MSG(TRACE_APP1, "CapabilityInformation = %x", (FMT__H, request->capability));
    //TRACE_MSG(TRACE_APP1, "MLME-ASSOCIATE.indication()", (FMT__0));
    /*
    ...
    ZB_MLME_BUILD_ASSOCIATE_RESPONSE(param, device_address, TEST_TH_RFD0_SHORT_ADDRESS, 0);
...
ZB_MLME_COMM_STATUS_INDICATION(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, "Check for positive association response. (Status = 0x00, success)", (FMT__0));
    zb_buf_free(param);
}
