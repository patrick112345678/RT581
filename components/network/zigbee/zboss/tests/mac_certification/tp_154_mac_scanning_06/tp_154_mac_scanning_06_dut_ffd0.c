 ...
 * DSR.
 */
/* PURPOSE: TP/154/MAC/SCANNING-06 DUT FFD
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_SCANNING_06_DUT_FFD0
#define ZB_TRACE_FILE_ID 57205
#include "zb_common.h"
...
static zb_ieee_addr_t g_ieee_addr_dut_ffd = TEST_DUT_FFD0_MAC_ADDRESS;

static void test_started_cb(zb_uint8_t param);
static void test_mlme_reset_request(zb_uint8_t param, zb_uint16_t user_param);
...
MAIN()
{
    ...
    ZB_INIT("tp_154_mac_scanning_06_dut_ffd0");
#if UART_CONTROL
    test_control_init();
...
static void test_started_cb(zb_uint8_t unused)
{
    ...
    /* Need to receive 5 beacons while one buffer can contain only 4 beacons + scan confirm. */
    zb_buf_get_out_delayed_ext(test_mlme_reset_request, 0, 2 * ZB_IO_BUF_SIZE);
}
...
static void test_set_ieee_addr(zb_uint8_t param)
{
    ...
    ZB_MEMCPY((zb_uint8_t *)(req + 1), g_ieee_addr_dut_ffd, sizeof(zb_ieee_addr_t));
...
static void test_mlme_scan_request(zb_uint8_t param)
{
    ...
    //ZB_MLME_BUILD_SCAN_REQUEST(param, TEST_PAGE, 0x07FBF800, TEST_SCAN_TYPE, TEST_SCAN_DURATION);
    ZB_SCHEDULE_ALARM(zb_mlme_scan_request, param, TEST_DUT_SCAN_TIMEOUT);
}
...
zb_bool_t auto_req_result6 = ZB_FALSE;
zb_bool_t no_auto_req_result6 = ZB_FALSE;
...
ZB_MLME_SCAN_CONFIRM(zb_uint8_t param)
{
    ...
    {
        TRACE_MSG(TRACE_APP1, "You should have not seen any beacon notify.", (FMT__0));
        if (scan_confirm->status == 0x00 &&
                scan_confirm->scan_type == 0x01 &&
                scan_confirm->channel_page == 0x00 &&
                scan_confirm->result_list_size == 0x05 &&
                scan_confirm->unscanned_channels == 0x00000000)
        {
            auto_req_result6 = ZB_TRUE;
        }
    }
    else
    {
        TRACE_MSG(TRACE_APP1, "You should have seen five beacon notify.", (FMT__0));
        if (scan_confirm->status == 0x00 &&
                scan_confirm->scan_type == 0x01 &&
                scan_confirm->channel_page == 0x00 &&
                ...
                scan_confirm->unscanned_channels == 0x00000000)
        {
            no_auto_req_result6 = ZB_TRUE;
        }
    }

    if (auto_req_result6 && no_auto_req_result6)
    {
        zb_osif_led_on(0); // blue
        TRACE_MSG(TRACE_APP1, "Scanning06 success", (FMT__0));
    }
