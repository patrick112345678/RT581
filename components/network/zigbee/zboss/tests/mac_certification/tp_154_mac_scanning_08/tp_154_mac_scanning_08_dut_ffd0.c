 ...
 * DSR.
 */
/* PURPOSE: TP/154/MAC/SCANNING-08 FFD0
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_SCANNING_08_DUT_FFD0
#define ZB_TRACE_FILE_ID 57213
#include "zb_common.h"
#include "zb_scheduler.h"
#include "zb_bufpool.h"
#include "../common/zb_cert_test_globals.h"
#include "tp_154_mac_scanning_08_common.h"
#include "zb_osif.h"
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_scanning_08_dut_ffd0");
...
ZB_MLME_ORPHAN_INDICATION(zb_uint8_t param)
{
  ...
  //TRACE_MSG(TRACE_APP1, "MLME-ORPHAN.indication()", (FMT__0));
	//TRACE_MSG(TRACE_APP1, "OrphanAddress=" TRACE_FORMAT_64,
              //(FMT__A, TRACE_ARG_64(ind->orphan_addr)));
	//TRACE_MSG(TRACE_APP1, "OrphanAddress = %x", (FMT__H, ind->orphan_addr));
  /* prepare and send orphan response */
  ZB_IEEE_ADDR_COPY(resp->orphan_addr, ind->orphan_addr);
  resp->short_addr = 0x0002;
...
ZB_MLME_COMM_STATUS_INDICATION(zb_uint8_t param)
{
  ...
  else
  {
	...
	*/
	...
	{
		zb_osif_led_on(0);
		TRACE_MSG(TRACE_APP1, "Scanning08 success", (FMT__0));
	}
  zb_buf_free(param);
