...
#define ZB_TEST_NAME TP_154_PHY24_TURNAROUND_TIME_02_DUT
#define ZB_TRACE_FILE_ID 57220
#include "zb_common.h"
...
MAIN()
{
    ...
    ZB_INIT("tp_154_phy24_turnaround_time_02_dut");
...
static void test_get_command(zb_uint8_t param)
{
    ...
    {
    case STARTING:
        printf("Send u/d to increase/decrease channel number. Send t to transmit 1 packet on the current channel.\n");
...
static void test_mcps_data_request(zb_uint8_t param)
{
    ...
    data_req->dst_addr.addr_short = TEST_TH_SHORT_ADDRESS;
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, "MCPS-DATA.confirm() Status = %x", (FMT__H, data_conf_params.status));
