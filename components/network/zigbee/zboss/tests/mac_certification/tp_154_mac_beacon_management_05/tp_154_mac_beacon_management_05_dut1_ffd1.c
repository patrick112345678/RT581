 ...
 * DSR.
 */
/*  PURPOSE: TP/154/MAC/BEACON-MANAGEMENT-05 DUT1 FFD1 implementation
*/


#define ZB_TEST_NAME TP_154_MAC_BEACON_MANAGEMENT_05_DUT1_FFD1
#define ZB_TRACE_FILE_ID 63842
#include "zb_common.h"
...
MAIN()
{
    ...
    ZB_INIT("tp_154_mac_beacon_management_05_dut1_ffd1");
...
static void test_set_ieee_addr(zb_uint8_t param)
{
    ...
    ZB_MEMCPY((zb_uint8_t *)(req + 1), g_ieee_addr_dut1, sizeof(zb_ieee_addr_t));
...
{
    zb_mlme_set_request_t *req;
    zb_uint16_t short_addr = TEST_DUT1_FFD1_SHORT_ADDRESS;
...
{
    zb_mlme_set_request_t *req;
    zb_uint8_t *beacon = TEST_DUT1_BEACON_PAYLOAD;
...
static void test_prepare_enhanced_beacon_rejoin(zb_uint8_t param)
{
    ...
    {
        ZB_EXTPANID_COPY(ext_pan_id, g_ieee_addr_dut2);
    }
    else
    {
...
static void test_rejoin_req_send_pkt(zb_uint8_t param)
{
    ...
    nwhdr->dst_addr = dut2_short_addr;
    nwhdr->src_addr = dut1_short_addr;
    nwhdr->radius = 1;
    nwhdr->seq_num = 110;

    ZB_64BIT_ADDR_COPY(nwhdr->dst_ieee_addr, g_ieee_addr_dut2);
    ZB_64BIT_ADDR_COPY(nwhdr->src_ieee_addr, g_ieee_addr_dut1);
    ...
    *cmd_id = ZB_NWK_CMD_REJOIN_REQUEST;
    ...
    mac_params->src_addr.addr_short = dut1_short_addr;
    mac_params->dst_addr.addr_short = dut2_short_addr;
...
static void test_rejoin_resp_send_pkt(zb_uint8_t param)
{
    ...
    *cmd_id = ZB_NWK_CMD_REJOIN_RESPONSE;
    ...
    rejoin_response->network_addr = dut2_short_addr;
...
ZB_MLME_BEACON_NOTIFY_INDICATION(zb_uint8_t param)
{
    ...
    if (ind->beacon_type == ZB_MAC_BEACON_TYPE_ENHANCED_BEACON
            ...
            && (ind->pan_descriptor).enh_beacon_nwk_addr == TEST_DUT2_FFD0_SHORT_ADDRESS
            && ZB_IEEE_ADDR_CMP((ind->pan_descriptor).coord_address.addr_long, g_ieee_addr_dut2))
    {
...
ZB_MLME_SCAN_CONFIRM(zb_uint8_t param)
{
    ...
    {
        ZB_SCHEDULE_ALARM(test_incr_test_step, param, ZB_MILLISECONDS_TO_BEACON_INTERVAL(TEST_TIMEOUT));
    }
...
ZB_MCPS_DATA_INDICATION(zb_uint8_t param)
{
    ...
    else
    {
        ...
        if (nwk_hdr->dst_addr == TEST_DUT1_FFD1_SHORT_ADDRESS && ZB_IEEE_ADDR_CMP(nwk_hdr->dst_ieee_addr, g_ieee_addr_dut1))
        {
