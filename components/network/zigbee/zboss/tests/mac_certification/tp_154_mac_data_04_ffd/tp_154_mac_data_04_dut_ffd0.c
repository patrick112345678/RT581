 ...
 * DSR.
 */
/* PURPOSE: TP/154/MAC/DATA-04 DUT FFD0 (case 2)
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_DATA_04_FFD_DUT_FFD0
#define ZB_TRACE_FILE_ID 57565
#include "zb_common.h"
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_data_04_ffd_dut_ffd0");
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
	...
	if (confirm->status == MAC_SUCCESS)
  {
    zb_osif_led_on(0); // blue  
		...
		TRACE_MSG(TRACE_APP1, "Data04_DUT_FFD0 success", (FMT__0));
  }	
  zb_buf_free(param);
}
