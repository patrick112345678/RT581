 ...
 * DSR.
 */
/* PURPOSE: TP/154/MAC/DATA-03 TH FFD0
MAC-only build
*/

#define ZB_TEST_NAME TP_154_MAC_DATA_03_TH_FFD0
#define ZB_TRACE_FILE_ID 57369
#include "zb_common.h"
...
MAIN()
{
    ...
    ZB_INIT("tp_154_mac_data_03_th_ffd0");
...
{
    zb_mlme_set_request_t *req;
    zb_uint16_t panid = TEST_TH_FFD0_PAN_ID;
...
{
    zb_mlme_set_request_t *req;
    zb_uint8_t rx_on_when_idle = TEST_TH_FFD0_RX_ON_WHEN_IDLE;
...
static void test_mlme_start_request(zb_uint8_t param)
{
    ...
    req->pan_id = TEST_TH_FFD0_PAN_ID;
...
static void test_mcps_data_request(zb_uint8_t param)
{
    ...
    /* CR: 09/16/2016: [MM]: START: MAJOR(1):
       Handle "default" state in switch-case above.
     */
    ...
    case  DUT2TH_SHORT2SHORT_UNICAST_NOACK:
        ...
        data_req->src_addr.addr_short = TEST_TH_FFD0_SHORT_ADDRESS;
        data_req->dst_addr_mode = ZB_ADDR_16BIT_DEV_OR_BROADCAST;
        data_req->dst_addr.addr_short = TEST_DUT_FFD1_SHORT_ADDRESS;
        data_req->dst_pan_id = TEST_DUT_FFD1_PAN_ID;
    ...
    case  DUT2TH_SHORT2EXT_UNICAST_ACK:
        ...
        ZB_IEEE_ADDR_COPY(data_req->dst_addr.addr_long, g_ieee_addr_dut_ffd1);
        data_req->dst_pan_id = TEST_DUT_FFD1_PAN_ID;
        data_req->msdu_handle = 0;
        data_req->tx_options = 0x01;
        break;
    ...
    case  DUT2TH_EXT2SHORT_UNICAST_ACK:
        ...
        ZB_IEEE_ADDR_COPY(data_req->src_addr.addr_long, g_ieee_addr_th_ffd0);
        data_req->dst_addr_mode = ZB_ADDR_16BIT_DEV_OR_BROADCAST;
        data_req->dst_addr.addr_short = TEST_DUT_FFD1_SHORT_ADDRESS;
        data_req->dst_pan_id = TEST_DUT_FFD1_PAN_ID;
        data_req->msdu_handle = 0;
    ...
    case  DUT2TH_SHORT2BROADCAST_BCAST_PANID:
        TRACE_MSG(TRACE_APP1, "DUT2TH_SHORT2BROADCAST_BCAST_PANID", (FMT__0));
        data_req->src_addr_mode = ZB_ADDR_16BIT_DEV_OR_BROADCAST;
        data_req->src_addr.addr_short = TEST_TH_FFD0_SHORT_ADDRESS;
        data_req->dst_addr_mode = ZB_ADDR_16BIT_DEV_OR_BROADCAST;
        data_req->dst_addr.addr_short = 0xffff;
    ...
    /* CR: 09/16/2016: [MM]: START: MEDIUM(1):
     * Dirty solution: firstly some CPU time has been spent, but nothing
     * done here in case TEST_STEP_FINISHED.
     *
     * More likely, we shouldn't ever come here with TEST_STEP_FINISHED state.
     */
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
    ...
    if (dut_test_step < TEST_STEP_FINISHED)
    {
        if (dut_test_step >= DUT2TH_SHORT2SHORT_UNICAST_NOACK)
        {
...
ZB_MCPS_DATA_INDICATION(zb_uint8_t param)
{
    ...
    /* CR: 09/16/2016: [MM]: START: MAJOR(2):
     * Please note that incrementing of the "dut_test_step" variable
     * without checking any sanity checking is very dangerous;
     *
     * Actually, the bug has been met:
     * If any incoming Data frame is received, it will increase
     * the "dut_test_step" variable with previous value of
     * TEST_STEP_FINISHED, and call test_mcps_data_request
     * function, which sends an unexpected MAC Data frame.
     */
    ...
    if (dut_test_step < TEST_STEP_FINISHED)
    {
        ...
        if (dut_test_step >= TH2DUT_SHORT2BROADCAST_BCAST_PANID)
        {
            ZB_SCHEDULE_CALLBACK(test_mcps_data_request, param);
            param = 0;
        }
