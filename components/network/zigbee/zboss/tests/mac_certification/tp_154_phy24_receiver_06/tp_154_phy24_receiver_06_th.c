...
#define ZB_TEST_NAME TP_154_PHY24_RECEIVER_06_TH
#define ZB_TRACE_FILE_ID 57601
#include "zb_common.h"
...
MAIN()
{
  ...
  ZB_INIT("tp_154_phy24_receiver_06_th");
...
static void test_set_channel_req(zb_uint8_t param)
{
  ...
  req->confirm_cb_u.cb = test_set_channel_conf;
  zb_mlme_set_request(param);
}
...
static void test_set_channel_conf(zb_uint8_t param)
{
  if (gs_current_channel == TEST_CHANNEL_MIN)
  {
    ZB_SCHEDULE_ALARM(test_send_next_packet, param, 10*ZB_TIME_ONE_SECOND);
  }
  else
  {
