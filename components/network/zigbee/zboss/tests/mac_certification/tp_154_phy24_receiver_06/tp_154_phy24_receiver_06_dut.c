 ...
 * DSR.
 */
/* PURPOSE: TP/154/PHY24/RECEIVER-06
MAC-only build
*/

#define ZB_TEST_NAME TP_154_PHY24_RECEIVER_06_DUT
#define ZB_TRACE_FILE_ID 57585
#include "zb_common.h"
#include "zb_scheduler.h"
#include "zb_bufpool.h"
#include "../common/zb_cert_test_globals.h"
#include "tp_154_phy24_receiver_06_common.h"
...
static zb_bool_t gs_first_set_channel = ZB_TRUE;

/* Number of packets sent on operating channel */
static zb_uint8_t gs_current_channel;
static zb_uint16_t gs_packets_rcvd;
extern zb_step_status_t step_state;
extern zb_bool_t lqi_test;

MAIN()
{
    ARGV_UNUSED;

    ZB_INIT("tp_154_phy24_receiver_06_dut");
    lqi_test = 1;
...
static void test_set_channel_req(zb_uint8_t param)
{
    ...
    if (gs_first_set_channel)
    {
        gs_first_set_channel = ZB_FALSE;
        req->confirm_cb_u.cb = test_set_ieee_addr;
    }
    else
    {
...
static void test_get_command(zb_uint8_t param)
{
    ...
    {
    case STARTING:
        printf("Send u/d to increase/decrease channel number.\n");
...
static void test_set_channel_conf(zb_uint8_t param)
{
    ...
    {
        ZB_SCHEDULE_ALARM(test_set_channel_req, param, TEST_SWITCH_CHANNEL_TMO);
    }
    else
    {
...
ZB_MLME_RESET_CONFIRM(zb_uint8_t param)
{
    ...
    ZB_SCHEDULE_CALLBACK(test_set_page_req, param);
}
...
ZB_MCPS_DATA_INDICATION(zb_uint8_t param)
{
    ...
    //TRACE_MSG(TRACE_APP1, "PHY: channel: %hd packet: %hd, lqi: %hd, rssi: %hd",
    //TRACE_MSG(TRACE_APP1, "PHY: channel: %hd packet: %hd, lqi: %hd",
    //(FMT__H_H_H_H, gs_current_channel, gs_packets_rcvd, rssi));
