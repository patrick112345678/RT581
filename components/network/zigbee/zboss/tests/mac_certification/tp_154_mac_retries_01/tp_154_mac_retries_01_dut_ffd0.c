 ...
 * DSR.
 */
/*  PURPOSE: TP/154/MAC/RETRIES-01 DUT FFD0 implementation
*/

#define ZB_TEST_NAME TP_154_MAC_RETRIES_01_DUT_FFD0
#define ZB_TRACE_FILE_ID 57440
#include "zb_common.h"
#include "zb_scheduler.h"
#include "zb_bufpool.h"
#include "../common/zb_cert_test_globals.h"
#include "tp_154_mac_retries_01_common.h"
...
static void test_get_rx_on_when_idle(zb_uint8_t param);
static void test_set_max_frame_retries(zb_uint8_t param);
static void test_get_max_frame_retries(zb_uint8_t param);
...
MAIN()
{
  ...
  ZB_INIT("tp_154_mac_retries_01_dut_ffd0");
...
static void test_set_rx_on_when_idle(zb_uint8_t param)
{
  ...
  *(zb_uint8_t *)(req+1) = rx_on_when_idle;
  ...
  req->confirm_cb_u.cb = test_get_rx_on_when_idle;
  zb_mlme_set_request(param);
}
...
static void test_get_rx_on_when_idle(zb_uint8_t param)
{
  ...
  //TRACE_MSG(TRACE_APP1, "MLME-GET.request() RxOnWhenIdle", (FMT__0));
  req = zb_buf_initial_alloc(param, sizeof(zb_mlme_get_request_t));
  req->pib_attr   = ZB_PIB_ATTRIBUTE_RX_ON_WHEN_IDLE;
  req->pib_index  = 0;
...
static void test_mcps_data_request(zb_uint8_t param)
{
  ...
  data_req->dst_addr.addr_short = TEST_FAKE_DEST_SHORT_ADDRESS;
...
{
  zb_mlme_set_request_t *req;
  zb_uint8_t max_frame_retries = TEST_DUT_FFD0_MAX_FRAME_RETRIES;

  TRACE_MSG(TRACE_APP1, "MLME-SET.request() macMaxFrameRetries", (FMT__0));
  ...
  req->pib_length = sizeof(zb_uint8_t);
  *(zb_uint8_t *)(req+1) = max_frame_retries;

  req->confirm_cb_u.cb = test_get_max_frame_retries;
  zb_mlme_set_request(param);
}
...
static void test_get_max_frame_retries(zb_uint8_t param)
{
  ...
  //TRACE_MSG(TRACE_APP1, "MLME-GET.request() macMaxFrameRetries", (FMT__0));
...
ZB_MLME_START_CONFIRM(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "Check for 2 rounds of Max_Frame_Retries: 3 -> 1", (FMT__0));
...
ZB_MCPS_DATA_CONFIRM(zb_uint8_t param)
{
  ...
  if (dut_test_step == TEST_STEP_DEFAULT_MAC_RETRIES_NUMBER)
  {
    ZB_SCHEDULE_CALLBACK(test_set_max_frame_retries, param);
  }
  else
  {
    zb_buf_free(param);
