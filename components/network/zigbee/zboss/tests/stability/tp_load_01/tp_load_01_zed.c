...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(tp_load_01_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/* Identify cluster attributes data */
static zb_uint16_t attr_identify_time = ZB_ZCL_IDENTIFY_IDENTIFY_TIME_DEFAULT_VALUE;

ZB_ZCL_DECLARE_IDENTIFY_ATTRIB_LIST(tp_load_01_zed_identify_attr_list, &attr_identify_time);

/********************* Declare device **************************/
DECLARE_ZED_CLUSTER_LIST(tp_load_01_zed_device_clusters,
                         tp_load_01_zed_basic_attr_list,
                         tp_load_01_zed_identify_attr_list);

DECLARE_ZED_EP(tp_load_01_zed_device_ep,
               ZED_ENDPOINT,
               tp_load_01_zed_device_clusters);

DECLARE_ZED_CTX(tp_load_01_zed_device_ctx, tp_load_01_zed_device_ep);
/*************************************************************************/
...
static void test_send_zdo_req(zb_uint8_t param);
static void test_send_zdo_reqs(zb_uint8_t param);
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&tp_load_01_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_STEERING:
        ...
        if (status == 0)
        {
            ZB_SCHEDULE_ALARM(test_start_fb_initiator, 0, TP_LOAD_01_ZED_FB_INITIATOR_START_DELAY);
            ZB_SCHEDULE_ALARM(test_send_zdo_client_short_addr_req, 0, TP_LOAD_01_ZED_ZDO_CLIENT_FINDING_DELAY);
        }
        ...
        zb_uint8_t ep, zb_uint16_t cluster)
{
    ...
    if (status == ZB_BDB_COMM_BIND_SUCCESS && cluster == ZB_ZCL_CLUSTER_ID_ON_OFF)
    {
        ...
        ZB_SCHEDULE_APP_ALARM(test_send_zcl_reqs, 0, TP_LOAD_01_ZED_ZCL_REQ_DELAY);
    }
...
static void test_send_zcl_reqs(zb_uint8_t param)
{
    ...
    for (req_index = 0; req_index < TP_LOAD_01_ZED_ZCL_REQ_COUNT; req_index++)
    {
...
static void test_send_zdo_client_short_addr_req_cb(zb_uint8_t param)
{
    ...
    if (resp->status == ZB_ZDP_STATUS_SUCCESS)
    {
        ...
        ZB_SCHEDULE_APP_ALARM(test_send_zdo_reqs, param, TP_LOAD_01_ZED_ZDO_REQ_DELAY);
    }
    else
    {
        zb_buf_free(param);
...
static void test_send_zdo_req_cb(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> test_send_zdo_req_cb, param %hd, match_len %hd, status %hd",
              (FMT__H_H_H, param, resp->match_len, resp->status));

    zb_buf_reuse(param);

    if (resp->status == ZB_ZDP_STATUS_SUCCESS)
    {
        ZB_SCHEDULE_APP_CALLBACK(test_send_zdo_req, param);
    }
    else
    {
        ZB_SCHEDULE_APP_ALARM(test_send_zdo_req, param, TP_LOAD_01_DELAY_AFTER_FAILED_REQUEST);
    }
...
static void test_send_zdo_req(zb_uint8_t param)
{
    ...
    zb_uint16_t short_addr = g_short_addr_zdo_client;

    if (param == ZB_BUF_INVALID)
    {
        zb_buf_get_out_delayed(test_send_zdo_req);
        return;
    }
...
static void test_send_zdo_reqs(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> test_send_zdo_reqs, param %hd, addr 0x%x", (FMT__H_D, param, g_short_addr_zdo_client));

    for (req_index = 0; req_index < TP_LOAD_01_ZED_ZDO_REQ_COUNT; req_index++)
    {
#ifdef TP_LOAD_01_MAX_REQ_JITTER
        ZB_SCHEDULE_APP_ALARM(test_send_zdo_req, 0, ZB_RANDOM_JTR(TP_LOAD_01_MAX_REQ_JITTER));
#else
        zb_buf_get_out_delayed(test_send_zdo_req);
#endif
    }

    TRACE_MSG(TRACE_APP1, "<< test_send_zdo_reqs", (FMT__0));
}
