...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(tp_load_01_zc_basic_attr_list, &attr_zcl_version, &attr_power_source);

/* Identify cluster attributes data */
static zb_uint16_t attr_identify_time = ZB_ZCL_IDENTIFY_IDENTIFY_TIME_DEFAULT_VALUE;

ZB_ZCL_DECLARE_IDENTIFY_ATTRIB_LIST(tp_load_01_zc_identify_attr_list, &attr_identify_time);
...
ZB_ZCL_DECLARE_ON_OFF_ATTRIB_LIST_EXT(tp_load_01_zc_on_off_attr_list,
                                      &attr_on_off,
                                      &global_scene_ctrl,
                                      &on_time,
                                      &off_wait_time);
...
/********************* Declare device **************************/
DECLARE_ZC_CLUSTER_LIST(tp_load_01_zc_device_clusters,
                        tp_load_01_zc_basic_attr_list,
                        tp_load_01_zc_identify_attr_list,
                        tp_load_01_zc_on_off_attr_list);

DECLARE_ZC_EP(tp_load_01_zc_device_ep,
              ZC_ENDPOINT,
              tp_load_01_zc_device_clusters);

DECLARE_ZC_CTX(tp_load_01_zc_device_ctx, tp_load_01_zc_device_ep);
/*************************************************************************/
...
static void test_send_active_ep_req(zb_uint8_t param);
static void test_send_active_ep_reqs(zb_uint8_t param);
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&tp_load_01_zc_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    ...
    case ZB_BDB_SIGNAL_STEERING:
      ...
      if (status == 0)
      {
        ZB_SCHEDULE_ALARM(test_start_fb_target, 0, TP_LOAD_01_ZC_FB_TARGET_START_DELAY);
      }
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
      ...
      if (status == 0)
      {
        ...
        else if (ZB_IEEE_ADDR_CMP(params->ieee_addr, g_ieee_addr_zed))
        {
          ...
          ZB_SCHEDULE_APP_ALARM(test_send_active_ep_reqs, 0, TP_LOAD_01_ZC_ACTIVE_EP_REQ_DELAY);
        }
      }
...
static void test_send_active_ep_cb(zb_uint8_t param)
{
  ...
  if (resp->status == ZB_ZDP_STATUS_SUCCESS)
  {
    ZB_SCHEDULE_APP_CALLBACK(test_send_active_ep_req, param);
  }
  else
  {
    ZB_SCHEDULE_APP_ALARM(test_send_active_ep_req, param, TP_LOAD_01_DELAY_AFTER_FAILED_REQUEST);
  }
...
static void test_send_active_ep_req(zb_uint8_t param)
{
  ...
  {
    zb_buf_get_out_delayed(test_send_active_ep_req);
    return;
  }
...
static void test_send_active_ep_reqs(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, ">> test_send_active_ep_reqs, param %hd, addr 0x%x", (FMT__H_D, param, device_addr));

  for (req_index = 0; req_index < TP_LOAD_01_ZC_ACTIVE_EP_REQ_COUNT; req_index++)
  {
#ifdef TP_LOAD_01_MAX_REQ_JITTER
    ZB_SCHEDULE_APP_ALARM(test_send_active_ep_req, 0, ZB_RANDOM_JTR(TP_LOAD_01_MAX_REQ_JITTER));
#else
    zb_buf_get_out_delayed(test_send_active_ep_req);
#endif
  }

  TRACE_MSG(TRACE_APP1, "<< test_send_active_ep_reqs", (FMT__0));
}
