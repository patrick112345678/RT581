...
MAIN()
{
    ...
    ZB_INIT("zdo_zr" STR(NUMBER));

    /* set ieee addr */
    g_ieee_addr[1] = NUMBER;
    zb_set_long_address(g_ieee_addr);

    zb_set_network_router_role_legacy(1l << CHANNEL);
...
static void send_data(zb_uint8_t param)
{
    ...
    req->dst_addr.addr_short = 0x0000; /* send to ZC */
...
zb_uint8_t data_indication(zb_uint8_t param)
{
    ...
    if (ind->profileid == 0x0002)
    {
        ...
        /* zb_time_t delay = zb_random() % ZB_TIME_ONE_SECOND; */
        send_data(param);
        /* ZB_SCHEDULE_ALARM(send_data, param, delay); */
