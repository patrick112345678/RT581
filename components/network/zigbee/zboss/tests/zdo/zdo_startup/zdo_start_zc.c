...
#if 0
static void uart_rx_cb(zb_uint8_t param);
#define SERIAL_PUT(s) zb_osif_serial_put_bytes((zb_uint8_t*)s, strlen(s))
#else
#define SERIAL_PUT(s)
#endif
...
MAIN()
{
    ...
    ZB_INIT("zdo_zc");
    SERIAL_PUT("just after init");
    //zb_osif_set_uart_byte_received_cb(uart_rx_cb);
    ...
    SERIAL_PUT("before zdo_dev_start");
...
static void uart_rx_cb(zb_uint8_t param)
{
    TRACE_MSG(TRACE_APS3, "serial rx %hx %c", (FMT__H_H, param, param));
}
#endif
...
zb_uint8_t data_indication(zb_uint8_t param)
{
    ...
    if (ind->profileid == 0x0002)
    {
        ...
        ZB_SCHEDULE_ALARM(zc_send_data, param, ZB_RANDOM_VALUE(ZB_TIME_ONE_SECOND * 5 * TIMEOUT_MUL));
        //  zb_free_buf(asdu);
...
void zc_send_data_loop(zb_uint8_t param)
{
    ...
    {
        ZB_SCHEDULE_ALARM(zc_send_data_loop, 0, ZB_RANDOM_VALUE(3 * ZB_TIME_ONE_SECOND * TIMEOUT_MUL));
    }
    else
    {
        if (exchange_finished())
