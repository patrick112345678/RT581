...
#define ZB_TEST_NAME RTP_HA_11_TH_ZR

#define ZB_TRACE_FILE_ID 40445
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_ha_11_th_zr_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_TH_CLUSTER_LIST(rtp_ha_11_th_zr_device_clusters,
                        rtp_ha_11_th_zr_basic_attr_list);

DECLARE_TH_EP(rtp_ha_11_th_zr_device_ep,
              TH_ENDPOINT,
              rtp_ha_11_th_zr_device_clusters);

DECLARE_TH_CTX(rtp_ha_11_th_zr_device_ctx, rtp_ha_11_th_zr_device_ep);
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_ha_11_th_zr_device_ctx);
  ...
  switch (sig)
  {
    case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
      ...
      if (status == 0)
      {
        ...
        test_step_register(send_read_attr_req_delayed, 0, RTP_HA_11_STEP_1_TIME_ZR);

        test_control_start(TEST_MODE, RTP_HA_11_STEP_1_DELAY_ZR);
      }
