...
#define ZB_TEST_NAME RTP_ZDO_01_TH_ZR

#define ZB_TRACE_FILE_ID 40415
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_zdo_01_th_zr_basic_attr_list, &attr_zcl_version, &attr_power_source);

/* Identify cluster attributes data */
static zb_uint16_t attr_identify_time = 0;

ZB_ZCL_DECLARE_IDENTIFY_ATTRIB_LIST(rtp_zdo_01_th_zr_identify_attr_list, &attr_identify_time);

/********************* Declare device **************************/
DECLARE_TH_CLUSTER_LIST(rtp_zdo_01_th_zr_device_clusters,
                        rtp_zdo_01_th_zr_basic_attr_list,
                        rtp_zdo_01_th_zr_identify_attr_list);

DECLARE_TH_EP(rtp_zdo_01_th_zr_device_ep,
              THR1_ENDPOINT,
              rtp_zdo_01_th_zr_device_clusters);

DECLARE_TH_CTX(rtp_zdo_01_th_zr_device_ctx, rtp_zdo_01_th_zr_device_ep);
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_zdo_01_th_zr_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_FINDING_AND_BINDING_TARGET_FINISHED:
        ...
        if (status == 0)
        {
            test_step_register(send_read_attr_req_delayed, 0, RTP_ZDO_01_STEP_1_TIME_ZR);
            test_step_register(send_mgmt_lqi_req_delayed, 0, RTP_ZDO_01_STEP_2_TIME_ZR);

            test_control_start(TEST_MODE, RTP_ZDO_01_STEP_1_DELAY_ZR);
        }
...
static void send_mgmt_lqi_req(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_ZDO1, ">>mgmt_lqi_req buf = %d", (FMT__D, param));

    req_param->dst_addr = 0x0000; /* send to zc */
