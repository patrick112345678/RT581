...
RTP_APS_09 - light switch joins to network with several bulb devices and send commands with short interval

Objective:

       To confirm that the light switch does correctly handles missed aps ACK in case of binding transmissions and correctly handles APS Ack that is already not actual.
...
Test steps 8-9 and 10-11 can appear in reverse order 10-11 before 8-9
       ...
       12. DUT ZED receives not actual APS Ack from TH ZR1

       13. DUT ZED sends ZCL On/Off On command to TH ZR1 and TH ZR2

       14. DUT receives APS Ack from TH ZR1 and TH ZR2

       15. DUT ZED sends ZCL On/Off Off command to TH ZR1

       16. DUT ZED receives APS ACK from TH ZR1

       17. DUT ZED sends ZCL On/Off Off command to TH ZR2

       18. DUT ZED does not receive APS ACK from TH ZR2

Test steps 15-16 and 17-18 can appear in reverse order 17-18 before 15-16

       19. DUT ZED receives not actual APS Ack from TH ZR2
...
Trace validation:
    ...
    4. Repeat steps 1-3 three times (messages from the steps 1 and 2 should appear in the DUT's trace exactly 4 times!)
       ...
       5. There is no trace message "TEST FAILED" in the DUT's trace

       6. There is no trace message "Assertion failed" in the DUT's trace
