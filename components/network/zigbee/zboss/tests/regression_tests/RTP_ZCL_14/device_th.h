...
#define TH_DEVICE_ID   0xaa01
#define TH_DEVICE_VER  0  /*!< device version */

#define TH_IN_CLUSTER_NUM 1 /*!< IN (server) clusters number */
#define TH_OUT_CLUSTER_NUM 5 /*!<  OUT (client) clusters number */

#define TH_CLUSTER_NUM (TH_IN_CLUSTER_NUM + \
                         TH_OUT_CLUSTER_NUM)
...
#define TEST_SEND_MANUFACTURER_SPECIFIC_CMD_PAYLOAD_REQ(                \
  buffer, dst_addr, dst_addr_mode, dst_ep,                              \
  src_ep, profile, def_resp, cb, data, data_len)                        \
{                                                                       \
  zb_uint8_t* ptr = ZB_ZCL_START_PACKET(buffer);                        \
  ZB_ZCL_CONSTRUCT_SPECIFIC_COMMAND_REQ_FRAME_CONTROL_A(                \
    ptr, ZB_ZCL_FRAME_DIRECTION_TO_SRV,                                 \
    ZB_ZCL_MANUFACTURER_SPECIFIC, (def_resp));                          \
  ZB_ZCL_CONSTRUCT_COMMAND_HEADER_EXT(                                  \
    (ptr), (ZB_ZCL_GET_SEQ_NUM()), (ZB_ZCL_MANUFACTURER_SPECIFIC),      \
    (MANUFACTURER_SPECIFIC_CODE), (MANUFACTURER_SPECIFIC_CMD_ID));      \
  ZB_ZCL_PACKET_PUT_DATA_N(ptr, (data), (data_len));                    \
  ZB_ZCL_FINISH_PACKET((buffer), ptr)                                   \
  ZB_ZCL_SEND_COMMAND_SHORT(                                            \
      buffer, dst_addr, dst_addr_mode, dst_ep, src_ep, profile,         \
      ZB_ZCL_CLUSTER_ID_SCENES, cb);                                    \
}
