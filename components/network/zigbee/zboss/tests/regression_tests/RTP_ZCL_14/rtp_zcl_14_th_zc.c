...
#define ZB_TEST_NAME RTP_ZCL_14_TH_ZC

#define ZB_TRACE_FILE_ID 40353
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_zcl_14_th_zc_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_TH_CLUSTER_LIST(rtp_zcl_14_th_zc_device_clusters,
                        rtp_zcl_14_th_zc_basic_attr_list);

DECLARE_TH_EP(rtp_zcl_14_th_zc_device_ep,
              TH_ENDPOINT,
              rtp_zcl_14_th_zc_device_clusters);

DECLARE_TH_CTX(rtp_zcl_14_th_zc_device_ctx, rtp_zcl_14_th_zc_device_ep);
/*************************************************************************/
...
static void send_manufacturer_specific_req_delayed(zb_uint8_t unused);
static void send_manufacturer_specific_req(zb_uint8_t param);

static zb_uint8_t g_data[MANUFACTURER_SPECIFIC_CMD_PAYLOAD_LENGTH] = { 0xaa, 0x01, 0x0D, 0x00 };
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_zcl_14_th_zc_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_FINDING_AND_BINDING_INITIATOR_FINISHED:
        ...
        if (status == 0)
        {
            test_step_register(send_add_group_req_delayed, 0, RTP_ZCL_14_STEP_1_TIME_ZC);
            test_step_register(send_manufacturer_specific_req_delayed, 0, RTP_ZCL_14_STEP_2_TIME_ZC);

            test_control_start(TEST_MODE, RTP_ZCL_14_STEP_1_DELAY_ZC);
        }
...
static void send_manufacturer_specific_req_delayed(zb_uint8_t unused)
{
    ...
    zb_buf_get_out_delayed(send_manufacturer_specific_req);
}
...
static void send_manufacturer_specific_req(zb_uint8_t param)
{
    ...
    TEST_SEND_MANUFACTURER_SPECIFIC_CMD_PAYLOAD_REQ(param,
            group_id,
            ...
            ZB_ZCL_ENABLE_DEFAULT_RESPONSE,
            NULL,
            g_data, MANUFACTURER_SPECIFIC_CMD_PAYLOAD_LENGTH);
}
