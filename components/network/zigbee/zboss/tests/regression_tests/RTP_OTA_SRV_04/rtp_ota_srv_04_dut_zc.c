...
#define ZB_TEST_NAME RTP_OTA_SRV_04_DUT_ZC
#define ZB_TRACE_FILE_ID 40317
...
#include "rtp_ota_srv_04_dut_zc.h"
#include "rtp_ota_srv_04_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_ota_srv_04_dut_zc_basic_attr_list, &g_attr_zcl_version, &g_attr_power_source);
...
ZB_ZCL_DECLARE_OTA_UPGRADE_ATTRIB_LIST_SERVER(rtp_ota_srv_04_dut_zc_ota_upgrade_attr_list, &query_jitter, &current_time, 1);

/********************* Declare device **************************/

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_CLUSTER_LIST(rtp_ota_srv_04_dut_zc_ota_upgrade_server_clusters,
        rtp_ota_srv_04_dut_zc_basic_attr_list, rtp_ota_srv_04_dut_zc_ota_upgrade_attr_list);

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_EP(rtp_ota_srv_04_dut_zc_ota_upgrade_server_ep, ENDPOINT, rtp_ota_srv_04_dut_zc_ota_upgrade_server_clusters);

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_CTX(rtp_ota_srv_04_dut_zc_ota_upgrade_server_ctx, rtp_ota_srv_04_dut_zc_ota_upgrade_server_ep);
...
static zb_uint8_t g_image_block_requests_count = 0;
...
MAIN()
{
    ...
    /****************** Register Device ********************************/
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_ota_srv_04_dut_zc_ota_upgrade_server_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ...
        if (status == 0)
        {
            test_step_register(insert_ota_file, 0, RTP_OTA_SRV_04_STEP_1_TIME_ZC);

            test_control_start(TEST_MODE, RTP_OTA_SRV_04_STEP_1_DELAY_ZC);
        }
...
static void remove_ota_file(zb_uint8_t param)
{
    TRACE_MSG(TRACE_ZCL1, "> remove_ota_file %hd", (FMT__H, param));
    ...
    else
    {
        ...
        ZB_ZCL_OTA_UPGRADE_REMOVE_FILE(param, ENDPOINT, 0, ret);

        ZB_ASSERT(ret == RET_OK);
    }

    TRACE_MSG(TRACE_ZCL1, "< remove_ota_file", (FMT__0));
}
...
static zb_uint8_t test_data_indication_cb(zb_uint8_t param)
{
    ...
    if (ind->profileid == ZB_AF_HA_PROFILE_ID)
    {
        ...
        if (cmd_info.cmd_id == ZB_ZCL_CMD_OTA_UPGRADE_IMAGE_BLOCK_ID)
        {
            g_image_block_requests_count++;

            if (g_image_block_requests_count == 2)
            {
                ZB_SCHEDULE_CALLBACK(remove_ota_file, 0);
            }
        }
    }
