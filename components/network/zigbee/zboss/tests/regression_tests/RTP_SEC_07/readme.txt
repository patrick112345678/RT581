R22 - Security failure
RTP_SEC_07 - device does not stay on the network after unsuccessful TK decryption 

Objective:

    To confirm that end device will not stay on the network after unsuccessful TK decryption
...
Initial conditions:
    ...
    2. DUT ZED is built without using IC policy
    3. TH ZC1 is built with using IC policy (to disallow DUT ZED authorization)
    4. TH ZC2 is built without using IC policy (to allow DUT ZED authorization)
