...
#define ZB_TEST_NAME RTP_ZDO_07_TH_ZC
...
static zb_bool_t g_dut_assoc_done = ZB_FALSE;
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ...
        if (status == 0)
        {
            if (g_dut_assoc_done)
            {
                test_step_register(test_send_mgmt_leave_req, 0, RTP_ZDO_07_STEP_1_TIME_ZC);
                test_control_start(TEST_MODE, RTP_ZDO_07_STEP_1_DELAY_ZC);
            }

            g_dut_assoc_done = ZB_TRUE;
        }
