...
#define ZB_TEST_NAME RTP_APS_08_DUT_ZED
#define ZB_TRACE_FILE_ID 40446
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_08_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_08_dut_zed_device_clusters,
                        rtp_aps_08_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_08_dut_zed_device_ep,
              DUT_ENDPOINT,
              rtp_aps_08_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_08_dut_zed_device_ctx, rtp_aps_08_dut_zed_device_ep);
...
static void print_zr2_addr_lock_count(zb_uint8_t unused);

static zb_bool_t g_double_ack_is_received = ZB_FALSE;
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_08_dut_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    ...
    case ZB_BDB_SIGNAL_STEERING:
      ...
      if (status == 0)
      {
        test_step_register(find_light_bulb_delayed, 0, RTP_APS_08_STEP_1_TIME_ZED);
        test_step_register(light_control_send_on_off_toggle_delayed, 0, RTP_APS_08_STEP_2_TIME_ZED);

        test_control_start(TEST_MODE, RTP_APS_08_STEP_1_DELAY_ZED);
      }
...
static void light_control_send_on_off_toggle_cb(zb_uint8_t unused)
{
  ...
  print_zr2_addr_lock_count(0);
  g_double_ack_is_received = ZB_TRUE;

  ZB_SCHEDULE_ALARM(print_zr2_addr_lock_count, 0, TEST_TIME_UNTIL_LOCK_COUNT_PRINTING);
...
static void print_zr2_addr_lock_count(zb_uint8_t unused)
{
  ...
  zb_ret_t status;
  zb_uint8_t lock_count;
  ZVUNUSED(unused);

  status = zb_address_by_ieee(g_ieee_addr_th_zr2, ZB_FALSE, ZB_FALSE, &addr_ref);

  if (status == RET_OK)
  {
    lock_count = ZG->addr.addr_map[addr_ref].lock_cnt;

    if (g_double_ack_is_received)
    {
      TRACE_MSG(TRACE_APP1, "TH ZR2 addr lock count after second ack %hd", (FMT__H, lock_count));
    }
    else
    {
      TRACE_MSG(TRACE_APP1, "TH ZR2 addr lock count before second ack %hd", (FMT__H, lock_count));
    }
  }
}
