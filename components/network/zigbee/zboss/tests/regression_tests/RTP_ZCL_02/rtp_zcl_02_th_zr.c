...
#define ZB_TEST_NAME RTP_ZCL_02_TH_ZR

#define ZB_TRACE_FILE_ID 40463
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_zcl_02_th_zr_basic_attr_list, &attr_zcl_version, &attr_power_source);

/* Identify cluster attributes data */
static zb_uint16_t attr_identify_time = 0;

ZB_ZCL_DECLARE_IDENTIFY_ATTRIB_LIST(rtp_zcl_02_th_zr_identify_attr_list, &attr_identify_time);

/********************* Declare device **************************/
DECLARE_TH_CLUSTER_LIST(rtp_zcl_02_th_zr_device_clusters,
                        rtp_zcl_02_th_zr_basic_attr_list,
                        rtp_zcl_02_th_zr_identify_attr_list);

DECLARE_TH_EP(rtp_zcl_02_th_zr_device_ep,
              THR1_ENDPOINT,
              rtp_zcl_02_th_zr_device_clusters);

DECLARE_TH_CTX(rtp_zcl_02_th_zr_device_ctx, rtp_zcl_02_th_zr_device_ep);

static void trigger_fb_target(zb_uint8_t unused);
static void send_write_attr_delayed(zb_uint8_t unused);
static void send_write_attr(zb_uint8_t param);
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_zcl_02_th_zr_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    ...
    case ZB_BDB_SIGNAL_FINDING_AND_BINDING_TARGET_FINISHED:
      ...
      if (status == 0)
      {
        test_step_register(send_write_attr_delayed, 0, RTP_ZCL_02_STEP_1_TIME_ZR);

        test_control_start(TEST_MODE, RTP_ZCL_02_STEP_1_DELAY_ZR);
      }
...
static void send_write_attr_delayed(zb_uint8_t unused)
{
  ...
  zb_buf_get_out_delayed(send_write_attr);
}
