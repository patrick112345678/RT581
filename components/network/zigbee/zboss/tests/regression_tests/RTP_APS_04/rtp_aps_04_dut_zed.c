...
#define ZB_TEST_NAME RTP_APS_04_DUT_ZED

#define ZB_TRACE_FILE_ID 40359
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_04_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_04_dut_zed_device_clusters,
                        rtp_aps_04_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_04_dut_zed_device_ep,
              DUT_ENDPOINT,
              rtp_aps_04_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_04_dut_zed_device_ctx, rtp_aps_04_dut_zed_device_ep);
...
static void light_control_send_on_off_delayed(zb_bufid_t unused);
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_04_dut_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    ...
    case ZB_BDB_SIGNAL_STEERING:
      ...
      if (status == 0)
      {
        test_step_register(find_light_bulb_delayed, 0, RTP_APS_04_STEP_1_TIME_ZED);
        test_step_register(light_control_send_on_off_delayed, 0, RTP_APS_04_STEP_2_TIME_ZED);

        test_control_start(TEST_MODE, RTP_APS_04_STEP_1_DELAY_ZED);
      }
...
static void light_control_send_on_off(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP2, ">> light_control_send_on_off, param %hd", (FMT__H, param));
  ...
  TRACE_MSG(TRACE_APP2, "<< light_control_send_on_off", (FMT__0));
}
...
static void light_control_send_on_off_cb(zb_bufid_t param)
{
  ...
  if (cmd_send_status->status == ERROR_CODE(ERROR_CATEGORY_APS, ZB_APS_STATUS_NO_BOUND_DEVICE))
  {
    TRACE_MSG(TRACE_APP1, "ZB_APS_STATUS_NO_BOUND_DEVICE - test OK", (FMT__0));
  }
