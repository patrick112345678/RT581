...
#define ZB_TEST_NAME RTP_OTA_SRV_02_DUT_ZC

#define ZB_TRACE_FILE_ID 40403
//#include "zboss_api.h"
...
#include "rtp_ota_srv_02_dut_zc.h"
#include "rtp_ota_srv_02_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_ota_srv_02_dut_zc_basic_attr_list, &g_attr_zcl_version, &g_attr_power_source);
...
ZB_ZCL_DECLARE_OTA_UPGRADE_ATTRIB_LIST_SERVER(rtp_ota_srv_02_dut_zc_ota_upgrade_attr_list, &query_jitter, &current_time, 1);

/********************* Declare device **************************/

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_CLUSTER_LIST(rtp_ota_srv_02_dut_zc_ota_upgrade_server_clusters,
          rtp_ota_srv_02_dut_zc_basic_attr_list, rtp_ota_srv_02_dut_zc_ota_upgrade_attr_list);

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_EP(rtp_ota_srv_02_dut_zc_ota_upgrade_server_ep, ENDPOINT, rtp_ota_srv_02_dut_zc_ota_upgrade_server_clusters);

ZB_HA_DECLARE_OTA_UPGRADE_SERVER_CTX(rtp_ota_srv_02_dut_zc_ota_upgrade_server_ctx, rtp_ota_srv_02_dut_zc_ota_upgrade_server_ep);
...
static void test_zcl_device_cb(zb_uint8_t param);
...
MAIN()
{
  ...
  /****************** Register Device ********************************/
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_ota_srv_02_dut_zc_ota_upgrade_server_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch(sig)
  {
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
      ...
      if (status == 0)
      {
        test_step_register(insert_ota_file, 0, RTP_OTA_SRV_02_STEP_1_TIME_ZC);

        test_control_start(TEST_MODE, RTP_OTA_SRV_02_STEP_1_DELAY_ZC);
      }
...
static zb_zcl_ota_upgrade_server_variable_t * test_get_upgrade_server_variables(zb_uint8_t endpoint)
{
...
static void test_zcl_device_cb(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "> test_zcl_device_cb, param %hd", (FMT__H, param));

  if (device_cb_param->device_cb_id == ZB_ZCL_OTA_UPGRADE_SRV_QUERY_IMAGE_CB_ID)
  {
    TRACE_MSG(TRACE_APP1, "test_zcl_device_cb(): ZB_ZCL_OTA_UPGRADE_SRV_QUERY_IMAGE_CB_ID", (FMT__0));

    vars = test_get_upgrade_server_variables(ENDPOINT);
    ...
    for(i = 0; i < vars->table_length; i++)
    {
      ...
      {
        if( value->manufacturer_code == vars->table[i].file_header->manufacturer_code &&
            value->image_type == vars->table[i].file_header->image_type &&
            ZB_ZCL_OTA_UPGRADE_VERSION_CMP(vars->table[i].file_header->file_version,
                                           value->version))
        {
          TRACE_MSG(TRACE_APP1, "test_zcl_device_cb(): manufacturer_code = 0x%x - test OK",
                    (FMT__D, value->manufacturer_code));
          ret = RET_OK;
          *(value->table_idx) = i;
          break;
        }
  ...
  device_cb_param->status = ret;

  TRACE_MSG(TRACE_APP1, "< test_zcl_device_cb, ret %hd", (FMT__H, ret));
}
