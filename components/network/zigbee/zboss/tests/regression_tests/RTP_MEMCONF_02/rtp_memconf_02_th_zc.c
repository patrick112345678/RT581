...
/* PURPOSE: TH ZC
*/
#define ZB_TEST_NAME RTP_MEMCONF_02_TH_ZC

#define ZB_TRACE_FILE_ID 40383
#include "zb_common.h"
...
#include "rtp_memconf_02_common.h"
...
static void test_send_mgmt_leave_req_delayed(zb_uint8_t unused);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
      ...
      if (status == 0)
      {
        test_step_register(test_send_mgmt_leave_req_delayed, 0, RTP_MEMCONF_02_STEP_1_TIME_ZC);
        test_control_start(TEST_MODE, RTP_MEMCONF_02_STEP_1_DELAY_ZC);
      }
...
static void test_send_mgmt_leave_req(zb_uint8_t param)
{
  ...
  req->dst_addr = zb_address_short_by_ieee(g_ieee_addr_dut_zed);
