...
#define ZB_TEST_NAME RTP_APS_02_DUT_ZED

#define ZB_TRACE_FILE_ID 40475
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_02_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_02_dut_zed_device_clusters,
                         rtp_aps_02_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_02_dut_zed_device_ep,
               DUT_ENDPOINT,
               rtp_aps_02_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_02_dut_zed_device_ctx, rtp_aps_02_dut_zed_device_ep);
...
static void send_on_commands_by_bindings(zb_uint8_t unused);
static void light_control_send_on_off(zb_uint8_t param);
static void light_control_send_on_off_cb(zb_uint8_t param);

static void test_bindings_tables_after_all_responses(zb_uint8_t unused);
static void test_binding_trans_index(zb_uint8_t param);
static void test_errors_manager(zb_uint8_t unused);
...
static zb_uint8_t  g_test_error_count = 0;
static zb_uint16_t g_test_resp_count = 0;
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_02_dut_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
        TRACE_MSG(TRACE_APP1, "Device started, status %hd", (FMT__H, status));
        if (status == 0)
        {
    ...
    case ZB_BDB_SIGNAL_STEERING:
        TRACE_MSG(TRACE_APS1, "signal: ZB_BDB_SIGNAL_STEERING, status %hd", (FMT__H, status));
        if (status == 0)
        {
            test_step_register(find_light_bulb_delayed, 0, RTP_APS_02_STEP_1_TIME_ZED);
            test_step_register(send_on_commands_by_bindings, 0, RTP_APS_02_STEP_2_TIME_ZED);

            test_control_start(TEST_MODE, RTP_APS_02_STEP_1_DELAY_ZED);
        }
    ...
    default:
        TRACE_MSG(TRACE_APP1, "Unknown signal %hd, status %hd", (FMT__D_H, sig, status));
        break;
    }
...
static void find_light_bulb_cb(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, "<< find_light_bulb_cb", (FMT__0));
}
...
static void send_on_commands_by_bindings(zb_uint8_t unused)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> send_on_commands_by_bindings", (FMT__0));

    ZVUNUSED(unused);

    if (g_test_bound_bulb_count >= TEST_DUT_BULB_COUNT)
    {
        for (i = 0; i < TEST_DUT_ON_OFF_CMD_COUNT; i++)
        {
    ...
    else
    {
        ...
        ZB_SCHEDULE_ALARM(send_on_commands_by_bindings, 0, 2 * ZB_TIME_ONE_SECOND);
    }
...
static void light_control_send_on_off(zb_uint8_t param)
{
    zb_uint8_t cmd_id = ZB_ZCL_CMD_ON_OFF_ON_ID;
    zb_uint16_t addr = 0;
...
static void light_control_send_on_off_cb(zb_uint8_t param)
{
    TRACE_MSG(TRACE_APP1, " >> light_control_send_on_off_cb, param = %hd", (FMT__H, param));

    g_test_resp_count++;

    test_binding_trans_index(param);

    if (g_test_resp_count == TEST_DUT_ON_OFF_CMD_COUNT)
    {
        ZB_SCHEDULE_CALLBACK(test_bindings_tables_after_all_responses, 0);
        ZB_SCHEDULE_CALLBACK(test_errors_manager, 0);
    }

    TRACE_MSG(TRACE_APP1, "<< light_control_send_on_off_cb", (FMT__0));
...
static void test_binding_trans_index(zb_uint8_t param)
{
    ...
    for (i = 0; i < ZB_APS_BIND_TRANS_TABLE_SIZE; i++)
    {
        ...
        {
            TRACE_MSG(TRACE_ERROR, "found binding transmission entry, index %hd for buffer %hd", (FMT__H_H, i, param));
            g_test_error_count++;
        }
    }
#else
...
static void test_bindings_tables_after_all_responses(zb_uint8_t unused)
{
    ...
    ZVUNUSED(unused);
    TRACE_MSG(TRACE_APP1, ">> test_bindings_tables_after_all_responses", (FMT__0));

    for (i = 0; i < ZB_APS_BIND_TRANS_TABLE_SIZE; i++)
    {
        if (ZG->aps.binding.trans_table[i] != 0)
        {
            TRACE_MSG(TRACE_ERROR, "Binding transmission entry with index %hd for buffer %hd not removed", (FMT__H_H, i, ZG->aps.binding.trans_table[i]));
            g_test_error_count++;
        }
    }
    ...
    {
        for (j = 0; j < ZB_APS_BIND_TRANS_TABLE_SIZE; j++)
        {
            if (ZB_APS_IS_TRANS_INDEX(i, j))
            {
                TRACE_MSG(TRACE_ERROR, "Trans index for dtbli = %hd, tansi = %hd not empty", (FMT__H_H, i, j));
                g_test_error_count++;
            }
        }
    }
...
static void test_errors_manager(zb_uint8_t unused)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> test_errors_manager", (FMT__0));

    if (g_test_error_count)
    {
        TRACE_MSG(TRACE_APP1, "test_errors_manager: TEST FAILED", (FMT__0));
    }
