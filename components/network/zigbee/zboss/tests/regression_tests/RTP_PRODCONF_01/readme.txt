RTP_PRODCONF_01 - Production config application.

Objective:

    To confirm that production config applied correctly after ZB_ZDO_SIGNAL_PRODUCTION_CONFIG_READY returned and device is alive.
...
Initial conditions:
    ...
    2. APP trace level shall be enabled.
    3. Production config shall loaded to flash address 0xC8000
    3.1 Production config binary shall be generated from prodconfig_01_dut_zed.txt or prodconfig_01_dut_zed_ncp.txt
...
Test procedure:
    ...
    1. Power on TH ZC then DUT ZED
    2. Wait for 30 seconds for devices to complete writing trace
