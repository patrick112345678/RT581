...
#define ZB_TEST_NAME RTP_APS_07_DUT_ZED
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_07_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_07_dut_zed_device_clusters,
                         rtp_aps_07_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_07_dut_zed_device_ep,
               DUT_ENDPOINT,
               rtp_aps_07_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_07_dut_zed_device_ctx, rtp_aps_07_dut_zed_device_ep);
...
static void test_init_bind_addr_ctx(void);
...
static void test_binding_lock_table(void);
static void set_binding_lock_table(void);

typedef struct
{
    zb_bitfield_t lock_cnt;
    zb_ieee_addr_t ieee_addr;
} test_bind_addr_t;

static zb_bool_t  g_bulb_send_on_cmd      = ZB_TRUE;
static zb_bool_t  g_test_is_first_start   = ZB_TRUE;
static zb_uint8_t g_test_bound_bulb_count = 0;
static zb_uint8_t g_test_bound_wait_count = 0;
...
static test_bind_addr_t g_bind_addrs[TEST_DUT_BULB_COUNT];
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_07_dut_zed_device_ctx);
    test_init_bind_addr_ctx();
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_STEERING:
        ...
        if (status == 0)
        {
            if (g_test_is_first_start)
            {
                test_step_register(find_light_bulb_delayed, 0, RTP_APS_07_STEP_1_TIME_ZED);
                test_step_register(light_control_send_on_off_delayed, 0, RTP_APS_07_STEP_2_TIME_ZED);

                test_control_start(TEST_MODE, RTP_APS_07_STEP_1_DELAY_ZED);

                g_test_is_first_start = ZB_FALSE;
            }
        }
...
static void test_init_bind_addr_ctx(void)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> test_init_bind_addr_ctx", (FMT__0));

    for (i = 0; i < ZB_ARRAY_SIZE(g_bind_addrs); i++)
    {
        g_bind_addrs[i].lock_cnt = 0;
        ZB_IEEE_ADDR_COPY(g_bind_addrs[i].ieee_addr, g_zero_addr);
    }
...
static void find_light_bulb_cb(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, ">> find_light_bulb_cb param %hd, status %hd, resp match_len %hd", (FMT__H_H_H, param, resp->status, resp->match_len));

    if (resp->status == ZB_ZDP_STATUS_SUCCESS &&
            resp->match_len > 0 &&
            g_test_bound_bulb_count < TEST_DUT_BULB_COUNT)
    {
...
static void bulb_ieee_addr_req_cb(zb_uint8_t param)
{
    ...
    if (resp->status == ZB_ZDP_STATUS_SUCCESS && g_test_bound_bulb_count < TEST_DUT_BULB_COUNT)
    {
        ...
        ZB_IEEE_ADDR_COPY(g_bind_addrs[g_test_bound_bulb_count].ieee_addr, resp->ieee_addr);
        g_test_bound_bulb_count++;
    }
    else
    {
        zb_buf_free(param);
...
static void light_control_send_on_off_delayed(zb_uint8_t unused)
{
    ...
    if (g_test_bound_bulb_count >= TEST_DUT_BULB_COUNT ||
            g_test_bound_wait_count >= TEST_DUT_WAIT_BULB_COUNT)
    {
        g_test_bound_bulb_count = TEST_DUT_BULB_COUNT;
        set_binding_lock_table();

        zb_buf_get_out_delayed(light_control_send_on_off);
    }
    else
    {
        TRACE_MSG(TRACE_APP1, "Not all bulbs bound yet. Wait some time.", (FMT__0));
        ZB_SCHEDULE_ALARM(light_control_send_on_off_delayed, 0, 5 * ZB_TIME_ONE_SECOND);
        g_test_bound_wait_count++;
    }
...
static void light_control_send_on_off_cb(zb_bufid_t param)
{
    ...
    TRACE_MSG(TRACE_APP1, "light_control_send_on_off_cb, status %hd", (FMT__H, cmd_send_status->status));

    test_binding_lock_table();

    ZB_SCHEDULE_APP_ALARM(light_control_send_on_off_delayed, 0, TEST_COMMAND_INTERVAL);

    zb_buf_free(param);
}

static void set_binding_lock_table()
{
    zb_ret_t ret;
    zb_uindex_t i;
    zb_address_ieee_ref_t addr_ref = 0;

    TRACE_MSG(TRACE_APP1, ">> set_binding_lock_table", (FMT__0));

    i = 0;
    while (!g_bind_addrs[i].lock_cnt &&
            !ZB_IEEE_ADDR_IS_ZERO(g_bind_addrs[i].ieee_addr) &&
            i < TEST_DUT_BULB_COUNT)
    {
        ret = zb_address_by_ieee(g_bind_addrs[i].ieee_addr, ZB_FALSE, ZB_FALSE, &addr_ref);
        if (ret != RET_OK)
        {
            TRACE_MSG(TRACE_APP1, "TEST ERROR, no such address, ret = %d, addr = " TRACE_FORMAT_64, (FMT__H, ret, TRACE_ARG_64(g_bind_addrs[i].ieee_addr)));

        }
        else if (ZB_U2B(ZG->addr.addr_map[addr_ref].used))
        {
            TRACE_MSG(TRACE_APP1, "INFO, lock_cnt = %hd, ieee_addr: " TRACE_FORMAT_64,
                      (FMT__H_A, ZG->addr.addr_map[addr_ref].lock_cnt,
                       TRACE_ARG_64(g_bind_addrs[i].ieee_addr)));

            g_bind_addrs[i].lock_cnt = ZG->addr.addr_map[addr_ref].lock_cnt;
        }
        i++;
    }

    TRACE_MSG(TRACE_APP1, "<< set_binding_lock_table, last index %hd", (FMT__H, i));
}

static void test_binding_lock_table()
{
    zb_ret_t ret;
    zb_uindex_t i;
    zb_address_ieee_ref_t addr_ref = 0;

    TRACE_MSG(TRACE_APP1, ">> test_binding_lock_table", (FMT__0));

    i = 0;
    while (!ZB_IEEE_ADDR_IS_ZERO(g_bind_addrs[i].ieee_addr) &&
            i < TEST_DUT_BULB_COUNT)
    {
        ret = zb_address_by_ieee(g_bind_addrs[i].ieee_addr, ZB_FALSE, ZB_FALSE, &addr_ref);
        ...
        if (ret != RET_OK)
        {
            TRACE_MSG(TRACE_APP1, "TEST ERROR, ret = %d, addr = " TRACE_FORMAT_64, (FMT__H_A, ret, TRACE_ARG_64(g_bind_addrs[i].ieee_addr)));
        }
        else if (ZG->addr.addr_map[addr_ref].lock_cnt != g_bind_addrs[i].lock_cnt)
        {
            TRACE_MSG(TRACE_APP1, "TEST ERROR, lock_cnt not the same, lock_cnt = %hd, old_lock_cnt = %hd, ieee_addr: " TRACE_FORMAT_64,
                      (FMT__H_H_A, ZG->addr.addr_map[addr_ref].lock_cnt, g_bind_addrs[i].lock_cnt,
                       TRACE_ARG_64(g_bind_addrs[i].ieee_addr)));
        }
    ...
    TRACE_MSG(TRACE_APP1, "<< test_binding_lock_table, last index %hd", (FMT__H, i));
}
