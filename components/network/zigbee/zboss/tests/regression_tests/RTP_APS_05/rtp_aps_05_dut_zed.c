...
#define ZB_TEST_NAME RTP_APS_05_DUT_ZED
#define ZB_TRACE_FILE_ID 40385
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_05_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_05_dut_zed_device_clusters,
                         rtp_aps_05_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_05_dut_zed_device_ep,
               DUT_ENDPOINT,
               rtp_aps_05_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_05_dut_zed_device_ctx, rtp_aps_05_dut_zed_device_ep);
...
static void light_control_send_command_alarm(zb_uint8_t param);
static void light_control_send_command_alarm_delayed(zb_uint8_t unused);
static void light_control_send_on_off(zb_uint8_t param, zb_bool_t on_off);
static void light_control_send_step(zb_uint8_t param, zb_bool_t dir);

static void start_send_commands_with_next_interval(zb_uint8_t unused);
static void light_control_send_cmd_cb(zb_uint8_t param);
...
static zb_time_t g_commands_intervals[] =
{
    TEST_COMMAND_INTERVAL_1,
    TEST_COMMAND_INTERVAL_2
};

/* Determines whether we should turn on or off bulbs */
static zb_bool_t g_bulb_send_on_cmd = ZB_TRUE;
/* Determines whether we should step up or step down bulbs */
static zb_bool_t g_bulb_send_step_up_cmd = ZB_TRUE;
/* Determines whether we should send OnOff or Level Control command */
static zb_bool_t g_bulb_send_on_off_cmd = ZB_TRUE;

static zb_uint8_t g_bulb_commands_to_send_count = 0;
static zb_uint8_t g_bulb_commands_current_interval = -1;
static zb_uint8_t g_found_bulbs = 0;
...
MAIN()
{
    ...
    ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_05_dut_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_BDB_SIGNAL_STEERING:
        ...
        if (status == 0)
        {
            test_step_register(start_send_commands_with_next_interval, 0, RTP_APS_05_STEP_1_TIME_ZED);
            test_step_register(start_send_commands_with_next_interval, 0, RTP_APS_05_STEP_2_TIME_ZED);

            test_control_start(TEST_MODE, RTP_APS_05_STEP_1_DELAY_ZED);
        }
...
static void light_control_bind_bulb(zb_uint8_t param)
{
    ...
    if (next_cluster_id == BIND_CLUSTER_ID_DEFAULT)
    {
        ...
        g_found_bulbs++;
        if (g_found_bulbs == TEST_BULB_DEV_NUM)
        {
            ZB_SCHEDULE_CALLBACK(start_send_commands_with_next_interval, 0);
        }
    }
    else
    {
...
static void light_control_send_command_alarm(zb_uint8_t param)
{
    if (g_bulb_send_on_off_cmd)
    {
        light_control_send_on_off(param, g_bulb_send_on_cmd);
        g_bulb_send_on_cmd = !g_bulb_send_on_cmd;
    }
    else
    {
        light_control_send_step(param, g_bulb_send_step_up_cmd);
        g_bulb_send_step_up_cmd = !g_bulb_send_step_up_cmd;
    }
}
...
static void light_control_send_command_alarm_delayed(zb_uint8_t unused)
{
    ...
    zb_buf_get_out_delayed(light_control_send_command_alarm);
}
...
static void start_send_commands_with_next_interval(zb_uint8_t unused)
{
    ...
    if (!g_found_bulbs)
    {
        /* No bulb devices found yet. Find it. */
        ZB_SCHEDULE_CALLBACK(find_light_bulb_delayed, 0);
    }
    else if (g_bulb_commands_to_send_count)
    {
        /* Previous step is in progress. Wait. */
        ZB_SCHEDULE_ALARM(start_send_commands_with_next_interval, 0, 5 * ZB_TIME_ONE_SECOND);
    }
    else
    {
        /* Start sending commands. */
        g_bulb_commands_current_interval++;
        g_bulb_commands_to_send_count = TEST_COMMANDS_TO_CHANGE_INTERVAL;

        ZB_SCHEDULE_CALLBACK(light_control_send_command_alarm_delayed, 0);
    }
}
...
{
    zb_bufid_t buf = param;
    zb_uint8_t step_dir = (dir) ? (ZB_ZCL_LEVEL_CONTROL_STEP_MODE_UP) : (ZB_ZCL_LEVEL_CONTROL_STEP_MODE_DOWN);
    zb_uint16_t addr = 0;
    ...
    ZB_ZCL_LEVEL_CONTROL_SEND_STEP_REQ(
        ...
        light_control_send_cmd_cb,
        step_dir,
        LIGHT_CONTROL_DIMM_STEP,
        LIGHT_CONTROL_DIMM_TRANSACTION_TIME);
}

static void light_control_send_cmd_cb(zb_uint8_t param)
{
    g_bulb_commands_to_send_count--;
    g_bulb_send_on_off_cmd = !g_bulb_send_on_off_cmd;

    if (g_bulb_commands_to_send_count > 0)
    {
        ZB_SCHEDULE_APP_ALARM(light_control_send_command_alarm_delayed, 0, g_commands_intervals[g_bulb_commands_current_interval]);
    }
