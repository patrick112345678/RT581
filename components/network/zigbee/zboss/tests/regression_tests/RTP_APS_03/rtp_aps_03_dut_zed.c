...
#define ZB_TEST_NAME RTP_APS_03_DUT_ZED

#define ZB_TRACE_FILE_ID 40343
#include "zb_common.h"
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(rtp_aps_03_dut_zed_basic_attr_list, &attr_zcl_version, &attr_power_source);

/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(rtp_aps_03_dut_zed_device_clusters,
                        rtp_aps_03_dut_zed_basic_attr_list);

DECLARE_DUT_EP(rtp_aps_03_dut_zed_device_ep,
              DUT_ENDPOINT,
              rtp_aps_03_dut_zed_device_clusters);

DECLARE_DUT_CTX(rtp_aps_03_dut_zed_device_ctx, rtp_aps_03_dut_zed_device_ep);
...
static void binding_fill_table_over_capacity(zb_bufid_t unused);
...
static void send_light_on_cmd_when_table_empty(zb_uint8_t unused);

static zb_uint8_t g_test_on_off_cb_count = 0;
static zb_bool_t g_bulb_on_off = ZB_TRUE;
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&rtp_aps_03_dut_zed_device_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
      ...
      if (status == 0)
      {
        TRACE_MSG(TRACE_APP1, "Trans binding table size is %d", (FMT__D, ZB_APS_BIND_TRANS_TABLE_SIZE));
    ...
    case ZB_BDB_SIGNAL_STEERING:
      ...
      if (status == 0)
      {
        test_step_register(find_light_bulb_delayed, 0, RTP_APS_03_STEP_1_TIME_ZED);

        test_control_start(TEST_MODE, RTP_APS_03_STEP_1_DELAY_ZED);
      }
...
static void light_control_bind_bulb(zb_uint8_t param)
{
  ...
  if (next_cluster_id == BIND_CLUSTER_ID_DEFAULT)
  {
    ...
    ZB_SCHEDULE_CALLBACK(binding_fill_table_over_capacity, 0);
...
static void light_control_send_command_alarm(zb_uint8_t param)
{
  light_control_send_on_off(param, g_bulb_on_off);
}

static void binding_fill_table_over_capacity(zb_bufid_t unused)
{
  zb_uint16_t bulb_send_commands_left_count;
  ZVUNUSED(unused);

  bulb_send_commands_left_count = ZB_APS_BIND_TRANS_TABLE_SIZE + 1;
  ...
  if (g_test_bound_bulb_count >= TEST_DUT_BULB_COUNT)
  {
    ...
    while(--bulb_send_commands_left_count);
  }
  else
  {
    TRACE_MSG(TRACE_APP1, "binding_fill_table_over_capacity, bulb_count %hd",
              (FMT__H, g_test_bound_bulb_count));
  }
}
...
static void light_control_send_on_off_cb(zb_bufid_t param)
{
  ...
  if (cmd_send_status->status == RET_TABLE_FULL)
  {
    TRACE_MSG(TRACE_APP1, "RET_TABLE_FULL - test OK", (FMT__0));
  }
  else if (cmd_send_status->status == RET_OK)
  {
    TRACE_MSG(TRACE_APP1, "RET_OK - successfully got response", (FMT__0));
  }

  g_test_on_off_cb_count++;
  if (g_test_on_off_cb_count == ZB_APS_BIND_TRANS_TABLE_SIZE + 1)
  {
    ZB_SCHEDULE_ALARM(send_light_on_cmd_when_table_empty, 0, ZB_TIME_ONE_SECOND);
  }
...
static zb_bool_t check_table_empty()
{
#ifndef NCP_MODE_HOST
  zb_bool_t empty = ZB_TRUE;
  zb_uindex_t i;
  zb_uindex_t j;
  ...
  for (i = 0; i < ZB_APS_BIND_TRANS_TABLE_SIZE; i++)
  {
    ...
    {
      empty = ZB_FALSE;
      TRACE_MSG(TRACE_APP1, "Binding transmission entry with index %hd for buffer %hd not removed", (FMT__H_H, i, ZG->aps.binding.trans_table[i]));
      break;
    }
  }
  ...
  for (i = 0; i < ZG->aps.binding.dst_n_elements; i++)
  {
    for (j = 0; j < ZB_APS_BIND_TRANS_TABLE_SIZE; j++)
    {
      ...
      {
        empty = ZB_FALSE;
        TRACE_MSG(TRACE_APP1, "Trans index for dtbli = %hd, tansi = %hd not empty", (FMT__H_H, i, j));
        break;
      }
    }
  }
...
static void send_light_on_cmd_when_table_empty(zb_uint8_t unused)
{
  ...
  {
    TRACE_MSG(TRACE_APP1, "Table is empty, check that messages can be send", (FMT__0));

    g_bulb_on_off = ZB_FALSE;
    zb_buf_get_out_delayed(light_control_send_command_alarm);
  }
  else
  {
    TRACE_MSG(TRACE_APP1, "table cleaning - test FAILED", (FMT__0));
  }
}
