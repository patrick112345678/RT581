...
#define ZB_TEST_NAME RTP_BDB_12_DUT_ZED

#define ZB_TRACE_FILE_ID 40283
#include "zb_common.h"
...
static void trigger_steering(zb_uint8_t unused)
{
  ...
  status = bdb_start_top_level_commissioning(ZB_BDB_NETWORK_STEERING);

  if (!status)
  {
    TRACE_MSG(TRACE_APP1, "can not start bdb top level commissioning during active association", (FMT__0));
  }
}
