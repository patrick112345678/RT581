 ...
 * DSR.
 */
/* PURPOSE: test for ZC application written long APS messages using ZDO.
*/
#define ZB_TRACE_FILE_ID 195
#include "zb_config.h"
...
static void send_frag_data_delayed(zb_bufid_t param)
{
    ...
    zb_buf_get_out_delayed_ext(zc_send_data, 0, ZB_TEST_LONG_APS_MESSAGE_DATA_SIZE + sizeof(zb_apsde_data_req_t));
}
...
MAIN()
{
    ...
    ZB_INIT("zbs_echo_zc_long_aps_message");
...
#ifndef NCP_SDK
        ZB_SCHEDULE_ALARM(zc_send_data, param, ZB_TIME_ONE_SECOND * 3);
#else
...
static void zc_send_data(zb_uint8_t param)
{
    ...
    ptr = zb_buf_initial_alloc(param, ZB_TEST_LONG_APS_MESSAGE_DATA_SIZE);
