 ...
 * DSR.
 */
/* PURPOSE: test for ZED application written long APS messages using ZDO.
*/

#define ZB_TRACE_FILE_ID 104
#include "zb_common.h"
...
{
    ZVUNUSED(param);
    TRACE_MSG(TRACE_APS3, "send_frag_data_delayed", (FMT__0));
    zb_buf_get_out_delayed_ext(send_data, 0, ZB_TEST_LONG_APS_MESSAGE_DATA_SIZE + sizeof(zb_apsde_data_req_t));
}
...
MAIN()
{
    ...
    ZB_INIT("zbs_echo_ze_long_aps_message");
...
static void send_data(zb_bufid_t buf)
{
    ...
    req->tx_options = ZB_APSDE_TX_OPT_SECURITY_ENABLED | ZB_APSDE_TX_OPT_ACK_TX | ZB_APSDE_TX_OPT_FRAG_PERMITTED;
    req->radius = 1;
    req->profileid = 2;
    req->src_endpoint = 10;
    req->dst_endpoint = 10;
    ...
    ptr = zb_buf_initial_alloc(buf, ZB_TEST_LONG_APS_MESSAGE_DATA_SIZE);

    for (i = 0 ; i < ZB_TEST_LONG_APS_MESSAGE_DATA_SIZE ; ++i)
    {
        ptr[i] = i % 32 + '0';
    }
    ...
    {
        ZB_SCHEDULE_APP_ALARM(send_frag_data_delayed, 0, 10 * ZB_TIME_ONE_SECOND);
    }
    else
    {
...
zb_uint8_t data_indication(zb_uint8_t param)
{
    ...
    if (ind->profileid == 0x0002)
    {
        ...
        for (i = 0; i < zb_buf_len(param); ++i)
        {
            ...
            if (ptr[i] != i % 32 + '0')
            {
                /*ZB_SYSTEST_EXIT_ERR("Bad data %hx %c wants %x %c", (FMT__H_C_D_C, ptr[i], ptr[i],
        ...
        if (exchange_finished())
        {
            TRACE_MSG(TRACE_ERROR, "test_exit", (FMT__0));
            ZB_SCHEDULE_ALARM(test_exit, 0, 1 * ZB_TIME_ONE_SECOND);
        }
        zb_buf_free(param);
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_BDB_SIGNAL_DEVICE_REBOOT:
            ...
            {
                ZB_SCHEDULE_APP_ALARM(send_frag_data_delayed, 0, 3 * ZB_TIME_ONE_SECOND);
            }
            break;
        //! [signal_sleep]
