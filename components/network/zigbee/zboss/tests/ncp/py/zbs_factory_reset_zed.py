...
# * DSR.
#
# PURPOSE: Host-side test implements ZED test with factory reset.
#

# ZED makes factory reset on each start.
...
log_file_name = "zbs_factory_reset_zed.log"
...
class Test(BaseTest):
    ...
    def set_rx_on_when_idle_rsp(self, rsp, rsp_len):
        ...
        if self.reset_state == ncp_hl_rst_state_e.HAS_RESET_TO_FACTORY_DEFAULT:
            logger.info("make sure that ZED is not joined right after reset ('joined: False' is expected)")
    ...
    def on_nwk_join_complete(self, body):
        logger.info("make sure that ZED is joined successfully ('joined: True' is expected)")
        self.host.ncp_req_get_joined()
