...
# * DSR.
#
# PURPOSE: Host-side test implements neighbor table iteration by ZED device.
#

# ZED iterate through neighbor table after join.
...
log_file_name = "zbs_nbt_iter_zed.log"
...
class Test(BaseTest):
    ...
    def print_lqi(self, nb, rsp):
        if rsp.hdr.status_code == 0:
            logger.info("the only neighbor (parent) information: {}\n".format(nb))
