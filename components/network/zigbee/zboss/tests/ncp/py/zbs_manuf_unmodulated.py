...
# * DSR.
#
# PURPOSE: Host-side sample for Manufacture mode.
#

# Sample starts from setting NCP to a manufacture mode
# 1. Host sets and gets channel
# 2. Host sets and gets power
# 3. Host starts to transmit unmodulated tone for 5 sec.
# 4. Host stops unmodulated tone.
...
log_file_name = "zbs_manuf_unmodulated.log"
...
class Test(BaseTest):
    def __init__(self, channel_mask):
        ...
        self.update_response_switch({
            ...
            ncp_hl_call_code_e.NCP_HL_MANUF_START_TONE : self.manuf_start_tone_rsp,
            ncp_hl_call_code_e.NCP_HL_MANUF_STOP_TONE : self.manuf_stop_tone_rsp
        })
        ...
        self.required_packets = [[ncp_hl_call_code_e.NCP_HL_NCP_RESET_IND,          NCP_HL_STATUS.RET_OK],
                                 ...
                                 [ncp_hl_call_code_e.NCP_HL_MANUF_START_TONE,       NCP_HL_STATUS.RET_OK],
                                 [ncp_hl_call_code_e.NCP_HL_MANUF_STOP_TONE,        NCP_HL_STATUS.RET_OK],
                                 [ncp_hl_call_code_e.NCP_HL_NCP_RESET,              NCP_HL_STATUS.RET_OK],
                                ]
    ...
    def manuf_get_power_rsp(self, rsp, rsp_len):
        ...
        sleep(1)
        self.host.ncp_req_manuf_start_tone()

    def manuf_start_tone_rsp(self, rsp, rsp_len):
        self.rsp_log(rsp)
        sleep(TONE_DURATION)
        self.host.ncp_req_manuf_stop_tone()

    def manuf_stop_tone_rsp(self, rsp, rsp_len):
        self.rsp_log(rsp)
        sleep(1)
