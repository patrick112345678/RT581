...
# * DSR.
#
# PURPOSE: Host-side test implements ZR part of Key Establishment and checks "ncp_req_partner_lk_enable".
#
...
#
# Note: that test tests Partner LK establishment from our side (NCP_HL_SECUR_START_PARTNER_LK req and resp)
# Partner LK establishment will be tried by remote side but for the test to pass PLKE from remote needs
# to fail (we cannot get NCP_HL_SECUR_PARTNER_LK_FINISHED_IND indication).
#
# In capture initiation of second Partner LK establishment from the remote side test is not a mistake, and it
# must fail in binding step with ZDP_NOT_AUTHORIZED
#
...
log_file_name = "zbs_se_plke_en_dis.log"
...
class Test(BaseTest):
    ...
    def __init__(self, channel_mask):
        ...
        self.update_response_switch({
            ...
            ncp_hl_call_code_e.NCP_HL_SECUR_PARTNER_LK_ENABLE: self.partner_lk_en_rsp
        })
        ...
        self.required_packets = [[ncp_hl_call_code_e.NCP_HL_NCP_RESET_IND,                  NCP_HL_STATUS.RET_OK],
                                 ...
                                 [ncp_hl_call_code_e.NCP_HL_SECUR_PARTNER_LK_ENABLE,        NCP_HL_STATUS.RET_OK],
                                 # At this point remote side will try second partner LK to be established from remote side, we wait
                                 # 10 seconds in "partner_lk_en_rsp" to make sure that it is discarded
                                 ]
    ...
    def partner_lk_rsp(self, rsp, rsp_len):
        ...
        if status_id(rsp.hdr.status_category, rsp.hdr.status_code) == NCP_HL_STATUS.RET_TIMEOUT:
            ...
            logger.debug("PLKE request by us is complete")
            self.host.ncp_req_partner_lk_enable(False)

        # indication of partner link key establishment initiated by other device

    def partner_lk_en_rsp(self, rsp, rsp_len):
        self.rsp_log(rsp)
        # wait 10 seconds before terminating to make sure we do not receive "NCP_HL_SECUR_PARTNER_LK_FINISHED_IND"
        sleep(10)
    ...
    def partner_lk_ind(self, ind, ind_len):
        ...
        # Make the test fail if we receive the PLK from remote side
        logger.error("Partner LK initiatedfrom remote side (monolithic) received after we disabled the acceptance of PLKE.")
