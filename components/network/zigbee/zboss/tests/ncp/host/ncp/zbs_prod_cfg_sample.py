...
class Test(BaseTest):
    ...
    def __init__(self, channel_mask):     
        ...
        self.required_packets = [ncp_hl_call_code_e.NCP_HL_NCP_RESET,
                                 ...
                                 ncp_hl_call_code_e.NCP_HL_GET_LOCAL_IEEE_ADDR,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_ADD_IC,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_ADD_IC,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_GET_IC_BY_IEEE,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_GET_IC_BY_IEEE,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_DEL_IC,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_GET_IC_BY_IEEE,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_GET_CERT,
                                 ncp_hl_call_code_e.NCP_HL_SECUR_GET_LOCAL_IC,
                                 ncp_hl_call_code_e.NCP_HL_GET_SERIAL_NUMBER,
                                 ncp_hl_call_code_e.NCP_HL_GET_VENDOR_DATA,
                                 ]
    ...
    def secur_get_cert_rsp(self, rsp, rsp_len):
        ...
        if rsp.hdr.status_code == 0:
            cs_suite_no = ncp_hl_uint8_t.from_buffer(rsp.body, 0)
            if cs_suite_no.uint8 == 1:
                suite = ncp_hl_secur_add_cert_cs1_t.from_buffer(rsp.body, 0)
            elif cs_suite_no.uint8 == 2:
                suite = ncp_hl_secur_add_cert_cs2_t.from_buffer(rsp.body, 0)
            else:
    ...
    def get_vendor_data_rsp(self, rsp, rsp_len):
        self.rsp_delayed_log(rsp)
        payload = ncp_hl_payload_t.from_buffer(rsp.body, 0)
...
def main():
    ...
    fh = logging.FileHandler("zbs_prod_cfg_sample.log")
