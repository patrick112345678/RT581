...
void send_cmd(zb_uint8_t param)
{
  ...
  else
  {
    if (g_notificaions_num < TEST_NOTIFICATIONS_NUM)
    {
      ...
      zb_uint16_t cluster_id = ZB_ZCL_CLUSTER_ID_ON_OFF;
      zb_uint8_t* ptr;

      ptr = ZB_START_GPDF_PACKET(buf);
      ZB_GPDF_PUT_UINT8(ptr, ZB_GPDF_CMD_ZCL_TUNNELING_FROM_ZGPD);
      ZB_GPDF_PUT_UINT8(ptr, 0x00); /* options: common frame, no manuf id, to server */
      ZB_GPDF_PUT_UINT16(ptr, &cluster_id);
      ZB_GPDF_PUT_UINT8(ptr, ZB_ZCL_CMD_ON_OFF_TOGGLE_ID);
      ZB_GPDF_PUT_UINT8(ptr, 0x00); /* pl len */
