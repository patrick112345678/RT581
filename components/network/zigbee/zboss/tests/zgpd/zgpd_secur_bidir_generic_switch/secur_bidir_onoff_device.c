...
{
    TRACE_MSG(TRACE_APP1, ">> send_8bit_vector_press %hd", (FMT__H, param));
    ...
    else
    {
        if (g_notificaions_num < TEST_NOTIFICATIONS_NUM)
        {
            ...
            ZB_GPDF_PUT_UINT8(ptr, ZB_GPDF_CMD_8BIT_VECTOR_PRESS);

            if (g_notificaions_num % 2)
            {
                ZB_GPDF_PUT_UINT8(ptr, 0b01);
            }
            else
            {
                ZB_GPDF_PUT_UINT8(ptr, 0b10);
            }
    ...
    TRACE_MSG(TRACE_APP1, "<< send_8bit_vector_press", (FMT__0));
}
...
static void init_device()
{
    ...
    ZGPD->app_info_options.switch_info_present = 1;
    ZGPD->switch_config.num_of_contacs = 2;
    ZGPD->switch_config.switch_type = ZB_GPD_SWITCH_TYPE_BUTTON;
