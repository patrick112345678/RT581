...
zb_uint8_t   g_battery = 0x32;
zb_uint16_t  g_temp_measur = 0x00F0;

void zgpd_set_channel_and_call(zb_uint8_t param, zb_callback_t func);

void send_cmd(zb_uint8_t param)
{
    TRACE_MSG(TRACE_APP1, ">> send compact reporting %hd", (FMT__H, param));
    ...
    else
    {
        if (g_notificaions_num < TEST_NOTIFICATIONS_NUM)
        {
            ...
            g_battery -= 2;
            g_temp_measur -= 0x0010;

            ptr = ZB_START_GPDF_PACKET(buf);
            ZB_GPDF_PUT_UINT8(ptr, ZB_GPDF_CMD_COMPACT_ATTR_REPORTING);

            ZB_GPDF_PUT_UINT8(ptr, 0); /* report idx */
            ZB_GPDF_PUT_UINT8(ptr, g_battery); /* battery persistens remaining */
            ZB_GPDF_PUT_UINT16(ptr, &g_temp_measur); /* temp measr */
    ...
    TRACE_MSG(TRACE_APP1, "<< send compact reporting", (FMT__0));
}
...
{
    ZB_ZGPD_INIT_ZGPD_CTX(ZB_ZGP_APP_ID_0000, ZB_ZGPD_COMMISSIONING_BIDIR, ZB_ZGP_TEMPERATURE_SENSOR_DEV_ID);
    ...
    ZGPD->application_info_present = 1;
    ZB_BZERO(&ZGPD->app_info_options, sizeof(ZGPD->app_info_options));

    ZGPD->app_info_options.switch_info_present = 0;

    ZGPD->app_info_options.app_descr_flw = 1;

    ZGPD->app_descr.total_reports = 1;
    ZGPD->app_descr.next_report = 0;

    {
        /*                             DP.Opt  Cluster ID    Attr ID    AttrDT   Attr Opt     Offset/Val*/
        zb_uint8_t test_point_desc0[] = {0x00,  0x01, 0x00, 0x21, 0x00,  0x20,     0x31,     0x00,  0x32 }; /* power configuration */
        zb_uint8_t test_point_desc1[] = {0x00,  0x02, 0x04, 0x00, 0x00,  0x29,     0x32,     0x01,  0x01,  0x00 }; /* temp measurment */

        ZGPD->app_descr.reports[0].options.timeout_present = 0;
        ZGPD->app_descr.reports[0].point_descs_data_len = sizeof(test_point_desc0) + sizeof(test_point_desc1);
        ZB_MEMCPY(ZGPD->app_descr.reports[0].point_descs_data, test_point_desc0, sizeof(test_point_desc0));
        ZB_MEMCPY(ZGPD->app_descr.reports[0].point_descs_data + sizeof(test_point_desc0), test_point_desc1, sizeof(test_point_desc1));
    }
}
