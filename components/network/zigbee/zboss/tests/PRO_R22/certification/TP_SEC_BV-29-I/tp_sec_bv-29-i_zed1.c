...
#define ZB_TEST_NAME TP_SEC_BV_29_I_ZED1
#define ZB_TRACE_FILE_ID 40710
#include "zb_common.h"
...
static zb_uint8_t g_test_counter = 0;

static void start_fixed_poll(zb_uint8_t start_poll);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
	...
	if (is_first_start)
	{
	  ...
	  ZB_SCHEDULE_ALARM(start_fixed_poll, 0, TIME_ZED_STOP_POLL);

          test_step_register(start_fixed_poll, 0, TP_SEC_BV_29_I_STEP_1_TIME_ZED1);

          test_control_start(TEST_MODE, TP_SEC_BV_29_I_STEP_1_DELAY_ZED1);
