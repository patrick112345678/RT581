 ...
 * DSR.
 */
/* PURPOSE: TP/SEC/BV-29-I Security Remove Device (ZC)
Objective: DUT as ZC sends APS Remove device and ZDO Mgmt_Leave_req.
*/

#define ZB_TEST_NAME TP_SEC_BV_29_I_ZC
#define ZB_TRACE_FILE_ID 40711
#include "zb_common.h"
...
enum sec_bv_29_step_e
{
    SEC_BV_29_LEAVE_ZED2,
    SEC_BV_29_LEAVE_ZR
};
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
    ...
    case ZB_ZDO_SIGNAL_DEFAULT_START:
        if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
        {
            ...
            test_step_register(send_remove_device_cmd, 0, TP_SEC_BV_29_I_STEP_2_TIME_ZC);
            /* send a ZDO Mgmt_Leave_req for device ZED2 */
            test_step_register(send_mgmt_leave_cmd, 0, TP_SEC_BV_29_I_STEP_5_TIME_ZC);
            /* send a mgnt_lqi_req */
            test_step_register(send_mgmt_lqi_req, 0, TP_SEC_BV_29_I_STEP_8_TIME_ZC);
            /* send a ZDO Mgmt_Leave_req for device ZR */
            test_step_register(send_mgmt_leave_cmd, 0, TP_SEC_BV_29_I_STEP_9_TIME_ZC);

            test_control_start(TEST_MODE, TP_SEC_BV_29_I_STEP_2_DELAY_ZC);
#endif  /* TEST_ENABLED */
        }
        else
        {
...
static void send_mgmt_leave_cmd(zb_uint8_t unused)
{
    ...
    if (buf)
    {
        switch (test_counter)
        {
        case SEC_BV_29_LEAVE_ZED2:
            ZB_IEEE_ADDR_COPY(device_addr, &g_ieee_addr_ed2);
            break;
        case SEC_BV_29_LEAVE_ZR:
