 ...
 * DSR.
 */
/* PURPOSE: TP/R21/BV-11 - Processing of unencrypted NWK frames, gZR1 (Forming distributed network)
*/


#define ZB_TEST_NAME TP_R21_BV_11_GZR1_D
#define ZB_TRACE_FILE_ID 40870
...
static void start_test(zb_uint8_t param);
static void test_send_brcast_cmd_delayed(zb_uint8_t param);

/* removes gzr2 from neighbor table, creates routing entry for gzr2 */
static void test_remove_neighbor_zr2(zb_uint8_t param);
...
static zb_uint8_t g_dut_is_router = ZB_FALSE;
...
static void device_annce_cb(zb_zdo_device_annce_t *da)
{
  ...
  {
    g_dut_short_addr = da->nwk_addr;
    g_dut_is_router = !!ZB_MAC_CAP_GET_DEVICE_TYPE(da->capability);

    if (!test_started)
    {
      test_started = 1; /* prevent multiple scheduling of node_desc_req */
      ZB_SCHEDULE_ALARM(start_test, 0, 30 * ZB_TIME_ONE_SECOND);
    }
  }
  else if (ZB_IEEE_ADDR_CMP(da->ieee_addr, g_ieee_addr_gzr2))
  {
...
static void test_remove_neighbor_zr2(zb_uint8_t param)
{
  ...
  if (zb_nwk_delete_neighbor_by_short(g_gzr2_short_addr) == RET_OK)
  {
    routing_ent = zb_nwk_mesh_find_route(g_gzr2_short_addr);
    if (routing_ent == NULL)
    {
  ...
  if ( routing_ent )
  {
    ...
    routing_ent->dest_addr = g_gzr2_short_addr;
    ...
    zb_address_by_short(g_dut_short_addr, ZB_FALSE, ZB_FALSE, &dut_addr_ref);
    routing_ent->next_hop_addr_ref = dut_addr_ref;
    routing_ent->expiry = ZB_NWK_ROUTING_TABLE_EXPIRY;

    TRACE_MSG(TRACE_NWK1, "new routing ent : dest_addr 0x%x status %hd next_hop_ref 0x%x",
              (FMT__D_H_D,
...
static void test_get_peer_addr_resp(zb_uint8_t param)
{
  ...
  {
    ZB_LETOH16(&g_gzr2_short_addr, &resp->nwk_addr);
    g_dest_short_addr = g_dut_short_addr;
    zb_buf_get_out_delayed(test_send_unsecur_node_desc_req);
    zb_buf_get_out_delayed(test_send_secur_node_desc_req);
  }
...
static void test_node_desc_resp_handler(zb_uint8_t param)
{
  ...
  if ((iter == 0) && (g_dut_is_router))
  {
    /* This step only for router and ZC DUTs */
    ++iter;
    /* add routing table entry for gZR2
     * and remove gZR2 from neighbor table.
     */
    g_dest_short_addr = g_gzr2_short_addr;
    test_remove_neighbor_zr2(0);
...
static void test_send_secur_nwk_addr_req(zb_uint8_t param)
{
  ...
  req_param->dst_addr = g_dest_short_addr;
  req_param->start_index = 0;
  req_param->request_type = ZB_ZDO_SINGLE_DEVICE_RESP;
  ZB_IEEE_ADDR_COPY(req_param->ieee_addr, g_dest_ieee_addr);
  cb = (!ZB_NWK_IS_ADDRESS_BROADCAST(g_dest_short_addr))? test_get_peer_addr_resp: NULL;
...
static void test_send_unsecur_nwk_addr_req(zb_uint8_t param)
{
  ...
  test_send_unsecur_frame(param, ZDO_NWK_ADDR_REQ_CLID, g_dest_short_addr);
...
static void test_send_unsecur_frame(zb_uint8_t param, zb_uint16_t command_id, zb_uint16_t addr)
{
  ...
  TRACE_MSG(TRACE_APS1, ">>test_send_unsecur_frame, param = %h, command_id = %h, addr = %h",
  ...
  *byte_p = fc;
  ...
  nldereq->dst_addr = g_dest_short_addr;
