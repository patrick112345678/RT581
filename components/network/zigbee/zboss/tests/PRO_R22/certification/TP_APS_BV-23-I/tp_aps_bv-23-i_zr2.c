...
#define ZB_TEST_NAME TP_APS_BV_23_I_ZR2
#define ZB_TRACE_FILE_ID 40685
...
static void bind_req(zb_uint8_t param)
{
  ...
  zb_apsme_add_group_req_t *req = NULL;
  zb_uint16_t addr = 0;

  TRACE_MSG(TRACE_APP1, ">>bind_req with addr = 0x%x", (FMT__H, param));
  if (!buf)
  {
  ...
  addr = param;
  req = ZB_BUF_GET_PARAM(buf, zb_apsme_add_group_req_t);
  ZB_BZERO(req, sizeof(*req));
  req->group_address = addr;
  req->endpoint = ZR2_BIND_EP;
  ...
  if (0 == status)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_DEVICE_REBOOT:
        ...
        /* Add record to the binding table */
        ZB_SCHEDULE_CALLBACK(bind_req, ZR2_BIND_ADDR);
        break;
