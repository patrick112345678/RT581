...
#define ZB_TEST_NAME TP_PRO_BV_09_ZC
#define ZB_TRACE_FILE_ID 40703
...
static const zb_ieee_addr_t g_zr2_addr = {0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00};
...
static zb_uint16_t addr_ass_cb(zb_ieee_addr_t ieee_addr)
{
    static zb_uint16_t count = 0x0002;
    if (!ZB_MEMCMP(g_zr1_addr, ieee_addr, sizeof(zb_ieee_addr_t)))
    {
        return 0x0001;
    }
    else if (!ZB_MEMCMP(g_zr2_addr, ieee_addr, sizeof(zb_ieee_addr_t)))
    {
        return 0x0002;
    }
    else
...
static void change_link_status(zb_uint8_t param)
{
    ...
    if (RET_OK == zb_nwk_neighbor_get_by_ieee((zb_uint8_t *)g_zr1_addr, &nbt))
    {
        nbt->u.base.age = 0;
    ...
    if (RET_OK == zb_nwk_neighbor_get_by_ieee((zb_uint8_t *)g_zr2_addr, &nbt))
    {
        nbt->u.base.age = 0;
        nbt->u.base.outgoing_cost = 1;
