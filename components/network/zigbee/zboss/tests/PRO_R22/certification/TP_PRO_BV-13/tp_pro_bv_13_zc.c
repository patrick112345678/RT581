...
#define ZB_TEST_NAME TP_PRO_BV_13_ZC
#define ZB_TRACE_FILE_ID 40809
...
static zb_uint8_t ed_addr_counter = 0;
...
static zb_uint16_t addr_ass_cb(zb_ieee_addr_t ieee_addr)
{
    ...
    if (ZB_IEEE_ADDR_CMP(ieee_addr, g_ieee_addr_r))
    {
        res = ZR_SHORT_ADDR;
    }
    else
    ...
    if (ZB_IEEE_ADDR_CMP(ieee_addr, g_ieee_addr_ed))
    {
        if (ed_addr_counter == 0)
        {
            res = ZED_SHORT_ADDR_1;
            ed_addr_counter++;
        }
    }
