 ...
 * DSR.
 */
/* PURPOSE: TP/R21/BV-03 - Router (gZR) - joining to DUT ZR
*/


#define ZB_TEST_NAME TP_R21_BV_03_GZR
#define ZB_TRACE_FILE_ID 40776
...
MAIN()
{
  ...
  zb_zdo_set_aps_unsecure_join(ZB_TRUE);
	zb_set_network_router_role_legacy(1l << 15);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  else
  {
    ...
    ZB_SCHEDULE_ALARM(send_mac_data, 0, 5 * ZB_TIME_ONE_SECOND);
  }
...
static void send_mac_data(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APS3, ">>send_mac_data: param = %hd", (FMT__H, buf));

  /* MAC DATA: FC(2)| SN(1)| SRC_PAN(2)| MAC_ADDR(8)| DATA(10)| FCS(2) = 25 bytes  */

  buf_ptr = (zb_uint8_t*) zb_buf_initial_alloc(buf, 25);

  fc = 0;
  ZB_FCF_SET_FRAME_TYPE(&fc, MAC_FRAME_DATA);
  ZB_FCF_SET_SECURITY_BIT(&fc, 0);
  ZB_FCF_SET_FRAME_PENDING_BIT(&fc, 0);
  ZB_FCF_SET_ACK_REQUEST_BIT(&fc, 1);
  ...
  *buf_ptr++ = zb_cert_test_get_mac_tsn(); /* Sequence number */
  ...
  ZB_IEEE_ADDR_COPY(buf_ptr, g_ieee_addr_gzr);
...
#if 0
  PRINT_BUF_PAYLOAD_BY_REF(buf);
#endif
  ZB_SCHEDULE_TX_CB(send_mac_data_tx_cb, buf);
