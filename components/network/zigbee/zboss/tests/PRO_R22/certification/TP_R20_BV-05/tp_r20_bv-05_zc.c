...
#define ZB_TEST_NAME TP_R20_BV_05_ZC
#define ZB_TRACE_FILE_ID 40839
...
static void send_device_remove_no_secure(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_SECUR1, ">>force zr remove without security", (FMT__0));

  if (buf)
  {
    p = zb_buf_initial_alloc(buf, sizeof(zb_apsme_remove_device_pkt_t));

    ZB_IEEE_ADDR_COPY(p->child_address, g_ieee_addr2);

    zb_aps_send_command(buf,
                        zb_address_short_by_ieee((zb_uint8_t*) g_ieee_addr1),
  ...
  {
    TRACE_MSG(TRACE_SECUR1, "TEST FAILED: Could not get out buf!", (FMT__0));
  }

  TRACE_MSG(TRACE_SECUR1, "<<force zr remove without security", (FMT__0));
}

static void send_device_remove_ed(zb_uint8_t param)
{
  ...
  ZB_IEEE_ADDR_COPY(req->parent_address, g_ieee_addr1);
  ZB_IEEE_ADDR_COPY(req->child_address, g_ieee_addr2);
  ZB_SCHEDULE_CALLBACK(zb_secur_apsme_remove_device, param);
}

static void send_device_remove_route(zb_uint8_t param)
{
  zb_bufid_t buf = zb_buf_get_out();
  ZVUNUSED(param);
  ...
  if (buf)
  {
    zb_apsme_remove_device_req_t *req = ZB_BUF_GET_PARAM(buf, zb_apsme_remove_device_req_t);
    TRACE_MSG(TRACE_SECUR1, "force zr remove", (FMT__0));

    ZB_IEEE_ADDR_COPY(req->parent_address, g_ieee_addr1);
    ZB_IEEE_ADDR_COPY(req->child_address, g_ieee_addr1);

    ZB_SCHEDULE_CALLBACK(zb_secur_apsme_remove_device, buf);
  }
  else
  {
    TRACE_MSG(TRACE_SECUR1, "TEST FAILED: Could not get buffer!", (FMT__0));
  }
}

static void alarm1(zb_uint8_t param)
{
  ZVUNUSED(param);
  zb_buf_get_out_delayed(send_device_remove_ed);
}

static void alarm2(zb_uint8_t param)
{
  ZVUNUSED(param);
  zb_buf_get_out_delayed(send_device_remove_route);
}
  ...
  if (0 == status)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_DEVICE_REBOOT:
        ...
        ZB_SCHEDULE_ALARM(send_device_remove_no_secure, 0, 30*ZB_TIME_ONE_SECOND);

        ZB_SCHEDULE_ALARM(alarm1, 0, 35*ZB_TIME_ONE_SECOND);
        ZB_SCHEDULE_ALARM(alarm2, 0, 45*ZB_TIME_ONE_SECOND);
        break;
