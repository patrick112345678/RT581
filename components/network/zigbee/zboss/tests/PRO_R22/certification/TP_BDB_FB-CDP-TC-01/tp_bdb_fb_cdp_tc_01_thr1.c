...
#define ZB_TEST_NAME TP_BDB_FB_CDP_TC_01_THR1
#define ZB_TRACE_FILE_ID 40887
...
static zb_ieee_addr_t g_ieee_addr_other = IEEE_ADDR_OTHER;
...
static void get_peer_addr(zb_uint8_t unused)
{
    ...
    if (s_dut_short_addr == 0xffff)
    {
        ...
        zb_buf_get_out_delayed(request_nb_table);
    }
    else
    {
...
static void request_nb_table(zb_uint8_t param)
{
    ...
    zb_uint16_t dest_addr = zb_cert_test_get_parent_short_addr();
...
static void test_thr1_fsm(zb_uint8_t unused)
{
    ...
    case UNBIND_DEFAULT_BINDING:
                  ...
                  (FMT__D, s_bind_table_cache_sz));
        {
            ...
            {
                zb_buf_get_out_delayed(remove_default_binding);
            }
            else
            {
                ++s_step_idx;
    ...
    if (call_cb != NULL)
    {
        zb_buf_get_out_delayed_ext(call_cb, param2, 0);
    }
...
static void check_bind_table_resp_cb(zb_uint8_t param)
{
    ...
    if (resp->status == ZB_ZDP_STATUS_SUCCESS)
    {
        ...
        {
            zb_buf_get_out_delayed_ext(check_bind_table, start_idx + nbrs, 0);
        }
        else
        {
            next_step = 1;
