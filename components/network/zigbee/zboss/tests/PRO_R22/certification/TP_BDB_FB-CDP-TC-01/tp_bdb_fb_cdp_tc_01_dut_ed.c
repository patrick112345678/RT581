...
#define ZB_TEST_NAME TP_BDB_FB_CDP_TC_01_DUT_ED
#define ZB_TRACE_FILE_ID 40890
...
ZB_ZCL_DECLARE_BASIC_ATTRIB_LIST(fb_cdp_tc_01_dut_ed_basic_attr_list, &attr_zcl_version, &attr_power_source);

ZB_ZCL_DECLARE_IDENTIFY_ATTRIB_LIST(fb_cdp_tc_01_dut_ed_identify_attr_list, &attr_identify_time);

/********************* Declare device **************************/
DECLARE_ON_OFF_CLIENT_CLUSTER_LIST(fb_cdp_tc_01_dut_ed_on_off_controller_clusters,
                                   fb_cdp_tc_01_dut_ed_basic_attr_list,
                                   fb_cdp_tc_01_dut_ed_identify_attr_list);

DECLARE_ON_OFF_CLIENT_SIMPLE_DESC(fb_cdp_tc_01_dut_ed_ep1, DUT_ENDPOINT1,
                                  ON_OFF_CLIENT_IN_CLUSTER_NUM, ON_OFF_CLIENT_OUT_CLUSTER_NUM);

DECLARE_ON_OFF_CLIENT_EP(fb_cdp_tc_01_dut_ed_ep1, DUT_ENDPOINT1,
                         fb_cdp_tc_01_dut_ed_on_off_controller_clusters);

DECLARE_ON_OFF_CLIENT_CTX(fb_cdp_tc_01_dut_ed_on_off_controller_ctx,
                          fb_cdp_tc_01_dut_ed_ep1);
...
MAIN()
{
  ...
  ZB_AF_REGISTER_DEVICE_CTX(&fb_cdp_tc_01_dut_ed_on_off_controller_ctx);
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  switch (sig)
  {
    case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
      ...
      if (status == 0)
      {
        ZB_SCHEDULE_CALLBACK(trigger_fb_initiator, 0);
