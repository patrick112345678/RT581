 ...
 * DSR.
 */
/* PURPOSE: TP/PRO/BV-03 Rejoin end device (Rx - Off)
...
#define ZB_TEST_NAME TP_PRO_BV_03_ZC
#define ZB_TRACE_FILE_ID 40553
...
MAIN()
{
  ...
  /* accept only one child - see update beacon payload alrm callback */
...
ZB_ZDO_STARTUP_COMPLETE(zb_uint8_t param)
{
  ...
  if (0 == status)
  {
	...
	/* Prevent zed joining to the zc */
	zb_association_permit(0);

        /* Allow zed rejoin to the zc */
        ZB_SCHEDULE_ALARM(zb_association_permit, 1, 20*ZB_TIME_ONE_SECOND);
	break;
