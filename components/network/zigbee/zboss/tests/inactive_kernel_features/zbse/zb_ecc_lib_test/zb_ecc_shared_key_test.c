...
MAIN()
{
  ...
  ZB_INIT("zb_ecc_shared_key_test");
  ...
  //for(j = 0;1;j++)
  {
    ...
    {
      printf("Make error\n");
      *(eph_prv_dev2+34) = *(eph_prv_dev2+34) ^ 1;
    }
    ...
    if(0 == flag)
    {
      TRACE_MSG(TRACE_APP1, ">>> Unit:%s, [#%d], suite [%d], Status: Passed", (FMT__P_P_D_D, test_names[i], i, suite_type[i]));
    }
    else
    {
      TRACE_MSG(TRACE_APP1, ">>> Unit:%s, [#%d], suite [%d], Status: Failed", (FMT__P_P_D_D, test_names[i], i, suite_type[i]));
    }
    if(flag == 1)
    counter++;
  }
