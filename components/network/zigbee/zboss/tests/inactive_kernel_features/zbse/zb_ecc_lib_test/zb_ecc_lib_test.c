...
zb_uint32_t base_test(void)
{
    ...
    ZB_MEMCPY(buff, pub_key, ZB_ECC_PUBLIC_KEY_LEN[sect163k1]);
    zb_ecc_inverse_bytes(buff, ZB_ECC_PUBLIC_KEY_LEN[sect163k1]);
    if (RET_OK != (res = zb_ecc_point_decompression((zb_uint32_t *)buff, &point)))
    {
        ...
        TRACE_MSG(TRACE_APP1, "Private key after generation", (FMT__0));
        DUMP_TRAF("Private key-->", prv_key, ZB_ECC_PRIVATE_KEY_LEN[sect163k1], 0);

        TRACE_MSG(TRACE_APP1, "Public key after generation", (FMT__0));
        DUMP_TRAF("Public key-->", pub_key, ZB_ECC_PUBLIC_KEY_LEN[sect163k1], 0);
    ...
    if (ZB_PK_VALID != (res = zb_ecc_pk_validation(&point)))
    {
        ...
        return RET_ERROR; //FIXME: need to implement pk_validation procedure for sect163k1 curve
    }
    else
    {
...
void zb_ecc_point_addition(zb_ecc_point_aff_t *P, const zb_ecc_point_aff_t *Q, zb_ecc_point_aff_t *R)
{
...
{
    zb_ecc_point_aff_t G2, G22, GN, G3;

    zb_ecc_modsq(G163k1.x, G2.x);
    dump_bytes((zb_uint8_t *)G2.x, 22, "G2.x");
    zb_ecc_modsq(G163k1.y, G2.y);
    dump_bytes((zb_uint8_t *)G2.y, 22, "G2.y");
    zb_ecc_modsq(G2.x, G22.x);
    dump_bytes((zb_uint8_t *)G22.x, 22, "G22.x");
    zb_ecc_modsq(G2.y, G22.y);
    dump_bytes((zb_uint8_t *)G22.y, 22, "G22.y");
    zb_ecc_point_negation(&G163k1, &GN);
    zb_ecc_point_addition(&G22, &GN, &G3);
    dump_bytes((zb_uint8_t *)G3.x, 22, "G3.x");
    dump_bytes((zb_uint8_t *)G3.y, 22, "G3.y");

    dump_dwords((zb_uint8_t *)G3.x, 22, "G3.x");
    dump_dwords((zb_uint8_t *)G3.y, 22, "G3.y");
}
...
zb_uint8_t test_key_generation(zb_uint8_t *prv_key, zb_uint8_t *exp_pub_key)
{
    ...
    // ZB_MEMSET(&key, 0, sizeof(zb_ecc_key_pair_t));
    // ZB_MEMSET(&point, 0, sizeof(zb_ecc_point_aff_t));
    zb_ecc_key_generation(prv_key, prv_key, pub_key);

    if (0 != ZB_MEMCMP(pub_key, exp_pub_key, ZB_ECC_PUBLIC_KEY_LEN[sect163k1]))
    {
        TRACE_MSG(TRACE_APP1, "Calculated public key '!=' expected public key", (FMT__0));

        DUMP_TRAF("Calculated public key-->", pub_key,  ZB_ECC_PUBLIC_KEY_LEN[sect163k1], 0);

        DUMP_TRAF("Expected public key:-->", exp_pub_key,  ZB_ECC_PUBLIC_KEY_LEN[sect163k1], 0);
...
zb_uint8_t initiate_key_establishment_request(zb_uint8_t *exp_pub_key, zb_uint8_t *cert)
{
    ...
    // zb_uint32_t *CA_pub_key = (zb_uint32_t *)_CA_public_key;
    zb_uint8_t x_pub_key_dev2[ZB_ECC_PUBLIC_KEY_LEN[sect163k1]];
     ...
     * initiator(device 2)
     */
    // ZB_MEMSET(x_pub_key_dev2, 0, ZB_ECC_MAX_PUB_KEY_LEN_IN_BYTES);
    if (RET_OK != zb_ecc_ecqv_extraction(CA_pub_key, cert, cert, x_pub_key_dev2))
    {
    ...
    if (0 != ZB_MEMCMP(x_pub_key_dev2, exp_pub_key, ZB_ECC_PUBLIC_KEY_LEN[sect163k1]))
    {
                                 ...
                                 zb_uint8_t *eph_pub_dev1, zb_uint8_t *pub_dev2, zb_uint8_t *eph_pub_dev2, zb_uint8_t *skey1)
{
    ...
    if (RET_OK != zb_ecc_ecmqv(prv_dev1, eph_prv_dev1, eph_pub_dev1, pub_dev2, eph_pub_dev2, skey1))
    {
        ...
        DUMP_TRAF("Shared key between device 1 and device 2 is-->", skey1, 21, 0);
...
MAIN()
{
    zb_uint8_t skey1[24];  /*NOTE: up to 24 bytes for correct comparison*/
    zb_uint8_t skey2[24];

    ARGV_UNUSED;
    ZB_MEMSET(skey1, 0, 24);
    ZB_MEMSET(skey2, 0, 24);

    ZB_INIT("zb_ecc_lib_test");
