...
function print_test_results()
{
    ...
    if grep -e "Sync time with 0x0 ep 1" gas_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "Get network time\: " gas_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "The second authoritative time server: ep=7" gas_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Master bit is set. Time synchronization is not allowed." gas_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
        echo "Failed: Gas device shouldn't set real time clock at Time attribute writing."
    fi

    if grep "Master bit is set. Time synchronization is not allowed." gas_device1.log >/dev/null \
       || grep "Real Time Clock callback not assigned (use ZB_ZCL_TIME_SET_REAL_TIME_CLOCK_CB) or invalid time value received" gas_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Don't write Time Status attribute: Master Bit is set" gas_device1.log >/dev/null
    then
    ...
    if grep "Sync time with 0x0 ep 1" esi_zr1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "Get network time\: " esi_zr1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "The second authoritative time server: ep=7" esi_zr1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Master bit is set. Time synchronization is not allowed." esi_device1.log >/dev/null \
       || grep "Real Time Clock callback not assigned (use ZB_ZCL_TIME_SET_REAL_TIME_CLOCK_CB) or invalid time value received" esi_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "<< zb_zcl_set_attr_val, cluster_id 0xa, attr_id 0x1 status 0" esi_device1.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
        echo "Failed: ESI coordinator device should write Time Status attribute"
    fi
...
energy_service_interface/esi_device1 &
coordPID=$!
echo $coordPID
wait_for_start esi_device1
