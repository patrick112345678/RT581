...
function print_test_results()
{
    ...
    if grep -e "Sync time with .* ep 7" gas_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "Get network time\: " gas_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "The most authoritative time server: ep=7, .*, auth_level=2" gas_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Master bit is set. Time synchronization is not allowed." gas_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "zb_zcl_set_attr_val, cluster_id 0xa, attr_id 0x0 status 87" gas_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Don't write Time Status attribute: Master Bit is set" gas_device3.log >/dev/null
    then
    ...
    if grep -e "Sync time with .* ep 7" esi_zr3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "Get network time\: " esi_zr3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep -e "The most authoritative time server: ep=7, .*, auth_level=2" esi_zr3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "Real Time Clock callback not assigned (use ZB_ZCL_TIME_SET_REAL_TIME_CLOCK_CB) or invalid time value received" esi_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
    ...
    if grep "<< zb_zcl_set_attr_val, cluster_id 0xa, attr_id 0x1 status 0" esi_device3.log >/dev/null
    then
        let "passed += 1"
        let "failed -= 1"
    else
...
energy_service_interface/esi_device3 &
coordPID=$!
echo $coordPID
wait_for_start esi_device3
