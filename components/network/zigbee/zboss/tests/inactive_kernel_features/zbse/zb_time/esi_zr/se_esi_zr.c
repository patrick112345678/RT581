...
#define ESI_USE_ALL_PRICE_CLUSTER_ATTRIBUTES 0
...
#define ESI_TIME_STATUS_INITIAL ((1 << ZB_ZCL_TIME_SYNCHRONIZED))

#elif TEST_SUITE == 2

#define ESI_TIME_STATUS_INITIAL ((1 << ZB_ZCL_TIME_MASTER_ZONE_DST))

#elif TEST_SUITE == 3

#define ESI_TIME_STATUS_INITIAL ((1 << ZB_ZCL_TIME_SUPERSEDING))
...
typedef struct esi_dev_ctx_s
{
  ...
  zb_addr_u time_client_address;
  zb_uint8_t time_client_ep;
...
static void esi_get_sched_prices_cb_on_sent(zb_uint8_t param)
{
  ...
  ZB_SCHEDULE_ALARM(esi_get_sched_prices_cb_send_pp, 0, ESI_PRICE_ACK_TIMEOUT);
...
static void esi_price_ack_cb(zb_uint8_t param)
{
  ...
  if (ESI_DEV_PRICE_CTX().state == ESI_DEV_GET_SCHED_PRICES_STATE_WAIT_PRICE_ACK)
  {
    ...
    zb_uint8_t pp_buf = 0;
    ESI_DEV_PRICE_CTX().state = ESI_DEV_GET_SCHED_PRICES_STATE_FINI;
    ZB_SCHEDULE_ALARM_CANCEL_AND_GET_BUF(esi_get_sched_prices_cb_send_pp, ZB_ALARM_ALL_CB, &pp_buf);
...
static void esi_dev_cmd_send_lce_event3(zb_uint8_t param)
{
  ...
  else
  {
    ...
    ZB_ZCL_DRLC_SEND_CMD_CANCEL_ALL_LCE(param,
                                        ...
                                        ESI_DEV_ENDPOINT,
                                        &cancel_control);
    ZB_SCHEDULE_ALARM(esi_dev_cmd_send_lce_event1, 0, ESI_LOAD_CONTROL_EVENT_GROUP_TIMEOUT);
  }
}
/** [esi_dev_cmd_send_lce_event3]  */
...
static void esi_dev_cmd_send_lce_event2(zb_uint8_t param)
{
  ...
  else
  {
    ...
    ZB_ZCL_DRLC_SEND_CMD_CANCEL_LCE(param,
                                    ...
                                    ESI_DEV_ENDPOINT,
                                    &payload);
    ZB_SCHEDULE_ALARM(esi_dev_cmd_send_lce_event3, 0, ESI_LOAD_CONTROL_EVENT_TIMEOUT);
  }
}
/** [esi_dev_cmd_send_lce_event2] */
...
static void esi_dev_cmd_send_lce_event1(zb_uint8_t param)
{
  ...
  else
  {
    ...
    ZB_ZCL_DRLC_SEND_CMD_LOAD_CONTROL_EVENT(param,
                                            ...
                                            ESI_DEV_ENDPOINT,
                                            &payload);
    ZB_SCHEDULE_ALARM(esi_dev_cmd_send_lce_event2, 0, ESI_LOAD_CONTROL_EVENT_TIMEOUT);
  }
}
/** [esi_dev_cmd_send_lce_event1]  */
...
static void esi_dev_handle_time_synchronization(zb_uint8_t param)
{
  const zb_zcl_time_sync_payload_t *pl_in = ZB_ZCL_DEVICE_CMD_PARAM_IN_GET(param, zb_zcl_time_sync_payload_t);

  TRACE_MSG(TRACE_APP1, ">> esi_dev_handle_time_synchronization", (FMT__0));
...
static void esi_zcl_cmd_device_cb(zb_uint8_t param)
{
  ...
  switch (ZB_ZCL_DEVICE_CMD_PARAM_CB_ID(param))
  {
    ...
    case ZB_ZCL_TIME_SYNC_CB_ID:
      ...
      esi_dev_handle_time_synchronization(param);
...
/** [ESI_DEV_SET_ROLE] */
  zb_se_set_network_router_role(ESI_DEV_CHANNEL_MASK);
/** [ESI_DEV_SET_ROLE] */
...
static void esi_dev_left_button_handler_auto_join(zb_uint8_t param)
{
  if (!param)
  {
    /* Button is pressed, get buffer for outgoing command */
    ZB_GET_OUT_BUF_DELAYED(esi_dev_left_button_handler_auto_join);
  }
  else
  {
...
static void esi_write_time_attrs(zb_uint8_t param)
{

  if (!param)
  {
    ZB_GET_OUT_BUF_DELAYED(esi_write_time_attrs);
  }
  else
  {
  ...
  zb_uint8_t time_status = ((1 << ZB_ZCL_TIME_MASTER) | (1 << ZB_ZCL_TIME_SYNCHRONIZED));

#else
#if TEST_SUITE == 1

  zb_uint8_t time_status = ((1 << ZB_ZCL_TIME_SUPERSEDING));
  ...
  zb_uint8_t time_status = ((1 << ZB_ZCL_TIME_MASTER));

#endif
#endif


    TRACE_MSG(TRACE_APP1, ">> esi_write_time_attrs param %hd", (FMT__H, param));
    ...
    ZB_ZCL_GENERAL_ADD_VALUE_WRITE_ATTR_REQ(cmd_ptr,
                                            ...
                                            (zb_uint8_t *)(&g_dev_ctx.time_attrs.time));
    ...
    ZB_ZCL_GENERAL_SEND_WRITE_ATTR_REQ(buf,
                                       cmd_ptr,
                                       g_dev_ctx.time_client_address,
                                       ZB_APS_ADDR_MODE_64_ENDP_PRESENT,
                                       g_dev_ctx.time_client_ep,
                                       ESI_DEV_ENDPOINT,
                                       ZB_AF_SE_PROFILE_ID,
                                       ZB_ZCL_CLUSTER_ID_TIME,
                                       NULL);

    TRACE_MSG(TRACE_APP1, "<< esi_write_time_attrs ret %hx", (FMT__H, ret));
  }
}
...
/** [SIGNAL_HANDLER_LOAD_CERT] */
      ...
      case ZB_SE_SIGNAL_SKIP_JOIN:
        ...
        zb_button_register_handler(BUTTON_LEFT, 0, esi_dev_left_button_handler_auto_join);
#else
        esi_dev_left_button_handler_auto_join(0);
#endif
        break;
...
/** [SIGNAL_HANDLER_BIND_DEV] */
        if (bind_params->cluster_id == ZB_ZCL_CLUSTER_ID_TIME)
        {
          g_dev_ctx.time_client_ep = bind_params->endpoint;
          ZB_IEEE_ADDR_COPY(g_dev_ctx.time_client_address.addr_long,
                            bind_params->device_addr);

          TRACE_MSG(TRACE_APP1, "Send bind request to Time server", (FMT__0));
          zb_se_service_discovery_bind_req(param, bind_params->device_addr, bind_params->endpoint);

          /*TODO: wait when device receive TCLK key and then write attrs */
          ZB_GET_OUT_BUF_DELAYED(esi_write_time_attrs);
...
/** [SIGNAL_HANDLER_BIND_INDICATION] */
          ...
          if (bind_params->cluster_id == ZB_ZCL_CLUSTER_ID_DRLC)
          {
            ...
            ZB_SCHEDULE_ALARM_CANCEL(esi_dev_cmd_send_lce_event1, ZB_ALARM_ANY_PARAM);
            ZB_SCHEDULE_CALLBACK(esi_dev_cmd_send_lce_event1, param);
            param = 0;
          }
#if ESI_ENABLE_CALENDAR_CLUSTER
          else if (bind_params->cluster_id == ZB_ZCL_CLUSTER_ID_CALENDAR)
          {
            ...
            ZB_SCHEDULE_ALARM_CANCEL(esi_send_publish_calendar, ZB_ALARM_ANY_PARAM);
            ZB_SCHEDULE_CALLBACK(esi_send_publish_calendar, param);
            param = 0;
          }
...
#endif /* ESI_ENABLE_CALENDAR_CLUSTER */
#if 0
          ZB_SCHEDULE_ALARM(zb_secur_nwk_key_switch_procedure, 0, ZB_TIME_ONE_SECOND * 20);
#endif
        }
        break;
