...
#define METERING_DEV_ENDPOINT 2
#define METERING_DEV_CHANNEL_MASK1 (1L << 22)
#define METERING_DEV_CHANNEL_MASK2 3
#define METERING_DEV_CHANNEL_PAGE_I1 0
#define METERING_DEV_CHANNEL_PAGE_I2 1
...
typedef struct metering_dev_ctx_s
{
    ...
    zb_zcl_prepayment_attrs_t prepayment_attrs;  /**< Prepayment cluster attributes >*/
} metering_dev_ctx_t;
...
static void metering_dev_save_data(zb_uint8_t param)
{
    ...
    ZB_SCHEDULE_ALARM(metering_dev_save_data, 0, METERING_DEV_APP_DATA_PERSISTING_TIMEOUT);
}
...
void metering_dev_clusters_attrs_init(zb_uint8_t param)
{
    ...
    g_dev_ctx.metering_attrs.unit_of_measure = ZB_ZCL_METERING_UNIT_OF_MEASURE_DEFAULT_VALUE;
...
void metering_dev_default_reporting_init(zb_uint8_t param)
{
    ...
    rep_info.u.send_info.max_interval = ZB_SE_METERING_DATA_UPDATE_RATE + 15;
    rep_info.u.send_info.def_min_interval = ZB_SE_METERING_DATA_UPDATE_RATE;
    rep_info.u.send_info.def_max_interval = ZB_SE_METERING_DATA_UPDATE_RATE + 15;
...
void metering_dev_app_init(zb_uint8_t param)
{
...
#if defined ZB_SUBGHZ_BAND_ENABLED
    {
        ...
        zb_channel_page_list_set_mask(channel_list, 0, METERING_DEV_CHANNEL_MASK1);
        zb_channel_page_list_set_mask(channel_list, METERING_DEV_CHANNEL_PAGE_I2, METERING_DEV_CHANNEL_MASK2);
        TRACE_MSG(TRACE_APP1, "ZED in MM mode start: page %d mask 0x%x page %d mask 0x%x",
                  (FMT__D_D_D_D, METERING_DEV_CHANNEL_PAGE_I1, METERING_DEV_CHANNEL_MASK1, METERING_DEV_CHANNEL_PAGE_I2, METERING_DEV_CHANNEL_MASK2));
...
static void metering_dev_measure(zb_uint8_t param)
{
    ...
    {
        ZB_SCHEDULE_CALLBACK(metering_dev_save_data, 0);
        g_dev_ctx.first_measurement_done = ZB_TRUE;
    }

    ZB_SCHEDULE_ALARM(metering_dev_measure, 0, METERING_DEV_MEASURE_TIMEOUT);
}
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_SIGNAL_DEVICE_REBOOT:
            ...
            ZB_SCHEDULE_ALARM_CANCEL(metering_dev_measure, ZB_ALARM_ANY_PARAM);
            ZB_SCHEDULE_CALLBACK(metering_dev_measure, 0);
