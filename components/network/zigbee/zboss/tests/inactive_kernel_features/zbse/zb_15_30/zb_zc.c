...
typedef struct esi_dev_ctx_s
{
    ...
    zb_addr_u cbke_partner_address;
...
void zc_update_link_key(zb_uint8_t param)
{
    zb_uint16_t addr_of_interest = zb_address_short_by_ieee(g_dev_ctx.cbke_partner_address.addr_long);
    se_tc_update_link_key(addr_of_interest);
    ZVUNUSED(param);
}
...
static void esi_get_sched_prices_cb_on_sent(zb_uint8_t param)
{
    ...
    ZB_SCHEDULE_ALARM(esi_get_sched_prices_cb_send_pp, param, ESI_PRICE_ACK_TIMEOUT);
                                       ...
                                       const zb_zcl_parsed_hdr_t *in_cmd_info)
{
    ...
    {
        TRACE_MSG(TRACE_APP1, "Got ZB_ZCL_DRLC_EVENT_STATUS_LCE_RECEIVED status from 0x%x", (FMT__D, ZB_ZCL_PARSED_HDR_SHORT_DATA(in_cmd_info).source.u.short_addr));
    }
    if (ZB_ZCL_DRLC_EVENT_REJECTED_UNDEFINED_EVENT == in->event_status)
    {
        TRACE_MSG(TRACE_APP1, "Got ZB_ZCL_DRLC_EVENT_STATUS_REJECTED_UNDEFINED_EVENT status from 0x%x", (FMT__D, ZB_ZCL_PARSED_HDR_SHORT_DATA(in_cmd_info).source.u.short_addr));
    }
    if (ZB_ZCL_DRLC_EVENT_EVENT_STARTED == in->event_status)
    {
        TRACE_MSG(TRACE_APP1, "Got ZB_ZCL_DRLC_EVENT_STATUS_EVENT_STARTED status from 0x%x", (FMT__D, ZB_ZCL_PARSED_HDR_SHORT_DATA(in_cmd_info).source.u.short_addr));
    }
    if (ZB_ZCL_DRLC_EVENT_EVENT_CANCELLED == in->event_status)
    {
        TRACE_MSG(TRACE_APP1, "Got ZB_ZCL_DRLC_EVENT_STATUS_EVENT_CANCELLED status from 0x%x", (FMT__D, ZB_ZCL_PARSED_HDR_SHORT_DATA(in_cmd_info).source.u.short_addr));
    }
}
...
void zboss_signal_handler(zb_uint8_t param)
{
    ...
    if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
    {
        ...
        case ZB_SE_TC_SIGNAL_CHILD_JOIN:
        {
...
#endif
#if 1
            ZB_IEEE_ADDR_COPY(g_dev_ctx.cbke_partner_address.addr_long, remote_device_addr);
            ZB_SCHEDULE_ALARM(zc_update_link_key, 0, ZB_TIME_ONE_SECOND * 20);
#endif
        }
        break;
