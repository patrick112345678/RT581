...
#define ZB_CONSOLE_MONITOR_MAX_PAYLOAD_SIZE 254
...
{
    zb_uint8_t payload[ZB_CONSOLE_MONITOR_MAX_PAYLOAD_SIZE];
} ZB_PACKED_STRUCT
...
typedef struct zb_console_monitor_ctx_s
{
    ...
    zb_uint8_t max_length;        /* Length of Data buffer */
    zb_uint8_t pos;               /* Written */
    zb_bool_t rx_ready;           /* Monitor ready to RX */
    volatile zb_bool_t cmd_recvd;
    zb_ret_t status;

#ifdef ZB_MULTITEST_USE_STDIO
    zb_osif_file_t *input_file;
    zb_osif_file_t *output_file;
#endif
