...
zb_ieee_addr_t g_ed_addr = {0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void test_kill_devices(zb_uint8_t param)
{
  ZVUNUSED(param);

  TRACE_MSG(TRACE_ZLL3, "> test_kill_devices", (FMT__0));

  //ZB_SCHEDULE_ALARM_CANCEL(zll_scan_res_timeout, ZB_ALARM_ANY_PARAM);

  ZLL_TRAN_CTX().n_device_infos = 0;
...
void test_next_step(zb_uint8_t param)
{
  ...
  case TEST_STEP_INITIAL:
    ...
    ZB_SCHEDULE_ALARM(test_kill_devices, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(500));
