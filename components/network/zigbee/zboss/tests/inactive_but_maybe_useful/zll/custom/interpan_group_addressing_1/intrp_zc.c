...
enum test_states_e
{
    ...
    TEST_STATE_PACKET2_SENT       = 2,
    TEST_STATE_FINISHED           = 3,
    MAX_REPORTED_TEST_STEP_NUMBER = 2,
};
...
zb_uint8_t g_test_result[MAX_REPORTED_TEST_STEP_NUMBER] = {0, 0};
zb_uint8_t g_test_result_ethalone[MAX_REPORTED_TEST_STEP_NUMBER] =
{
    PACKET_SENT_MASK | DEVICE1_RESPONDED_MASK,
    PACKET_SENT_MASK | DEVICE1_RESPONDED_MASK | DEVICE2_RESPONDED_MASK
};
...
zb_uint16_t g_dst_pan_id = ZB_INTRP_BROADCAST_SHORT_ADDR;
/* Address GroupId for current transaction via step number (before step change) */
zb_uint16_t g_dst_group_id[2] = {INTRP_TEST_GROUP_ID_1, ZB_INTRP_BROADCAST_SHORT_ADDR};
...
/* CR:MEDIUM readme information is not actual for this test */
/*! Test step */
void next_step(zb_uint8_t param)
{
    ...
    case TEST_STATE_PACKET1_SENT:
        TRACE_MSG(TRACE_APS3, "Sending packet 2", (FMT__0));

        send_intrp_data(param);
        g_test_state = TEST_STATE_PACKET2_SENT;
        break;
    case TEST_STATE_PACKET2_SENT:
    ...
    {
        for (cnt = 0; cnt < MAX_REPORTED_TEST_STEP_NUMBER; ++cnt)
        {
            if (g_test_result[cnt] != g_test_result_ethalone[cnt])
            {
                TRACE_MSG(
                    TRACE_ERROR,
                    "ERROR Test step %hd failed with result %hd",
                    (FMT__H_H, cnt, g_test_result[cnt]));
                ++g_error_cnt;
            }
        }
...
void send_intrp_data(zb_uint8_t param)
{
    ...
    /* CR:MEDIUM No need to copy 16 bit values, assign values directly */
    ZB_MEMCPY(
        &(request->dst_addr.addr_short),
        &g_dst_group_id[g_test_state],
        /* CR:MINOR not obviouse result of sizeof() operation in this case */
...
void zb_intrp_data_confirm(zb_uint8_t param)
{
    ...
    else if (*data < MAX_REPORTED_TEST_STEP_NUMBER)
    {
        g_test_result[*data] |= PACKET_SENT_MASK;
    }
...
void zb_intrp_data_indication(zb_uint8_t param)
{
    ...
    /* CR:MEDIUM copy g_test_profile_id to tmp_val, then compare it. What for?
       compare indication, it can be referenced without copying */
    ...
    /* CR:MEDIUM copy g_test_cluster_id to tmp_val, then compare it. What  for? */
