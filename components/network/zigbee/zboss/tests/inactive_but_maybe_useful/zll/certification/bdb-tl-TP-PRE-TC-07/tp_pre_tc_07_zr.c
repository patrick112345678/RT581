...
//  zb_secur_setup_preconfigured_key(g_master_key, 0);
//  ZB_NIB_SET_USE_PRECONFIGURED_KEY(ZB_TRUE);
  ...
  ZDO_CTX().conf_attr.permit_join_duration = 0xff;
  secur_tc_init();
  ...
  /* Skip all explicit security-related assignments except keys contents and
   * info, if required. In our case keep defaults filled in
