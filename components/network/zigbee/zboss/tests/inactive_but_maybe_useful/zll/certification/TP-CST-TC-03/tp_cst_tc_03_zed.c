...
enum test_step_e
{
  ...
  TEST_STEP_SEND_REMOVE_GRP,    /**< ZED send unicasts a ZCL "remove all groups" command to gZR. */
  TEST_STEP_SEND_ADD_GRP,       /**< ZED send unicasts a ZCL "add group" command to gZR to group 1. */
  ...
  TEST_STEP_SEND_TOGGLE2,       /**< ZED send unicasts a ZCL "toggle" command to gZR. */
...
void test_next_step(zb_uint8_t param)
{
  ...
  case TEST_STEP_START:
    TRACE_MSG(TRACE_ZLL3, "TEST_STEP_SEND_REMOVE_GRP", (FMT__0));
    g_test_step = TEST_STEP_SEND_REMOVE_GRP;
  ...
  case TEST_STEP_SEND_REMOVE_GRP:
    TRACE_MSG(TRACE_ZLL3, "TEST_STEP_SEND_ADD_GRP", (FMT__0));
    g_test_step = TEST_STEP_SEND_ADD_GRP;
    ZB_ZCL_GROUPS_SEND_ADD_GROUP_REQ(buffer, DST_ADDR, DST_ADDR_MODE, DST_ENDPOINT,
        ENDPOINT, ZB_AF_ZLL_PROFILE_ID, ZB_FALSE, NULL, 1);
    break;

  case TEST_STEP_SEND_ADD_GRP:
