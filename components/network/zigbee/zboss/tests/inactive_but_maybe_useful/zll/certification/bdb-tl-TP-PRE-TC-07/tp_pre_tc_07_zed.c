...
MAIN()
{
...
#if 0 // not required if bdb def ZB_SECURITY
    ...
    */
    ...
    /* Setup well-known nwk key to simplify packet view in Wireshark */
...
{
    zb_bool_t next_step = ZB_FALSE;
    zb_zdo_app_signal_type_t sig = zb_get_app_signal(param, NULL);

    TRACE_MSG(TRACE_ZLL3, "> zb_zdo_startup_complete %hd sig %hd status %hd",
    ...
    case ZB_BDB_SIGNAL_TOUCHLINK:
        ...
        {
            TRACE_MSG(TRACE_APS1, "Touchlink target started", (FMT__0));
        ...
        {
            TRACE_MSG(TRACE_ZLL3, "Device start FAILED", (FMT__0));
        }
        break;
    case ZB_BDB_SIGNAL_TOUCHLINK_NWK:
        TRACE_MSG(TRACE_APS1, "Touchlink target : network started", (FMT__0));
        break;
    default:
    ...
    case ZB_ZLL_DEVICE_DISCOVERY_TASK:
        TRACE_MSG(TRACE_ZLL3, "Device discovery status %i", (FMT__H, ZB_GET_APP_SIGNAL_STATUS(param)));
        if (ZB_GET_APP_SIGNAL_STATUS(param) == ZB_ZLL_TASK_STATUS_FINISHED)
        {
...
#endif
    TRACE_MSG(TRACE_ZLL3, "g_test_step %d next_step %d", (FMT__D_D, g_test_step, next_step));
    ...
    */
    ...
    if (ZB_ZLL_IS_FACTORY_NEW())
    {
        /*
            ...
            {
              TRACE_MSG(TRACE_ZLL3, "New Network status %hd", (FMT__H, ZB_GET_APP_SIGNAL_STATUS(param)));      TRACE_MSG(TRACE_ERROR, "ERROR extended PAN Id is not zero: %s",
