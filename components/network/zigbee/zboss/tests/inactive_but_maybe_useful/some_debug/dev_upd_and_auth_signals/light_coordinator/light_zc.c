...
MAIN()
{
  ...
  MAC_ADD_VISIBLE_LONG(bulb_addr);
    ...
    switch (dev->match_step)
    {
      case MATCH_STEP_ON_OFF_LVL_CTRL_SERVER:
        if (resp->match_len > 0)
        {
          ...
          ZB_SCHEDULE_CALLBACK2(device_assign, buf_ref, ZB_ARRAY_IDX_BY_ELEM(zc_ctx.devices, dev));
        }
        else
        {
          dev->match_step = MATCH_STEP_ON_OFF_LVL_CTRL_CLIENT;
          ZB_SCHEDULE_CALLBACK2(device_match, buf_ref, ZB_ARRAY_IDX_BY_ELEM(zc_ctx.devices, dev));
        }
        break;
      case MATCH_STEP_ON_OFF_LVL_CTRL_CLIENT:
        if (resp->match_len > 0)
...
void permit_joining_cb(zb_uint8_t param) ZB_CALLBACK
{
  TRACE_MSG(TRACE_ERROR, "permit joining started", (FMT__0));
  zb_buf_free(param);
}

void disable_zc_permit_joining(zb_uint8_t param)
{
  zb_bufid_t buf = zb_buf_get_out();
  zb_zdo_mgmt_permit_joining_req_param_t *req_param =
    ZB_BUF_GET_PARAM(buf, zb_zdo_mgmt_permit_joining_req_param_t);

  TRACE_MSG(TRACE_ERROR, "> send_permit_joinig", (FMT__0));

  req_param->dest_addr = 0x0000;
  req_param->permit_duration = 0x00;
...
static void device_annce_handler(zb_uint8_t buf_ref)
{
  ...
  //try to found slot for device
  {
    ...
    /* ZB_SCHEDULE_CALLBACK(disable_zc_permit_joining, 0); */
  }
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      ...
      case ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ...
        ZB_SCHEDULE_ALARM(device_annce_handler, param, ZB_TIME_ONE_SECOND * 5);
        param = 0;
        break;
#endif  /* ZC_AUTO_SEARCH_AND_BIND */
    }
