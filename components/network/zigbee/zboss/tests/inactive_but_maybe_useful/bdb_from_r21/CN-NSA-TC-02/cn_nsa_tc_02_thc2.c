 ...
 * DSR.
 */
/* PURPOSE: TH ZC2 (sending malformed beacons)
*/

#define ZB_TEST_NAME CN_NSA_TC_02_THC2
#define ZB_TRACE_FILE_ID 41114
#include "zb_common.h"
...
MAIN()
{
    ...
    ZB_IEEE_ADDR_COPY(ZB_PIBCACHE_EXTENDED_ADDRESS(), &g_ieee_addr_thc2);
    ...
    ZB_BDB().bdb_primary_channel_set = TEST_BDB_THC2_PRIMARY_CHANNEL;
...
static void thc_test_fsm(zb_uint8_t unused)
{
    ...
    switch (s_step_idx)
    {
    case THC_CLOSE_NETWORK:
        TRACE_MSG(TRACE_APS2, "Close network", (FMT__0));
        ZB_GET_OUT_BUF_DELAYED(close_network);
        break;
...
static void close_network(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_APS2, ">>close_network", (FMT__0));

    TRACE_MSG(TRACE_APS2, "Closing permit join on device, buf = %d",
              (FMT__D, ZB_REF_FROM_BUF(buf)));
...
static void thc_set_beacon_pl(zb_uint8_t param)
{
    ...
    {
    case THC_SET_ILLEGAL_BEACON_PAYLOAD_1:
        pl->stack_profile = TEST_THC2_INVALID_STACK_PROFILE;
        break;
    ...
    case THC_SET_SHORT_BEACON_PAYLOAD:
        req->pib_length = TEST_TCH2_SHORT_BEACON_PL_LEN;
        ptr = (zb_uint8_t *) pl;
