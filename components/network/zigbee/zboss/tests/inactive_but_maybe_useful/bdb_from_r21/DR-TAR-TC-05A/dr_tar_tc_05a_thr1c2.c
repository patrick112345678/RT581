...
#define ZB_TEST_NAME DR_TAR_TC_05A_THR1C2
#define ZB_TRACE_FILE_ID 41226
#include "zb_common.h"
...
static void send_mgmt_leave_req(zb_uint8_t param);
static void mgmt_leave_cb(zb_uint8_t param);
...
MAIN()
{
    ...
    ZB_IEEE_ADDR_COPY(ZB_PIBCACHE_EXTENDED_ADDRESS(), &g_ieee_addr_thr1c2);
...
static void top_level_fsm(zb_uint8_t unused)
{
    ...
    case TOP_LEVEL_CLOSE_NETWORK:
    {
        ...
        ZDO_CTX().handle.allow_auth = ZB_TRUE;
        ZB_SCHEDULE_ALARM(top_level_forward_test, 0, THR1C2_START_TEST_DELAY);
    }
    break;
    ...
    break;
    case TOP_LEVEL_SEND_MGMT_LEAVE:
    {
        ZDO_CTX().handle.allow_auth = ZB_FALSE;
        ZB_GET_OUT_BUF_DELAYED(send_mgmt_leave_req);
    }
    break;
    default:
    {
...
static void top_level_forward_test(zb_uint8_t unused)
{
    ...
    case TOP_LEVEL_MGMT_LQI:
    {
        TRACE_MSG(TRACE_APP1, "top_level_forward_test: go to next step", (FMT__0));
        ++s_test_desc.top_level_state;
        ZB_SCHEDULE_ALARM(top_level_fsm, 0, THR1C2_SEND_MGMT_LEAVET_DELAY);
    }
    break;
...
static void send_mgmt_leave_req(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_ZDO1, ">>send_mgnt_leave_req: buf_param = %d", (FMT__D, param));

    req->dst_addr = s_test_desc.dest_addr;
    ZB_IEEE_ADDR_COPY(req->device_address, g_ieee_addr_dut);
    req->remove_children = 0;
    req->rejoin = 0;
    zdo_mgmt_leave_req(param, mgmt_leave_cb);

    TRACE_MSG(TRACE_ZDO1, "<<send_mgnt_leave_req", (FMT__0));
}
...
static void mgmt_leave_cb(zb_uint8_t param)
{
    ...
    TRACE_MSG(TRACE_ZDO1, ">>mgmt_leave_cb: buf_param = %d", (FMT__D, param));

    zb_free_buf(buf);
    top_level_forward_test(0);

    TRACE_MSG(TRACE_ZDO1, "<<mgmt_leave_cb", (FMT__0));
}
