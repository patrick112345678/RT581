 ...
 * DSR.
 */
/* PURPOSE: THr1. Join, then F&B in Target mode, then accept join from THe1
*/


#define ZB_TEST_NAME FB_PRE_TC_01A_ZR1
#define ZB_TRACE_FILE_ID 41213
#include "zb_common.h"
...
static void cancel_fb_target(zb_uint8_t param)
{
  (void)param;
  TRACE_MSG(TRACE_APS1, "Cancel f&b target", (FMT__0));
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_STEERING:
        TRACE_MSG(TRACE_APS1, "Successfull steering. Start F&B target", (FMT__0));
      //binding
        zb_bdb_finding_binding_target(DUT_ENDPOINT);
        /* stop f&b before 180s timeout to run the test faster */
        ZB_SCHEDULE_ALARM(cancel_fb_target, 0, ZB_MILLISECONDS_TO_BEACON_INTERVAL(5*1000));
        break;
