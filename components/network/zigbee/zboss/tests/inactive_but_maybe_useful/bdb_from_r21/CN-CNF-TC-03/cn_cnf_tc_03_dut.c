...
#define ZB_TEST_NAME CN_CNF_TC_03_DUT
#define ZB_TRACE_FILE_ID 41171
#include "zb_common.h"
...
static void trigger_network_formation(zb_uint8_t param);
...
MAIN()
{
  ...
  ZB_BDB().bdb_mode = 1;
  /* DUT is capable to form a distributed network */
...
static void close_network(zb_uint8_t param)
{
  ...
  if (buf)
  {
    ...
    TRACE_MSG(TRACE_APS1, "Closing permit join on device, buf = %d",
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
        ...
        ZB_SCHEDULE_ALARM(close_network, 0, TEST_CLOSE_NETWORK_DELAY);
        ZB_SCHEDULE_ALARM(trigger_network_formation, 0, TEST_DUT_NETWORK_FORMATION_DELAY);
        break;
      default:
