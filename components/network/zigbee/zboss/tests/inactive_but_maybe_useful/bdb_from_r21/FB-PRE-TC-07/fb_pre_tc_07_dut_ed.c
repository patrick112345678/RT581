...
#define ZB_TEST_NAME FB_PRE_TC_07_DUT_ED
#define ZB_TRACE_FILE_ID 41076
#include "zb_common.h"
...
/********************* Declare device **************************/
DECLARE_DUT_CLUSTER_LIST(dut_device_clusters,
                         basic_attr_list,
                         identify_attr_list,
                         temp_meas_attr_list,
                         illum_meas_attr_list,
                         rel_humidity_meas_attr_list);

DECLARE_DUT_EP(dut_device_ep,
               DUT_ENDPOINT,
               dut_device_clusters);

DECLARE_DUT_CTX(dut_device_ctx, dut_device_ep);
/*************************************************************************/
...
MAIN()
{
    ...
    ZB_BDB().bdb_mode = 1;
    ZB_CERT_HACKS().dst_binding_table_size = DUT_DST_BIND_TABLE_SIZE;

    ZB_AIB().aps_use_nvram = 1;

    ZB_NIB_DEVICE_TYPE() = ZB_NWK_DEVICE_TYPE_ED;
    ZB_AF_REGISTER_DEVICE_CTX(&dut_device_ctx);
