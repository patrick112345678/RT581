 ...
 * DSR.
 */
/* PURPOSE: DUT ZED (target, Groups Server)
*/


#define ZB_TEST_NAME FB_PRE_TC_06_DUT_ED
#define ZB_TRACE_FILE_ID 41233
#include "zb_common.h"
...
static void trigger_identifying(zb_uint8_t unused);
...
{
  ZVUNUSED(unused);
  zb_zcl_start_identifying(DUT_ENDPOINT, ZB_BDB().bdb_commissioning_time);
}
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_FINDING_AND_BINDING_TARGET_FINISHED:
        ...
        {
          ZB_SCHEDULE_ALARM(trigger_identifying, 0, DUT_START_IDENTIFYING_DELAY);
        }
        break;
