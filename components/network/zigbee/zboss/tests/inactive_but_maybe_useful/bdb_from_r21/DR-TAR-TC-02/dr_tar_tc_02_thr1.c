...
#define ZB_TEST_NAME DR_TAR_TC_02_THR1
#define ZB_TRACE_FILE_ID 41091
#include "zb_common.h"
...
DECLARE_ON_OFF_CLIENT_EP(on_off_client_ep, THR1_ENDPOINT, on_off_client_clusters);
...
{
  TOP_LEVEL_INIT,
  TOP_LEVEL_RTFD_TO_EP_X,
  TOP_LEVEL_RTFD_TO_BRCAST_EP,
  TOP_LEVEL_BRCAST_RTFD,
  TOP_LEVEL_RTFD_NEGATIVE_TEST
};
...
{
  LOW_LEVEL_INITIAL_MGMT_BIND, /* only for TOP_LEVEL_INIT state */
  LOW_LEVEL_READ_ATTR_X_1,
  LOW_LEVEL_TOGGLE_ON_OFF_X,
  LOW_LEVEL_READ_ATTR_Y_1,
  LOW_LEVEL_SWITCH_ON_Y,
  LOW_LEVEL_READ_REPORTING_CONFIG_X_1,
  LOW_LEVEL_CHANGE_REPORTING_CONFIG_X_1,
  LOW_LEVEL_READ_REPORTING_CONFIG_X_2,
  LOW_LEVEL_SEND_RTFD,
  LOW_LEVEL_READ_REPORTING_CONFIG_X_3,
  LOW_LEVEL_READ_ATTR_X_2,
  LOW_LEVEL_MGMT_BIND,
  LOW_LEVEL_READ_ATTR_Y_2
};
...
typedef struct test_params_s
{
  int         top_level_state;
  int         low_level_state;
  int         forced_forwarding;
  zb_uint16_t min_report_int;
  zb_uint16_t max_report_int;
  zb_bool_t   ep_x_attr_on_off_value;
  zb_bool_t   ep_y_attr_on_off_value;
  zb_uint16_t dest_addr;
  zb_uint8_t  dest_ep;
  zb_uint16_t func_params[MAX_FUNC_PARAMS];
  zb_uint16_t rtfd_dest_addr;
  zb_uint8_t  rtfd_dest_ep;
} test_params_t;
...
static void copy_on_off_value(zb_buf_t *buf, zb_uint8_t src_ep);
...
static void top_level_fsm(zb_uint8_t unused)
{
  ...
  switch (s_test_desc.top_level_state)
  {
    case TOP_LEVEL_INIT:
      {
        s_test_desc.low_level_state = LOW_LEVEL_INITIAL_MGMT_BIND;
      }
      break;
    case TOP_LEVEL_RTFD_TO_EP_X:
      {
        s_test_desc.low_level_state = LOW_LEVEL_READ_ATTR_X_1;
        s_test_desc.rtfd_dest_addr  = zb_address_short_by_ieee(g_ieee_addr_dut);
        s_test_desc.rtfd_dest_ep    = DUT_ENDPOINT1;
      }
      break;
    case TOP_LEVEL_RTFD_TO_BRCAST_EP:
      {
        s_test_desc.low_level_state = LOW_LEVEL_READ_ATTR_X_1;
        s_test_desc.rtfd_dest_addr  = zb_address_short_by_ieee(g_ieee_addr_dut);
        s_test_desc.rtfd_dest_ep    = 0xff;
      }
      break;
    case TOP_LEVEL_BRCAST_RTFD:
      {
        s_test_desc.forced_forwarding = 1;
        s_test_desc.low_level_state = LOW_LEVEL_READ_ATTR_X_1;
        s_test_desc.rtfd_dest_addr  = 0xffff;
        s_test_desc.rtfd_dest_ep    = 0xff;
      }
      break;
    case TOP_LEVEL_RTFD_NEGATIVE_TEST:
      {
        s_test_desc.forced_forwarding = 1;
        s_test_desc.low_level_state = LOW_LEVEL_READ_ATTR_X_1;
        s_test_desc.rtfd_dest_addr  = zb_address_short_by_ieee(g_ieee_addr_dut);
        s_test_desc.rtfd_dest_ep    = DUT_ENDPOINT_INACTIVE;
      }
      break;
    default:
      {
  ...
  if (run_low_level_fsm)
  {
    TRACE_MSG(TRACE_APP2, "top_level_fsm: rtfd_dest_addr = 0x%x, rtfd_dest_ep = %d",
              (FMT__H_D, s_test_desc.rtfd_dest_addr, s_test_desc.rtfd_dest_ep));
    low_level_fsm(0);
  }
...
static void low_level_fsm(zb_uint8_t unused)
{
  ...
  switch (s_test_desc.low_level_state)
  {
    ...
    case LOW_LEVEL_READ_ATTR_X_1:
    case LOW_LEVEL_READ_ATTR_X_2:
      {
        next_func = send_read_attr_req;
      }
      break;

    case LOW_LEVEL_TOGGLE_ON_OFF_X:
      {
        s_test_desc.func_params[0] = !s_test_desc.ep_x_attr_on_off_value;
        next_func = send_toggle_on_off;
      }
      break;
    case LOW_LEVEL_SWITCH_ON_Y:
      {
        s_test_desc.dest_ep = DUT_ENDPOINT2;
        s_test_desc.func_params[0] = !s_test_desc.ep_x_attr_on_off_value;
        next_func = send_switch_on;
      }
      break;

    case LOW_LEVEL_READ_REPORTING_CONFIG_X_1:
    case LOW_LEVEL_READ_REPORTING_CONFIG_X_2:
    case LOW_LEVEL_READ_REPORTING_CONFIG_X_3:
      {
        next_func = send_read_reporting_config_req;
      }
      break;

    case LOW_LEVEL_CHANGE_REPORTING_CONFIG_X_1:
      {
    ...
    case LOW_LEVEL_SEND_RTFD:
      {
        next_func = send_reset_to_factory_defaults;
      }
      break;

    case LOW_LEVEL_MGMT_BIND:
      {
        ZB_GET_OUT_BUF_DELAYED2(send_mgmt_bind_req, 0);
      }
      break;

    case LOW_LEVEL_READ_ATTR_Y_1:
    case LOW_LEVEL_READ_ATTR_Y_2:
      {
        s_test_desc.dest_ep = DUT_ENDPOINT2;
        next_func = send_read_attr_req;
      }
      ...
      && cmd_info->is_common_command)
  {
    switch (cmd_info->cmd_id)
    {
      ...
      case ZB_ZCL_CMD_READ_ATTRIB_RESP:
        copy_on_off_value(buf, cmd_info->addr_data.common_data.src_endpoint);
        is_command_of_interest = 1;
        break;
...
static void copy_on_off_value(zb_buf_t *buf, zb_uint8_t src_ep)
{
  ...
  if ( (resp->status == ZB_ZCL_STATUS_SUCCESS) &&
       (resp->attr_id == ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID) )
  {
    switch (src_ep)
    {
      case DUT_ENDPOINT1:
        s_test_desc.ep_x_attr_on_off_value = (zb_bool_t) resp->attr_value;
        break;
      case DUT_ENDPOINT2:
        s_test_desc.ep_y_attr_on_off_value = (zb_bool_t) resp->attr_value;;
        break;
    }
  }
}
...
static void top_level_forward_test(zb_uint8_t unused)
{
  ...
  if (forward_test)
  {
    ZB_SCHEDULE_CALLBACK(top_level_fsm, 0);
  }
...
static void low_level_forward_test(zb_uint8_t unused)
{
  ...
  if (move_chain)
  {
    ZB_SCHEDULE_ALARM(low_level_fsm, 0, THR1_SHORT_DELAY);
  }

  if (forward_test)
  {
    ZB_SCHEDULE_ALARM(top_level_forward_test, 0, THR1_DELAY);
  }
...
static void send_switch_on(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_ZCL2, ">>send_switch_on: buf_param = %d", (FMT__D, param));

  ZB_ZCL_ON_OFF_SEND_ON_REQ(buf, dest_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
                            dest_ep, THR1_ENDPOINT, ZB_AF_HA_PROFILE_ID,
                            ZB_ZCL_ENABLE_DEFAULT_RESPONSE, NULL);

  TRACE_MSG(TRACE_ZCL2, "<<send_switch_on", (FMT__0));
}
...
{
  zb_buf_t *buf = ZB_BUF_FROM_REF(param);
  zb_uint16_t dest_addr = s_test_desc.rtfd_dest_addr;
  zb_uint8_t dest_ep = s_test_desc.rtfd_dest_ep;

  TRACE_MSG(TRACE_ZCL2, ">>send_reset_to_factory_defaults: buf_param = %d", (FMT__D, param));

  ZB_ZCL_BASIC_SEND_RESET_REQ(buf, dest_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
                              dest_ep, THR1_ENDPOINT, ZB_AF_HA_PROFILE_ID,
                              ZB_ZCL_ENABLE_DEFAULT_RESPONSE, NULL);

  if (s_test_desc.forced_forwarding)
  {
    s_test_desc.forced_forwarding = 0;
    ZB_SCHEDULE_ALARM(low_level_forward_test, 0, THR1_DELAY);
  }

  TRACE_MSG(TRACE_ZCL2, "<<send_reset_to_factory_defaults", (FMT__0));
}
