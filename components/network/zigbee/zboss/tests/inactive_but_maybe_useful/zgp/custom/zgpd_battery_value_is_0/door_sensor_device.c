...
zb_bool_t inc_voltage = ZB_FALSE;
...
void battery_notification_loop(zb_uint8_t param)
{
  ...
  if(param==0)
  {
    if (g_reporting_index == TEST_REPORT_COUNT)
    {
  ...
  else if (g_commissioned)
  {
    ...
    zb_uint8_t val = (g_reporting_index % 2 ? (inc_voltage ? 30 : 20) : 0);
    ...
    if (g_reporting_index % 2)
    {
      inc_voltage = !inc_voltage;
      TRACE_MSG(TRACE_APP1, "inc_voltage %i", (FMT__H, inc_voltage));
      TRACE_MSG(TRACE_APP1, "g_reporting_index %i", (FMT__H, g_reporting_index));
    }

    ZB_SCHEDULE_ALARM(battery_notification_loop, 0, TEST_REPORT_TIMEOUT);
