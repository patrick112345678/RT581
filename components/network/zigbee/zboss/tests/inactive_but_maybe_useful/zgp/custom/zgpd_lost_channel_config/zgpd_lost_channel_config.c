...
void channel_config_ind_test_cb(zb_uint8_t param);

zb_zgpd_test_callbacks_t test_cbs = 
{
  .zgpd_channel_config_ind_test_cb = channel_config_ind_test_cb,
};

zb_uint8_t g_lost_ch_config_packets_num = 0;
...
void channel_config_ind_test_cb(zb_uint8_t param)
{
  if (g_lost_ch_config_packets_num < LOST_CHANNEL_CONFIG_PACKETS_NUM)
  {
    ++g_lost_ch_config_packets_num;
    TRACE_MSG(TRACE_APP1, "lost ch config packet; g_lost_ch_config_packets_num %i",
              (FMT__H, g_lost_ch_config_packets_num));
    zb_free_buf(ZB_BUF_FROM_REF(param));
  }
  else
  {
    zgpd_channel_config_ind(param);
  }
}
