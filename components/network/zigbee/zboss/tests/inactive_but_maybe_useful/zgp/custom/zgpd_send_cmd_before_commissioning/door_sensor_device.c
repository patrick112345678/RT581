...
void battery_notification_loop(zb_uint8_t param)
{
    ...
    else
    {
        if (g_notificaions_num < TEST_NOTIFICATIONS_NUM)
        {
            ...
            zb_uint8_t val =
                100 - volt_diff * g_notificaions_num;
...
{
    zgpd_set_channel_and_call(param, battery_notification_loop);
}
...
void zgpd_startup_complete(zb_uint8_t param)
{
    ...
    if (buf->u.hdr.status == RET_OK)
    {
        ...
        ZB_GET_OUT_BUF_DELAYED(zgpd_set_channel_and_call_delayed);
    }
    else
    {
