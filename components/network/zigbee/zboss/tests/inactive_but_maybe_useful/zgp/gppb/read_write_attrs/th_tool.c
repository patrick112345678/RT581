...
void next_step(zb_buf_t *zcl_cmd_buf)
{
    ...
    else if (g_test_state == TEST_STATE_WRITE_MAX_PROXY_TABLE_ENTRIES)
    {
        val = 0x0a;
    ...
    else if (g_test_state == TEST_STATE_READ_PROXY_TABLE1 ||
    ...
    else if (g_test_state == TEST_STATE_WRITE_PROXY_TABLE)
    {
        val = 0x0000;
        attr_val = (zb_uint8_t *)&val;
        zgp_cluster_write_attr(ZB_REF_FROM_BUF(zcl_cmd_buf), DUT_GPP_ADDR, DUT_GPP_ADDR_MODE,
