...
#define ZB_TEST_NAME GPP_TUNNELING_COMMISSIONED_GROUPCAST_TH_GPT
#define ZB_TRACE_FILE_ID 41301
  ...
  switch (TEST_DEVICE_CTX.test_state)
  {
    ...
    case TEST_STATE_READ_GPP_PROXY_TABLE3:
      zgp_cluster_read_attr(buf_ref, DUT_GPPB_ADDR, DUT_GPPB_ADDR_MODE,
                            ...
                            ZB_ZCL_ENABLE_DEFAULT_RESPONSE, cb);
#ifdef ZB_CERTIFICATION_HACKS
      ZB_CERT_HACKS().gp_sink_use_assigned_alias_for_dgroup_commissioning = 0;
      ZB_CERT_HACKS().gp_sink_assigned_alias = 0;
...
static void perform_next_state(zb_uint8_t param)
{
  ...
  zb_led_blink_on(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
    ...
    case TEST_STATE_READY_TO_COMMISSIONING2:
      ZB_CERT_HACKS().gp_sink_use_assigned_alias_for_dgroup_commissioning = 1;
      ZB_CERT_HACKS().gp_sink_assigned_alias = 0x4444;
      zb_zgps_start_commissioning(ZGP_GPS_GET_COMMISSIONING_WINDOW() *
                                  ZB_TIME_ONE_SECOND);
#ifdef ZB_USE_BUTTONS
      zb_led_blink_on(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
