...
#define ZB_TEST_NAME GPS_CT_BASED_COMMISSIONING_TH_GPD
#define ZB_TRACE_FILE_ID 41518
...
static zb_ieee_addr_t g_th_gpd_addr1 = TH_GPD_IEEE_ADDR1;
static zb_ieee_addr_t g_th_gpd_addr2 = TH_GPD_IEEE_ADDR2;
...
ZB_ZGPD_DECLARE_SIMPLE_TEST_TEMPLATE(TEST_DEVICE_CTX, 40000)
...
static void make_gpdf(zb_buf_t *buf, zb_uint8_t **ptr)
{
    ...
    {
        ZB_IEEE_ADDR_COPY(ZB_PIBCACHE_EXTENDED_ADDRESS(), &g_th_gpd_addr2);
        ZB_IEEE_ADDR_COPY(&g_zgpd_ctx.id.addr.ieee_addr, &g_th_gpd_addr2);
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_TOGGLE);
        break;
    }
    };
...
static void zgp_custom_startup()
{
    ...
    ZB_IEEE_ADDR_COPY(ZB_PIBCACHE_EXTENDED_ADDRESS(), &g_th_gpd_addr1);
    ZB_IEEE_ADDR_COPY(&g_zgpd_ctx.id.addr.ieee_addr, &g_th_gpd_addr1);
