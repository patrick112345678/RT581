...
#define ZB_TEST_NAME GPS_TUNNELING_COMMISSIONED_GROUPCAST_DUT_GPS
#define ZB_TRACE_FILE_ID 41326
...
static zb_bool_t custom_comm_cb(zb_zgpd_id_t *zgpd_id, zb_zgp_comm_status_t result)
{
  ...
  TRACE_MSG(TRACE_APP1, "COMM_CB: %hd", (FMT__H, result));
  ...
  if (result == ZB_ZGP_COMMISSIONING_TIMED_OUT &&
      TEST_DEVICE_CTX.test_state == TEST_STATE_READY_TO_COMMISSIONING)
  {
#ifdef ZB_USE_BUTTONS
    zb_led_blink_off(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
#endif
  ...
  if (result == ZB_ZGP_COMMISSIONING_CANCELLED_BY_USER &&
      TEST_DEVICE_CTX.test_state == TEST_STATE_STOP_COMMISSIONING)
  {
#ifdef ZB_USE_BUTTONS
    zb_led_blink_off(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
#endif
...
static void perform_next_state(zb_uint8_t param)
{
  ...
  zb_led_blink_on(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
      ...
      break;
#ifndef ZB_NSNG
    case TEST_STATE_STOP_COMMISSIONING:
