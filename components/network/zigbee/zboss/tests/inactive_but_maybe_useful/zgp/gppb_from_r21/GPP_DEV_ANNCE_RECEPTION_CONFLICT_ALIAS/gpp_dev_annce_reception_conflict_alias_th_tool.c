...
#define ZB_TEST_NAME GPP_DEV_ANNCE_RECEPTION_CONFLICT_ALIAS_TH_TOOL
#define ZB_TRACE_FILE_ID 41357
...
enum test_states_e
{
  ...
  TEST_READ_OUT_DUT_PROXY_TABLE1,
  TEST_STATE_SEND_PAIRING2,
  TEST_READ_OUT_DUT_PROXY_TABLE2,
  TEST_STATE_FINISHED
};
...
static void send_zcl(zb_uint8_t buf_ref, zb_callback_t cb)
{
  ...
  switch (TEST_DEVICE_CTX.test_state)
  {
    case TEST_STATE_SEND_PAIRING1:
      ...
      ZB_ZGPC_SET_PAUSE(90);
      break;
    case TEST_STATE_SEND_PAIRING2:
      zgpc_send_pairing(buf_ref, ZB_TRUE);
      ZB_ZGPC_SET_PAUSE(10);
      break;
    case TEST_READ_OUT_DUT_PROXY_TABLE1:
    case TEST_READ_OUT_DUT_PROXY_TABLE2:
...
static void zgpc_send_pairing(zb_uint8_t param, zb_bool_t add_sink)
{
  ...
  zb_bool_t use_alias;
  zb_uint32_t src_id;
  zb_uint16_t alias_id;

  ZB_BZERO(&ent, sizeof(zb_zgp_sink_tbl_ent_t));

  if (TEST_DEVICE_CTX.test_state == TEST_STATE_SEND_PAIRING1)
  {
    use_alias = ZB_FALSE;
    alias_id = 0x0000;
    src_id = (TEST_ZGPD_SRC_ID & 0xFFFF0000) | g_dut_addr;
  }
  else
  {
    use_alias = ZB_TRUE;
    src_id = TEST_ZGPD_SRC_ID;
    alias_id = g_dut_addr;
  }
  ...
  ent.zgpd_assigned_alias = alias_id;
