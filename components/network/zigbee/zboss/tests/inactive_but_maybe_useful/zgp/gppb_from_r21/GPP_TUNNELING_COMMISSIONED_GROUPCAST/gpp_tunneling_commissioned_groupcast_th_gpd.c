...
#define ZB_TEST_NAME GPP_TUNNELING_COMMISSIONED_GROUPCAST_TH_GPD
#define ZB_TRACE_FILE_ID 41299
#include "zb_common.h"
...
enum test_states_e
{
  ...
  TEST_STATE_SEND_DATA_GPDF,
  TEST_STATE_DECOMMISSIONING,
  TEST_STATE_FINISHED
};
...
static void make_gpdf(zb_buf_t *buf, zb_uint8_t **ptr)
{
  ...
  {
    case TEST_STATE_SEND_DATA_GPDF:
    {
      if (g_cnt == 2)
      {
...
static void perform_next_state(zb_uint8_t param)
{
  ...
  if (g_cnt == 2 && TEST_STATE_COMMISSIONING)
  {
  ...
  switch (TEST_DEVICE_CTX.test_state)
  {
    case TEST_STATE_COMMISSIONING:
      ...
      ZB_ZGPD_SET_PAUSE(1);
      break;
    case TEST_STATE_DECOMMISSIONING:
