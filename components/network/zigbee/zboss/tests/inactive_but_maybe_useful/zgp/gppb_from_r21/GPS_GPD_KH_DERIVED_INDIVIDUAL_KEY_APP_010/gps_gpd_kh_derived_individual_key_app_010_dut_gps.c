...
#define ZB_TEST_NAME GPS_GPD_KH_DERIVED_INDIVIDUAL_KEY_APP_010_DUT_GPS
#define ZB_TRACE_FILE_ID 41416
...
static void send_gpdf(zb_uint8_t param)
{
    zb_buf_t *buf = ZB_BUF_FROM_REF(param);
    zb_zgps_send_cmd_params_t *params;

    TRACE_MSG(TRACE_APP1, ">>send_gpdf: param = %d", (FMT__D, param));

    params = ZB_GET_BUF_PARAM(buf, zb_zgps_send_cmd_params_t);

    ZB_BZERO(params, sizeof(*params));
    params->handle = ZB_ZGP_HANDLE_DEFAULT_HANDLE;
    params->tx_options = ZB_GP_DATA_REQ_USE_GP_TX_QUEUE;
    params->lifetime = ZB_GP_TX_QUEUE_ENTRY_LIFETIME_INF;
    params->zgpd_id.addr.src_id = TEST_ZGPD_SRC_ID;
    params->zgpd_id.app_id = ZB_ZGP_APP_ID_0000;
    params->cmd_id = ZB_GPDF_CMD_TOGGLE;

    zb_zgps_send_data(param);

    TRACE_MSG(TRACE_APP1, "<<send_gpdf", (FMT__0));
}
...
static zb_bool_t custom_comm_cb(zb_zgpd_id_t *zgpd_id, zb_zgp_comm_status_t result)
{
    ...
    TRACE_MSG(TRACE_APP1, "result = %d, state = %d",
