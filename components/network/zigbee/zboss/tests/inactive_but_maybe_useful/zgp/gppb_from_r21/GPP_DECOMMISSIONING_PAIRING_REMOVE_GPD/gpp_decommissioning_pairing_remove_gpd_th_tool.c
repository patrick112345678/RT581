...
#define ZB_TEST_NAME GPP_DECOMMISSIONING_PAIRING_REMOVE_GPD_TH_TOOL
#define ZB_TRACE_FILE_ID 41481
...
enum test_states_e
{
  ...
  TEST_STATE_TURN_ON_COMMISSIONING_AGAIN,
...
static void perform_next_state(zb_uint8_t param)
{
  ...
  switch (TEST_DEVICE_CTX.test_state)
  {
    ...
    case TEST_STATE_TURN_ON_COMMISSIONING_AGAIN:
...
static zb_bool_t custom_comm_cb(zb_zgpd_id_t *zgpd_id, zb_zgp_comm_status_t result)
{
  ...
  switch (result)
  {
    ...
    case ZB_ZGP_ZGPD_DECOMMISSIONED:
      if (TEST_DEVICE_CTX.test_state == TEST_STATE_TURN_ON_COMMISSIONING_AGAIN)
      {
#ifdef ZB_USE_BUTTONS
        zb_led_blink_off(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
#endif
