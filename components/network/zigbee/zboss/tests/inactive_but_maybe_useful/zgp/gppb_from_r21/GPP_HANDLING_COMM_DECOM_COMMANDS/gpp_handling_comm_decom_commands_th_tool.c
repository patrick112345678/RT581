...
#define ZB_TEST_NAME GPP_HANDLING_COMM_DECOM_COMMANDS_TH_TOOL
#define ZB_TRACE_FILE_ID 41353
...
enum test_states_e
{
  ...
  TEST_STATE_WAIT_FOR_NC_COMPLETED, /* skip negative test cases */
  TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE1,
  TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID1,
  TEST_STATE_WAIT_FOR_DECOMMISSIONING,
  TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE2,
  TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID2,
  TEST_STATE_FINISHED
};
...
static void send_zcl(zb_uint8_t buf_ref, zb_callback_t cb)
{
  ...
  {
    case TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE1:
    case TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE2:
      ...
      ZB_ZGPC_SET_PAUSE(5);
      break;
    case TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID1:
    case TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID2:
      zgp_cluster_read_attr(buf_ref, g_dut_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
...
static void perform_next_state(zb_uint8_t param)
{
  ...
  switch (TEST_DEVICE_CTX.test_state)
  {
    ...
    case TEST_STATE_READY_TO_COMMISSIONING:
      /* Transition to next state should be performed in custom_comm_cb */
...
#endif
      break;
    case TEST_STATE_WAIT_FOR_NC_COMPLETED:
      /* just wait */
      ZB_ZGPC_SET_PAUSE(TEST_WAIT_FOR_NC_DELAY);
      ZB_SCHEDULE_ALARM(test_send_command, 0, ZB_TIME_ONE_SECOND);
      break;
    case TEST_STATE_WAIT_FOR_DECOMMISSIONING:
      /* Transition to next state should be performed in custom_comm_cb */
      break;
...
static zb_bool_t custom_comm_cb(zb_zgpd_id_t *zgpd_id, zb_zgp_comm_status_t result)
{
  ...
  switch (result)
  {
    ...
    case ZB_ZGP_ZGPD_DECOMMISSIONED:
      if (TEST_DEVICE_CTX.test_state == TEST_STATE_WAIT_FOR_DECOMMISSIONING)
      {
#ifdef ZB_USE_BUTTONS
        zb_led_blink_off(ZB_LED_ARG_CREATE(3, ZB_LED_BLINK_HALF_SEC));
#endif
