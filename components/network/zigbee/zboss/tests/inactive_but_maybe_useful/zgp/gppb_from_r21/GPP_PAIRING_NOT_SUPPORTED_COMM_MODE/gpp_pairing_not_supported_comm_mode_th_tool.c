 ...
 * DSR.
 */
/* PURPOSE: TH Tool (and ZC) for test GPP_PAIRING_NOT_SUPPORTED_COMM_MODE
*/

#define ZB_TEST_NAME GPP_PAIRING_NOT_SUPPORTED_COMM_MODE_TH_TOOL
#define ZB_TRACE_FILE_ID 41410
#include "zb_common.h"
#include "zboss_api.h"
...
{
    TEST_STATE_INITIAL,
    TEST_STATE_READ_GPP_FUNCTIONALITY,
...
static void send_zcl(zb_uint8_t buf_ref, zb_callback_t cb)
{
    ...
    {
    case TEST_STATE_READ_GPP_FUNCTIONALITY:
        zgp_cluster_read_attr(buf_ref, g_dut_short_address, ZB_ADDR_16BIT_DEV_OR_BROADCAST,
    ...
    case TEST_STATE_READ_GPP_PROXY_TABLE2:
        id.addr.src_id = TH_TOOL_GPD_ID_ADDR_POOL_START_ADDR;
...
static void zgpc_send_pairing(zb_uint8_t param, zb_bool_t add_sink)
{
    ...
    ent.options = ZGP_TBL_SINK_FILL_OPTIONS(
                      ZB_ZGP_APP_ID_0000,
                      ZGP_COMMUNICATION_MODE_FULL_UNICAST,
                      ZGPD_SEQ_NUM_CAP,
                      ZGPD_RX_ON_CAP,
                      ZGPD_FIX_LOC,
    ...
    ent.zgpd_id.src_id = TH_TOOL_GPD_ID_ADDR_POOL_START_ADDR;
    ent.security_counter = TH_TOOL_START_MAC_DSN_VALUE - 1;
