...
#define ZB_TEST_NAME GPP_DECOMMISSIONING_PRECOMM_GROUPCAST_TH_TOOL
#define ZB_TRACE_FILE_ID 41461
...
static zb_uint16_t g_sink2_short_addr = ZB_MAC_SHORT_ADDR_NOT_ALLOCATED;
static zb_ieee_addr_t g_sink2_long_addr = TH_GPS2_IEEE_ADDR;
...
enum test_states_e
{
  ...
  TEST_STATE_INIT_CREATE_PAIRING_ON_DUT,
  TEST_STATE_INIT_READ_OUT_DUT_GPP_PROXY_TABLE,
  TEST_STATE_REMOVE_PAIRING,
  TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE,
  TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID,
  TEST_STATE_FINISHED
};
...
static void send_zcl(zb_uint8_t buf_ref, zb_callback_t cb)
{
  ...
  {
    case TEST_STATE_INIT_CREATE_PAIRING_ON_DUT:
      zgpc_send_pairing(buf_ref, ZB_TRUE);
      ZB_ZGPC_SET_PAUSE(10);
      break;
    case TEST_STATE_REMOVE_PAIRING:
      zgpc_send_pairing(buf_ref, ZB_FALSE);
      ZB_ZGPC_SET_PAUSE(10);
      break;
    case TEST_STATE_INIT_READ_OUT_DUT_GPP_PROXY_TABLE:
    case TEST_STATE_READ_OUT_DUT_GPP_PROXY_TABLE:
      ...
      ZB_ZGPC_SET_PAUSE(5);
      break;
    case TEST_STATE_READ_OUT_DUT_GPP_BLOCKED_ID:
      zgp_cluster_read_attr(buf_ref, g_dut_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT,
...
static void zgpc_send_pairing(zb_uint8_t param, zb_bool_t add_sink)
{
  ...
  ent.u.sink.sgrp[0].sink_group = SINK_GROUP_NNNN;
...
static void dev_annce_cb(zb_zdo_device_annce_t *da)
{
  ...
  else if (ZB_IEEE_ADDR_CMP(g_sink2_long_addr, da->ieee_addr) == ZB_TRUE)
  {
    g_sink2_short_addr = da->nwk_addr;
  }

  if ((TEST_DEVICE_CTX.test_state == TEST_STATE_WAIT_FOR_DUT_STARTUP)
      && (g_dut_addr != ZB_MAC_SHORT_ADDR_NOT_ALLOCATED)
      && (g_sink2_short_addr != ZB_MAC_SHORT_ADDR_NOT_ALLOCATED))
  {
    ZB_SCHEDULE_ALARM(perform_next_state, 0, 20 * ZB_TIME_ONE_SECOND);
  }
}
