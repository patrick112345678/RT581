/* ZBOSS Zigbee software protocol stack
 *
 * Copyright (c) 2012-2020 DSR Corporation, Denver CO, USA.
 * www.dsr-zboss.com
 * www.dsr-corporation.com
 * All rights reserved.
 *
 * This is unpublished proprietary source code of DSR Corporation
 * The copyright notice does not evidence any actual or intended
 * publication of such source code.
 *
 * ZBOSS is a registered trademark of Data Storage Research LLC d/b/a DSR
 * Corporation
 *
 * Commercial Usage
 * Licensees holding valid DSR Commercial licenses may use
 * this file in accordance with the DSR Commercial License
 * Agreement provided with the Software or, alternatively, in accordance
 * with the terms contained in a written agreement between you and
 * DSR.
 */
/* PURPOSE: Simple ZGPD for send GPDF as described
in 2.3.4 test specification.
*/

#define ZB_TEST_NAME GPP_SECURED_10_DATA_GPDF_APP_010_TH_GPD
#define ZB_TRACE_FILE_ID 41517
#include "zb_common.h"
#include "zgpd/zb_zgpd.h"
#include "test_config.h"
#include "../include/zgp_test_templates.h"

#ifdef ZB_CERTIFICATION_HACKS

static zb_ieee_addr_t g_zgpd_addr = TH_GPD_IEEE_ADDR;
static zb_uint8_t   g_zgpd_key[] = TEST_SEC_KEY;

static zb_uint8_t g_bad_key[] = { 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0, 0xba, 0xd0};

/*! Program states according to test scenario */
enum test_states_e
{
    TEST_STATE_INITIAL,
    TEST_STATE_SET_IEEE,
    TEST_STATE_SET_MAC_DSN_INIT,
    TEST_STATE_COMMISSIONING1,
    TEST_STATE_SET_MAC_DSN,
    TEST_STATE_SEND_DATA_GPDF1,
    TEST_STATE_SEND_DATA_GPDF2,
    TEST_STATE_SEND_DATA_GPDF3,
    TEST_STATE_SEND_DATA_GPDF4,
    TEST_STATE_SEND_DATA_GPDF5,
    TEST_STATE_SET_MAC_DSN1,
    TEST_STATE_SEND_DATA_GPDF6,
    TEST_STATE_SET_MAC_DSN2,
    TEST_STATE_SEND_DATA_GPDF7,
    TEST_STATE_SET_MAC_DSN3,
    TEST_STATE_SEND_DATA_GPDF8,
    TEST_STATE_FINISHED
};

ZB_ZGPD_DECLARE_SIMPLE_TEST_TEMPLATE(TEST_DEVICE_CTX, 2000)

static void make_gpdf(zb_buf_t *buf, zb_uint8_t **ptr)
{
    ZVUNUSED(buf);
    switch (TEST_DEVICE_CTX.test_state)
    {
    case TEST_STATE_SEND_DATA_GPDF1:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_ON);
        ZB_ZGPD_SET_PAUSE(1);
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF2:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_MEMCPY(ZGPD->ch_replace_key, g_bad_key, ZB_CCM_KEY_SIZE);
        ZB_ZGPD_CHACK_SET(ZB_ZGPD_CH_REPLACE_KEY);
        ZB_ZGP_SET_PASSED_STATE_SEQUENCE();
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF3:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        ZGPD->ch_replace_key_type = ZB_ZGP_SEC_KEY_TYPE_ZGPD_INDIVIDUAL;
        ZB_ZGPD_CHACK_SET(ZB_ZGPD_CH_REPLACE_KEY_TYPE);
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF4:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        ZGPD->ch_replace_sec_level = ZB_ZGP_SEC_LEVEL_NO_SECURITY;
        ZB_ZGPD_CHACK_SET(ZB_ZGPD_CH_REPLACE_SEC_LEVEL);
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF5:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        ZGPD->ch_replace_sec_level = ZB_ZGP_SEC_LEVEL_REDUCED;
        ZB_ZGPD_CHACK_SET(ZB_ZGPD_CH_REPLACE_SEC_LEVEL);
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF6:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF7:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        ZB_ZGPD_CHACK_SET(ZB_ZGPD_CH_REPLACE_DIRECTION);
        ZGPD->ch_replace_direction = 1;
        ZB_ZGPD_SET_PAUSE(2);
        ZB_ZGP_RESET_PASSED_STATE_SEQUENCE();
        break;
    }
    case TEST_STATE_SEND_DATA_GPDF8:
    {
        ZB_GPDF_PUT_UINT8(*ptr, ZB_GPDF_CMD_OFF);
        ZB_ZGPD_CHACK_RESET_ALL();
        break;
    }
    };
}

ZB_ZGPD_DECLARE_COMMISSIONING_CALLBACK()

static void zgpd_set_dsn(zb_uint8_t param, zb_callback_t cb)
{
    zgpd_set_dsn_and_call(param, cb);
}

static void zgpd_set_ieee(zb_uint8_t param, zb_callback_t cb)
{
    zgpd_set_ieee_and_call(param, cb);
}

static void perform_next_state(zb_uint8_t param)
{
    if (TEST_DEVICE_CTX.pause)
    {
        ZB_SCHEDULE_ALARM(perform_next_state, 0,
                          ZB_TIME_ONE_SECOND * TEST_DEVICE_CTX.pause);
        TEST_DEVICE_CTX.pause = 0;
        return;
    }
    TEST_DEVICE_CTX.test_state++;

    switch (TEST_DEVICE_CTX.test_state)
    {
    case TEST_STATE_COMMISSIONING1:
        if (param)
        {
            zb_free_buf(ZB_BUF_FROM_REF(param));
        }
        zb_zgpd_start_commissioning(comm_cb);
        ZB_ZGPD_SET_PAUSE(3);
        break;
    case TEST_STATE_SET_IEEE:
    {
        if (param == 0)
        {
            ZB_GET_OUT_BUF_DELAYED(perform_next_state);
            TEST_DEVICE_CTX.test_state--;
            break;
        }
        zgpd_set_ieee(param, perform_next_state);
        break;
    }
    case TEST_STATE_SET_MAC_DSN_INIT:
    {
        if (param == 0)
        {
            ZB_GET_OUT_BUF_DELAYED(perform_next_state);
            TEST_DEVICE_CTX.test_state--;
            break;
        }
        ZGPD->mac_dsn = 0x81;
        ZGPD->security_frame_counter = 0xAA81;
        zgpd_set_dsn(param, perform_next_state);
        break;
    }
    case TEST_STATE_SET_MAC_DSN:
    {
        if (param == 0)
        {
            ZB_GET_OUT_BUF_DELAYED(perform_next_state);
            TEST_DEVICE_CTX.test_state--;
            break;
        }
        ZGPD->mac_dsn = 0xc3;
        ZGPD->security_frame_counter = 0xAAc3;
        zgpd_set_dsn(param, perform_next_state);
        break;
    }
    case TEST_STATE_SET_MAC_DSN1:
    {
        if (param == 0)
        {
            ZB_GET_OUT_BUF_DELAYED(perform_next_state);
            TEST_DEVICE_CTX.test_state--;
            break;
        }
        ZGPD->mac_dsn = 0xc2;
        ZGPD->security_frame_counter = 0xAAc2;
        zgpd_set_dsn(param, perform_next_state);
        break;
    }
    case TEST_STATE_SET_MAC_DSN2:
    case TEST_STATE_SET_MAC_DSN3:
    {
        if (param == 0)
        {
            ZB_GET_OUT_BUF_DELAYED(perform_next_state);
            TEST_DEVICE_CTX.test_state--;
            break;
        }
        ZGPD->mac_dsn = 0xc4;
        ZGPD->security_frame_counter = 0xAAc4;
        zgpd_set_dsn(param, perform_next_state);
        break;
    }
    case TEST_STATE_FINISHED:
        TRACE_MSG(TRACE_APP1, "Test finished. Status: OK", (FMT__0));
        break;
    default:
    {
        if (param)
        {
            zb_free_buf(ZB_BUF_FROM_REF(param));
        }
        ZB_SCHEDULE_ALARM(test_send_command, 0, (zb_time_t)(.1 * ZB_TIME_ONE_SECOND));
    }
    };
}

static void zgp_custom_startup()
{
#if ! (defined KEIL || defined ZB_PLATFORM_LINUX_ARM_2400)
#endif

    /* Init device, load IB values from nvram or set it to default */

    ZB_INIT("th_gpd");


    ZB_ZGPD_INIT_ZGPD_CTX(ZB_ZGP_APP_ID_0010, ZB_ZGPD_COMMISSIONING_UNIDIR, ZB_ZGP_ON_OFF_SWITCH_DEV_ID);

    ZB_IEEE_ADDR_COPY(ZB_PIBCACHE_EXTENDED_ADDRESS(), &g_zgpd_addr);
    ZB_IEEE_ADDR_COPY(&g_zgpd_ctx.id.addr.ieee_addr, &g_zgpd_addr);
    g_zgpd_ctx.id.endpoint = 1;
    ZB_ZGPD_SET_SECURITY_LEVEL(ZB_ZGP_SEC_LEVEL_FULL_NO_ENC);
    ZB_ZGPD_SET_SECURITY_KEY_TYPE(ZB_ZGP_SEC_KEY_TYPE_GROUP);
    ZB_ZGPD_SET_SECURITY_KEY(g_zgpd_key);
    ZB_ZGPD_SET_OOB_KEY(g_zgpd_key);

    ZGPD->channel = TEST_CHANNEL;
}

#endif /* ZB_CERTIFICATION_HACKS */

ZB_ZGPD_TH_DECLARE_STARTUP_PROCESS()
