...
/**
   ...
   Also drop association if an association response is povided in theindirect queue.
   @param param - reference to buffer.
*/
...
/**
  Sends frame to the transmitter with check for manual acknowlege.
...
*/
#define ZB_MAC_LBT_ACK_WINDOW_USEC    (ZB_MAC_LBT_ACK_WINDOW_SYMBOLS * ZB_SUB_GHZ_SYMBOL_DURATION_USEC)
/*!
...
*/
#define ZB_MAC_LBT_TIMEOUT_USEC    (ZB_MAC_LBT_TIMEOUT_SYMBOLS * ZB_SUB_GHZ_SYMBOL_DURATION_USEC)
/*!
...
void zb_mac_flip_bits(zb_uint8_t *buf, zb_uint8_t len);
...
#else
/*
  In Linux nsng sometimes produce delays transporting ACKs to/from device.
  Seems that is Linux limitation: it is not real-time OS.
  To exclude unnecessary retransmits let;s increase ack wait duration.
 */
#define ZB_MAC_PIB_ACK_WAIT_DURATION_CONST 8U
