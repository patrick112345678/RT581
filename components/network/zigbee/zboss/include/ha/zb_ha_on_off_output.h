...
#define ZB_HA_ON_OFF_OUTPUT_IN_CLUSTER_NUM_LED2  4 /*!< On/Off output IN cluster number */
...
/** @brief Declare cluster list for On/Off output device
    ...
    @param identify_attr_list - attribute list for Identify cluster
 */
#define ZB_HA_DECLARE_ON_OFF_OUTPUT_CLUSTER_LIST_LED2(               \
  cluster_list_name,                                            \
  on_off_attr_list,                                             \
  basic_attr_list,                                              \
  identify_attr_list,                                           \
...
/** @brief Declare simple descriptor for On/Off output device
    ...
    definitions, because these values are used to form simple descriptor type name
*/
#define ZB_ZCL_DECLARE_ON_OFF_OUTPUT_SIMPLE_DESC_LED2(ep_name, ep_id, in_clust_num, out_clust_num) \
      ZB_DECLARE_SIMPLE_DESC(in_clust_num, out_clust_num);                                    \
      ZB_AF_SIMPLE_DESC_TYPE(in_clust_num, out_clust_num)  simple_desc_##ep_name =            \
      {                                                                                       \
        ep_id,                                                                                \
...
/** @brief Declare endpoint for On/off output device
    ...
    @param cluster_list - endpoint cluster list
 */
#define ZB_HA_DECLARE_ON_OFF_OUTPUT_EP_LED2(ep_name, ep_id, cluster_list)   \
      ZB_ZCL_DECLARE_ON_OFF_OUTPUT_SIMPLE_DESC_LED2(                        \
          ep_name,                                                     \
          ep_id,                                                       \
          ZB_HA_ON_OFF_OUTPUT_IN_CLUSTER_NUM_LED2,                          \
          ZB_HA_ON_OFF_OUTPUT_OUT_CLUSTER_NUM);                        \
      ZBOSS_DEVICE_DECLARE_REPORTING_CTX(reporting_info## device_ctx_name,           \
                                         ZB_HA_ON_OFF_OUTPUT_REPORT_ATTR_COUNT);     \
      ZB_AF_DECLARE_ENDPOINT_DESC(ep_name,                                           \
...
/** @brief Declare On/Off Output device context.
    ...
    @param ep_name - endpoint variable name.
*/
#define ZB_HA_DECLARE_ON_OFF_OUTPUT_CTX2(device_ctx, ep_name1, ep_name2)      \
