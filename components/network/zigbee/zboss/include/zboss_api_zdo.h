...
//ZED start rejoin
#define ZB_SIGNAL_DEVICE_START_REJOIN 54U

#ifdef ZB_RAF_PERMIT_JOIN_ZDO_SIGNAL
/** Notifies the application that the device permit join changing.
 * @parblock
 * When generated:
 *  - Upon sending the "permit join" command.
 *
 * Status codes:
 *  - RET_OK: Device has sent the "permit join" command.
 *  - Does not return error status.
 *
 * Signal parameters:
 *  - @ref zb_zdo_signal_permit_join_params_t
 *
 * @snippet
 * @endparblock */
#define ZB_ZDO_SIGNAL_PERMIT_JOIN 80U
#endif
...
#ifdef ZB_RAF_PERMIT_JOIN_ZDO_SIGNAL
/**
   @brief permit join parameters

   Stack passes this parameter to application when permit join chaging.
  */
typedef struct zb_zdo_signal_permit_join_params_s
{
  zb_uint8_t permit_duration;   /**< The length of time in seconds.  0x00 and 0xff indicate that
                                  * permission is disabled or enabled
                                  */
} zb_zdo_signal_permit_join_params_t;
#endif
...
/**
   @brief Permit Join Indication parameters

   Stack passes this parameter to application upon receipt of Permit Join indication
   primitive.
  */
typedef struct zb_zdo_signal_permit_join_indication_params_s
{
  zb_uint8_t permit_duration;   /**< The length of time in seconds.  0x00 and 0xff indicate that
                                  * permission is disabled or enabled
                                  */
} zb_zdo_signal_permit_join_indication_params_t;
...
/** @brief Parameters for Mgmt_Rtg_req.
  * @see ZB spec, subclause 2.4.3.3.3.
  */
...
/** @brief Response for Mgmt_Rtg_rsp.
  * @see ZB spec, subclause 2.4.4.3.3.
  */
typedef ZB_PACKED_PRE struct zb_zdo_mgmt_rtg_resp_s
{
  ...
  zb_uint8_t start_index;                /*!< Starting index within the Routing
                                          * Table to begin rreporting for the RoutingTableList.*/
...
/* bits 0 - 2, mask 0x7 */
/**
   Set status of routing table record to type 'type': bits 0 - 2, mask 0x7;

   @param var - routing table record type_flags
   @param type - routing table status value

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_SET_ROUTE_STATUS(var, type) ( ( var ) &= ~0x7U, ( var ) |= ( type ) )
/**
   Get status of routing table record.

   @param var - routing table record type_flags

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_GET_ROUTE_STATUS(var) ( var & 0x7U )

/* bit 3, mask 0x8 */
/**
   Set no_route_cache Memory Constrained attribute of routing table record to type 'type':
   bit 3, mask 0x8;

   @param var - routing table record type_flags
   @param type - MemoryConstrained value

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_SET_ROUTE_NO_ROUTE_CACHE(var, type) ( ( var ) &= ~0x8U, ( var ) |= (( type ) << 3U) )
/**
   Get no_route_cache Memory Constrained of routing table record.

   @param var - routing table record type_flags

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_GET_ROUTE_NO_ROUTE_CACHE(var) ( (var & 0x8U) >> 3U )

/* bit 4, mask 0x10 */
/**
   Set Many-to-one attribute of routing table record to type 'type':
   bits 4, mask 0x10;

   @param var - routing table record type_flags
   @param type - Many-to-one request value

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_SET_ROUTE_MANY_TO_ONE(var, type) ( ( var ) &= ~0x10U, ( var ) |= (( type ) << 4U) )
/**
   Get Many-to-one attribute of routing table record.

   @param var - routing table record type_flags

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_GET_ROUTE_MANY_TO_ONE(var) ( (var & 0x10) >> 4U )

/* bit 5, mask 0x20 */
/**
   Set Route Record Required attribute of routing table record to type 'type':
   bits 5, mask 0x20;

   @param var - routing table record type_flags
   @param type - Route record required request value

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_SET_ROUTE_RECORD_REQUIRED(var, type) ( ( var ) &= ~0x20U, ( var ) |= (( type ) << 5U) )
/**
   Get Route Record Required attribute of routing table record.

   @param var - routing table record type_flags

   @ref zb_zdo_routing_table_record_s
  */
#define ZB_ZDO_RECORD_GET_ROUTE_RECORD_REQUIRED(var) ( (var & 0x20) >> 5U )
/* GP add, end */
...
/* GP add, start */
/** @brief Sends  Mgmt_Rtg_req (see Zigbee spec 2.4.3.3.3)
   @param param - index of buffer with Rtg request parameters. \ref zb_zdo_mgmt_rtg_param_s
   @param cb    - user's function to call when got response from the remote.
   @return        - ZDP transaction sequence number or 0xFF if operation cannot be
                  performed now (nor enough memory, resources, etc.)
   @ref zb_zdo_mgmt_rtg_resp_s, \ref zb_zdo_routing_table_record_s

   @b Example:
@snippet doxygen_snippets.dox zboss_api_zdo_h_x
...
#ifdef ZB_RAF_PERMIT_JOIN_ZDO_SIGNAL
/**
   @brief sends   send zdo signal to application
   @param param   Permit join duration
