...
#define ZB_APP_DESCR_TIMEOUT (3*ZB_TIME_ONE_SECOND)

#define ZGP_TUNNELING_OPT_GET_FRAME_TYPE(opt)    (opt & 0x3)
#define ZGP_TUNNELING_OPT_GET_MANUF_PRESENT(opt) ((opt >> 2) & 0x1)
#define ZGP_TUNNELING_OPT_GET_DIRECTION(opt)     ((opt >> 3) & 0x1)
...
#define ZB_ZGP_IS_COMM_STATE(state) \
  (ZGP_CTXC().comm_data.state == (state))
...
/**
 * @brief Get runtime bit feild from runtime options
 *
...
/**
 * @brief Set runtime bit feild in runtime options
 *
 * @param ent     [in]  Pointer to table entry
 * @param field   [in]  bit field index
 *
...
/**
 * @brief Reset runtime bit feild current value in runtime options
 *
 * @param ent     [in]  Pointer to table entry
 * @param field   [in]  bit field index
 *
...
#define ZGP_GPS_GET_COMMISSIONING_WINDOW()\
  (ZB_ZGP_COMM_MODE_OPT_GET_ON_COMM_WIND_EXP(\
 ...
 *
 * @param rssi [in]  Incoiming RSSI value
...
#define ZB_ZGP_GP_PCONF_PAIRING_SEND_REQ_CREATE(buf, req, conf, cb)\
  ...
  (req)->sec_options = (conf)->sec_options;                        \
  (req)->security_counter = (conf)->sec_frame_counter;             \
  ZB_MEMCPY((req)->zgpd_key, (conf)->key, sizeof((conf)->key));    \
  (req)->zgpd_assigned_alias = (conf)->assigned_alias;             \
  (req)->groupcast_radius = (conf)->frwd_radius;                   \
  (req)->device_id = (conf)->device_id;                            \
  ZB_MEMCPY(&(req)->sgrp[0], &(conf)->sgrp[0], sizeof((conf)->sgrp)); \
  (req)->callback = (cb)
...
/**
 * @brief Construct GPDF Exntended NWK Frame control for ZGPD outgoing frame
 */
...
/**
 * @brief Perform GP Commissioning Notificatin request
 *
 * @param param          [in]  Buffer reference
