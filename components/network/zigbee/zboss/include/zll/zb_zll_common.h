...
{                                                                                       \
    zb_zll_start_new_nwk_param_t *parm =                                               \
        (zb_zll_start_new_nwk_param_t*)ZB_BUF_GET_PARAM(buffer, zb_zll_start_new_nwk_param_t); \
    (void)zb_buf_reuse((buffer));                                                      \
    parm->pan_id = (panid);                                                            \
    ZB_IEEE_ADDR_COPY(&(parm->ext_pan_id), (ext_panid));                               \
    parm->channel = (channel_index);                                                   \
    ZB_IEEE_ADDR_COPY(&(parm->dst_addr), (dest_addr));                                 \
...
#define ZB_ZLL_JOIN_ROUTER(buffer, dev_index, callback, status)      \
  {                                                                  \
    zb_zll_join_router_param_t *parm;                               \
    (void)zb_buf_reuse((buffer));                                    \
    parm = (zb_zll_join_router_param_t*)ZB_BUF_GET_PARAM(buffer, zb_zll_device_discovery_req_t); \
    parm->device_index = (dev_index);                               \
                                                                     \
...
#define ZB_ZLL_JOIN_ED(buffer, dev_index, callback, status)         \
  {                                                                 \
    zb_zll_join_end_device_param_t *parm;                          \
    (void)zb_buf_reuse((buffer));                                   \
    parm = (zb_zll_join_end_device_param_t*)ZB_BUF_GET_PARAM(buffer, zb_zll_device_discovery_req_t); \
    parm->device_index = (dev_index);                              \
                                                                    \
