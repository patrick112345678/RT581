...
void linux_spi_send_data(uint8_t *buf, uint16_t len)
{
  ...
  if (ret == 0 && tx_ctx.len != 0)
  {
    ...
    /* WARNING: this code removes the previous packet and it won't be sent!
       ...
       It is needed for GTW350 and Juna for the following reason:
  ...
  {
    /* That finally calls sgw_juna_bridge.c:ncp_ll_call_me() which wakes junabra main loop using the control pipe. */
    linux_spi_ctx.send_cb(ret);
  }
}
...
static void *io_thread(void *args)
{
  ...
  while (1)
  {
    ...
    if (can_pass_to_user)
    {
      ...
      /* So there is no risk of recursion - at least for junabra. */
...
static void spi_do_recv(void)
{
  ...
  {
    /* At Slave (typically Nordic) put entire packet into DMA buffer.  Master
...
#define SIGNATURE_LEN sizeof(pkt_hdr.sign)
#else
#define SIGNATURE_LEN 0
#endif
      len = pkt_hdr.hdr.len + SIGNATURE_LEN;
      TRACE_MSG(TRACE_USB3, "read packet header: packet len %d", (FMT__D, len));
    }
    else
    {
      TRACE_MSG(TRACE_ERROR, "transfer ret %d, crc 0x%hx does not match 0x%x", (FMT__D_H_H, ret, crc, pkt_hdr.hdr.crc));
    }
    ...
    if ((len <= SPI_MAX_SIZE) && len)
    {
       ...
       * again. Slave sends packet rest. It is true for both Nordic
       * and TI platforms (but simpler at TI where DMA buffer need not
       * be reloaded) */
...
#ifdef SPI_USEC_BETWEEN_RX
        usleep(SPI_USEC_BETWEEN_RX);
#endif
        /* receive backet rest (body) */
...
static void rx_swap_drv_bufs(void)
{
  ...
  rx_ctx.drv_index = (rx_ctx.drv_index + 1) % SPI_RX_OBJS;
}
...
static void rx_swap_user_bufs(void)
{
  ...
  rx_ctx.user_index = (rx_ctx.user_index + 1) % SPI_RX_OBJS;
}
