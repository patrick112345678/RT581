  ...
  zb_uint_t *alarm_timeout_ms)
{
  ...
  if (llq.res.status != ZBNCP_RET_OK)
  {
    ret = llq.res.status;
  }

  *received_bytes = zbncp_frag_process_rx_response(&frag_ctx, &llq.res);
  ...
  zb_uint_t *alarm_timeout_ms)
{
  ...
  /* If tx complete now but tx buf was busy, let's retry */
  if (tx_done
      && ret == ZBNCP_RET_BUSY)       /* see zbncp_div_store_tx_pkt() */
  {
    zb_uint_t timeout;
    if (*received_bytes)
    {
